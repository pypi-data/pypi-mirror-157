    ...
    def __AssignImportValues(self, wGlobDict:dict, FileName:str) -> None:
        ...
        for k in self.__WorkDict.keys():
            ...
            try:
                ...
                IsOk = False
            if IsOk:
                NameStr = f'{k} (Imported from {FileName} [{self.__Prefix}])'
                if isinstance(iVal,list) or isinstance(iVal,tuple):
                     for iVs in iVal:
                ...
                if Res is not None:
                    raise self.ParamError(Res)
                    # self.__WorkDict[k] = iVal
        for n,c in self.__Children.items():
            c.__AssignImportValues(wGlobDict,FileName=FileName)
