...
class Client():
	...
	def request_verify_code(self, email: str, resetPassword: bool = False):
		...
		**Parameters**
		...
		response = self.session.post(f"{self.api}/g/s/auth/request-security-validation", headers=self.headers(data=data), data=data)
		if response.status_code != 200:return exceptions.checkExceptions(json.loads(response.text))
	...
	def leave_chat(self, chatId: str, comId: str = None):
		...
		** options **
		...
		response = self.session.delete(f"{self.api}/g/s/chat/thread/{chatId}/member/{self.uid}", headers=self.headers())
		if response.status_code != 200: return exceptions.checkExceptions(json.loads(response.text))
		else: return response.status_code
	...
	def get_message_info(self, chatId: str, messageId: str): #do
		...
		** options **
		...
		"""
		response = self.session.get(f"{self.api}/g/s/chat/thread/{chatId}/message/{messageId}", headers=self.headers(data=data))
		if response.status_code != 200: return exceptions.checkExceptions(json.loads(response.text))
	...
	def get_community_info(self, comId: str):
		...
		** options **
		...
		response = self.session.get(f"{self.api}/g/s-x{comId}/community/info?withInfluencerList=1&withTopicList=true&influencerListOrderStrategy=fansCount", headers=self.headers())
		if response.status_code != 200: return exceptions.checkExceptions(json.loads(response.text))
