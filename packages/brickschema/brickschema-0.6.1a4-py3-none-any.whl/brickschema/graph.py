...
class BrickBase(rdflib.Graph):
    ...
    def expand(self, profile, backend=None, simplify=True, ontology_graph=None):
        ...
        if profile == "brick":
            ...
            _PYSHACL_ITERATIONS = 1
            og = rdflib.Graph()
            for t in self.triples((None, None, None)):
                og.add(t)
            for _ in range(_PYSHACL_ITERATIONS):
                valid, _, _ = pyshacl.validate(
                    data_graph=self, shacl_graph=ontology_graph, ont_graph=ontology_graph, advanced=True, allow_warnings=True, abort_on_first=True, inplace=True
                )
            return self
        elif profile == "owlrl":
