...
def create_pipeline(pipeline_package_path: str, pipeline_name: str, **kwargs):
    ...
    Returns:
        kfp_server_api.ApiPipeline: ApiPipeline object.
    """
    if get_pipeline(pipeline_name=pipeline_name):
        now = int(time.time())
        return client.upload_pipeline_version(
            pipeline_package_path, f"{pipeline_name}_{now}", **kwargs
        )
