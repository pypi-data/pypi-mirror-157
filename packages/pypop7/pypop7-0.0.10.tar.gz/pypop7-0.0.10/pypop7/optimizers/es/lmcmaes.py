...
class LMCMAES(ES):
    ...
    def _a_z(self, z=None, pm=None, vm=None, b=None):  # Cholesky factor - vector update
        ...
        for t in range(0, self._it):
            j_cur = self._j[t]
            k = b[j_cur] * np.dot(vm[j_cur], z)
            x = self._a * x + k * pm[j_cur]
        return x
    ...
    def _a_inv_z(self, v=None, vm=None, d=None, i=None):  # inverse Cholesky factor - vector update
        ...
        for t in range(0, i):
            j_cur = self._j[t]
            k = d[j_cur] * np.dot(vm[j_cur], v)
            v = self._c * v - k * vm[j_cur]
        return v

    def _update_distribution(self, mean=None, x=None, p_c=None, s=None, vm=None, pm=None,
                             b=None, d=None, y=None, y_bak=None, it=None):
        ...
        for i in range(i_min, self._it):
            v = np.copy(p_c[self._t[i]])
            v = self._a_inv_z(v, vm, d, i)
            b[j_cur] = self._bd_1 / v_norm * (v_norm_sqrt - 1)
            d[j_cur] = 1 / (self._bd_1 * v_norm) * (1 - 1 / v_norm_sqrt)
        y.sort()
    ...
    def optimize(self, fitness_function=None, args=None):  # for all generations (iterations)
        ...
        while True:
            y_bak = np.copy(y)
            x, y = self.iterate(mean, x, pm, vm, y, b, args, it)  # sample and evaluate offspring population
            if self.record_fitness:
                fitness.extend(y)
            if self._check_terminations():
                break
            mean, p_c, s, vm, pm, b, d = self._update_distribution(mean, x, p_c, s, vm, pm, b, d, y, y_bak, it)
