...
class DepIndex(ComplexityIndex):
    ...
    def _compute_value(self, element: TextElement) -> int:
        return sum(1 for word in element.get_words() if word.dep == self.dep_type.name.lower())

    def __repr__(self):
        return f"{self.reduce_function_abbr}({self.abbr}_{self.dep_type.name.lower()} / {self.reduce_depth_abbr})"
