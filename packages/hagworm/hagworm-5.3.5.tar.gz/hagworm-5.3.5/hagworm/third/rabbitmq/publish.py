...
class RabbitMQProducer:
    ...
    def __init__(self, url, pool_size=10, **kwargs):
...
class RabbitMQProducerForExchange(RabbitMQProducer):
    ...
    def __init__(self, url, exchange_name, exchange_type=aio_pika.ExchangeType.FANOUT, pool_size=10, **kwargs):
    ...
    async def _create_exchange(self):
        ...
        async with self.acquire_channel() as channel:
            return await channel.declare_exchange(self._exchange_name, self._exchange_type)

    def set_exchange_config(self, **kwargs):

        self._exchange_config.update(kwargs)
