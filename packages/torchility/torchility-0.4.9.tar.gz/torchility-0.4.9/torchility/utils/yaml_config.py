...
def load_yaml(yaml_path, encoding='utf8'):
    ...
    yaml file content:
        ...
        path_join: !join [*root, filename]      # use alias
        str_concat: !cat [*root, filename]
        items:
        - 1
        - 2
        - 3
    ...
    load result:
        {
            'data_root': './data',
            'path_join': './data/filename', 
            'str_concat': './datafilename', 
            'items': [1, 2, 3], 
            'key': {
                'key1':'value1',
                'key2': 'value2'
    ...
    yaml.add_constructor('!join', join_fn)
    yaml.add_constructor('!cat', cat_fn)
