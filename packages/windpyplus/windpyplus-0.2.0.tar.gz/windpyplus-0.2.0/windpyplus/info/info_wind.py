...
#from windpyplus.stockPool.allAstock import allAstock
...
def info1(stocklists, tradeDate=None):
    tradeDate = tradedate()[-1]
    data = w.wss(stocklists, "close,sec_name,industry_sw,ipo_date,mkt,sec_status,trade_code,compindex2,concept,",
                 f"tradeDate={tradeDate};priceAdj=F;cycle=D;industryType=3;index=1;unit=1;order=0;rptDate=20211231")
    ...
    #print(data)
    df = pd.DataFrame(data.Data, columns=data.Codes, index=data.Fields).T
    df.to_excel("D:/Stock/wind/info1.xlsx", sheet_name = tradedate(0)[-1])
    #rm._play_wav()
    print(df.head())
    return df   
...
def ROE_filter(stocklists, ROE_limit = 7.0): #TODO
