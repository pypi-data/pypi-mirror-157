...
def get_pic_from_localdb(jjdm,if_percentage=True,show_num=None):
    ...
    for i in range(3):
        ...
        if(i==0):
            # for jj in temp_ind_detail['jjdm'].unique():
            #calculate the industry value and growth rank
            industry_rank = temp_ind_detail.drop_duplicates(['行业名称'])
            industry_rank[
                ['行业主营业务增长率(时序均值)', '行业净利润增长率(时序均值)', '行业净资产收益率(时序均值)',
                 '行业每股净资产与价格比率(时序均值)', '行业每股收益比股价(时序均值)','行业经营现金流与价格比率(时序均值)',
                 '行业股息率(时序均值)']] = industry_rank[
                                       ['行业主营业务增长率(时序均值)', '行业净利润增长率(时序均值)', '行业净资产收益率(时序均值)', '行业每股净资产与价格比率(时序均值)',
                                        '行业每股收益比股价(时序均值)', '行业经营现金流与价格比率(时序均值)',
                                        '行业股息率(时序均值)']].rank(method='min') / len(industry_rank)
            industry_rank['行业综合成长排名'] = industry_rank[['行业净资产收益率(时序均值)', '行业主营业务增长率(时序均值)', '行业净利润增长率(时序均值)']].mean(axis=1)
            industry_rank['行业综合价值排名'] = industry_rank[['行业每股净资产与价格比率(时序均值)','行业每股收益比股价(时序均值)',
                                                       '行业经营现金流与价格比率(时序均值)','行业股息率(时序均值)']].mean(axis=1)
            industry_rank[['行业综合成长排名', '行业综合价值排名']] = industry_rank[['行业综合成长排名', '行业综合价值排名']].rank(method='min') / len(
                industry_rank)
            industry_rank=industry_rank[['行业名称','行业综合成长排名','行业综合价值排名']]

            tempdf = temp_ind_detail[temp_ind_detail['占持仓比例(时序均值)'] >= 0.1]
            tempdf=pd.merge(tempdf,industry_rank,how='left',on='行业名称')

            tempvalue=(tempdf[['净资产收益率(时序均值)', '主营业务增长率(时序均值)', '净利润增长率(时序均值)']].values / tempdf[
                ['行业净资产收益率(时序均值)', '行业主营业务增长率(时序均值)', '行业净利润增长率(时序均值)']].values - 1)
            tempvalue[tempvalue > 3] = 3
            tempvalue[tempvalue < -3] = -3
            tempdf['成长超额']=tempvalue.mean(axis=1)

            tempvalue=(tempdf[['每股净资产与价格比率(时序均值)','每股收益与价格比率(时序均值)',
                                                       '经营现金流与价格比率(时序均值)','股息率(时序均值)']].values / tempdf[
                ['行业每股净资产与价格比率(时序均值)','行业每股收益比股价(时序均值)',
                                                       '行业经营现金流与价格比率(时序均值)','行业股息率(时序均值)']].values - 1)
            tempvalue[tempvalue > 3] = 3
            tempvalue[tempvalue < -3] = -3
            ...
            tempdf=tempdf.groupby('jjdm').sum()[['占持仓比例(时序均值)',
                                                 '行业综合成长排名','行业综合价值排名','成长超额','价值超额','综合价值属性(时序均值)排名','综合成长属性(时序均值)排名']]
            tempdf['成长超额']=tempdf['成长超额']/tempdf['占持仓比例(时序均值)']
            tempdf['价值超额'] = tempdf['价值超额'] / tempdf['占持仓比例(时序均值)']
            tempdf['行业综合成长排名']=tempdf['行业综合成长排名']/tempdf['占持仓比例(时序均值)']
            tempdf['行业综合价值排名'] = tempdf['行业综合价值排名'] / tempdf['占持仓比例(时序均值)']
            ...
            tempdf['行业策略']=''
            tempdf.loc[tempdf['行业综合成长排名']>=0.75,'行业策略']=tempdf.loc[tempdf['行业综合成长排名']>=0.75]['行业策略']+'强成长,'
            tempdf.loc[(tempdf['行业综合成长排名']>=0.5)&(tempdf['行业综合成长排名']<0.75),'行业策略']=tempdf.loc[(tempdf['行业综合成长排名']>=0.5)&(tempdf['行业综合成长排名']<0.75)]['行业策略']+'成长,'
            tempdf.loc[tempdf['行业综合价值排名']>=0.75,'行业策略']=tempdf.loc[tempdf['行业综合价值排名']>=0.75]['行业策略']+'强价值,'
            tempdf.loc[(tempdf['行业综合价值排名']>=0.5)&(tempdf['行业综合价值排名']<0.75),'行业策略']=tempdf.loc[(tempdf['行业综合价值排名']>=0.5)&(tempdf['行业综合价值排名']<0.75)]['行业策略']+'价值,'
            tempdf.loc[tempdf['行业策略']=='','行业策略']='无,'
            tempdf['行业策略']=[x[0:-1] for x in tempdf['行业策略']]


            tempdf['行业个股相对偏好']=''
            tempdf.loc[tempdf['综合成长属性(时序均值)排名']>=0.7,'行业个股相对偏好']=tempdf.loc[tempdf['综合成长属性(时序均值)排名']>=0.7]['行业个股相对偏好']+'成长,'
            # tempdf.loc[(tempdf['综合成长属性(时序均值)排名']>=0.5)&(tempdf['综合成长属性(时序均值)排名']<0.75),'行业个股相对偏好']=tempdf.loc[(tempdf['综合成长属性(时序均值)排名']>=0.5)&(tempdf['综合成长属性(时序均值)排名']<0.75)]['行业个股相对偏好']+'成长,'
            tempdf.loc[tempdf['综合价值属性(时序均值)排名']>=0.7,'行业个股相对偏好']=tempdf.loc[tempdf['综合价值属性(时序均值)排名']>=0.7]['行业个股相对偏好']+'价值,'
            #tempdf.loc[(tempdf['综合价值属性(时序均值)排名']>=0.5)&(tempdf['综合价值属性(时序均值)排名']<0.75),'行业个股相对偏好']=tempdf.loc[(tempdf['综合价值属性(时序均值)排名']>=0.5)&(tempdf['综合价值属性(时序均值)排名']<0.75)]['行业个股相对偏好']+'价值,'
            tempdf.loc[tempdf['行业个股相对偏好']=='','行业个股相对偏好']='无,'
            tempdf['行业个股相对偏好']=[x[0:-1] for x in tempdf['行业个股相对偏好']]

            industry_p=pd.merge(industry_p,tempdf[['行业精选个股策略','行业策略','价值超额', '成长超额', '行业综合成长排名',
       '行业综合价值排名','行业个股相对偏好','综合成长属性(时序均值)排名','综合价值属性(时序均值)排名']],how='left',on='jjdm')
            industry_p=industry_p[['jjdm', '基金简称', '一级行业类型', '一级行业集中度', '一级行业换手率', '前五大行业', '行业策略','行业综合成长排名', '行业综合价值排名','行业精选个股策略','成长超额', '价值超额','龙头占比(时序均值)',
       '龙头占比(时序中位数)', '龙头占比(时序均值)排名', '龙头占比(时序中位数)排名', '二级行业类型', '二级行业集中度',
       '二级行业换手率', '三级行业类型', '三级行业集中度', '三级行业换手率','行业个股相对偏好','综合成长属性(时序均值)排名','综合价值属性(时序均值)排名', 'asofdate']]
        ind_detial_float_col = temp_ind_detail.columns.tolist()
        ind_detial_float_col.sort()
        for col in ['总市值(时序中位数)', '总市值(时序中位数)排名',
                    '总市值(时序均值)', '总市值(时序均值)排名', '行业平均市值(时序中位数)', '25分位数市值(时序中位数)',
    ...
    sql="SELECT * from nav_jj_ret_analysis where {0} and asofdate='{1}' "\
        .format(jjdm,latest_date)
    jj_performance=pd.read_sql(sql,con=localdb)
    ...
    hurst['业绩特征'] = '随机'
    hurst.loc[hurst['H'] <= 0.3,'业绩特征']='趋势反转'
    hurst.loc[hurst['H'] >= 0.7,'业绩特征']='趋势保持'
    ...
    jj_performance=jj_performance[['jjdm','业绩预期回归强度', '长期相对业绩表现', '相对业绩稳定性','业绩特征','month_rank_mean', 'month_rank_std', 'quarter_rank_mean',
       ...
       'calmark_ratio_rank', 'treynor_ratio', 'treynor_ratio_rank',
       'asofdate']]
    ...
    return value_p,value_p_hbs,value_sp,value_sp_hbs, size_p,size_p_hbs,size_sp,size_sp_hbs,\
           industry_p,industry_sp,theme_p,theme_sp,industry_detail_df_list,stock_p,stock_tp,jj_performance
...
def industry_pic_all(jj_base_info,th1=0.5, th2=0.5,if_prv=False):

    if(if_prv):
        property_table='hbs_prv_industry_property_new'
        industry_level_table = 'hbs_prv_industry_property'
        ind_shift_property_table='hbs_prv_industry_shift_property'
        theme_shift_property_table='hbs_prv_theme_shift_property'
        ind_property_pic_table='jjpic_prv_industry_p'
        ind_shift_pic_table='jjpic_prv_industry_sp'
        theme_property_pic_table='jjpic_prv_theme_p'
        theme_shift_pic_table='jjpic_prv_theme_sp'
        industry_level_pic_table = 'jjpic_prv_industry_detail'
    else:
...
def style_pic_all(jj_base_info,fre, th1=0.5, th2=0.5, if_percentage=True,if_prv=False):


    if(if_prv):
        nav_property_table='nav_prv_style_property'
        hbs_property_table='hbs_prv'
        nav_shift_table='nav_prv_shift_property'
        jjpic_table='jjpic_prv'
    else:
...
def save_entire_jjpic2db(if_prv=False):
    if(if_prv):
    ...
    #ind_cen=industry_pic_all(jj_base_info,if_prv=if_prv)
    style_pic_all(jj_base_info,fre='M', if_percentage=False,if_prv=if_prv)
...
def save_pic_as_excelfromlocaldb(jjdm_list=None):
    ...
    value_p, value_p_hbs, value_sp, value_sp_hbs, size_p, size_p_hbs, size_sp, size_sp_hbs, \
    industry_p, industry_sp, theme_p, theme_sp, industry_detail_df_list, stock_p, stock_tp,jj_performance = get_pic_from_localdb(
        jjdm_con)


    filename=r"E:\GitFolder\docs\公募核心池报告.xlsx"
    ...
    df_list=[industry_p, industry_sp, theme_p, theme_sp,
               ...
               stock_p,stock_tp,jj_performance]
...
if __name__ == '__main__':
    ...
    # save_pic_as_excelfromlocaldb(jjdm_list=mutual_core_jjdm_list)
    #
    # jjdm='000001'
