...
class DiscriminativeSubNetwork(nn.Module):
    ...
    def forward(self, batch: Tensor) -> Tensor:
        ...
        segmentation, max_activation_val = self.decoder_segment(act1, act2, act3, act4, act5, act6)
...
class DecoderDiscriminative(nn.Module):
    ...
    def forward(self, act1: Tensor, act2: Tensor, act3: Tensor, act4: Tensor, act5: Tensor, act6: Tensor) -> Tensor:
        ...
        # Return max value of concatenated decoder activations
        max_activation_fn = lambda x: torch.max(einops.rearrange(x, "b c h w -> b c (h w)"), -1)[0]
        max_activation_val = torch.hstack([max_activation_fn(act) for act in [db_b, db1, db2, db3, db4]])


        out = self.fin_out(db4)
        return out, max_activation_val
