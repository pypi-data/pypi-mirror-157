...
def verbnet(doc):
	...
	for name, ibeg, iend in matcher(doc):
		...
		i = ibeg + int(arr[-1]) 
		doc.user_data[f"verbnet-{i}"] = {"type":"verbnet", "start": ibeg, "end": iend, "lem":doc[i].lemma_, "lempos":doc[i].lemma_ + "_" + doc[i].pos_, "chunk":arr[0].strip()}
...
def clause(doc):  # {'S.prep-0': {'type': 'S.prep', 'start': 0, 'end': 2, 'lem': 'consider', 'chunk': 'Considering the possibility'}, 'S.conj-9': {'type': 'S.conj', 'start': 9, 'end': 12, 'lem': 'be', 'chunk': 'she is ok .'}}
	for v in [t for t in doc if t.pos_ == 'VERB' and t.dep_ != 'ROOT' ] : # non-root
		...
		start = children[0].i
		doc.user_data[f"{type}-{start}"] = {"type":"cl", "tag": "S." + v.dep_,  "start": start, "end":children[-1].i + 1, "lem":v.lemma_,"lempos":v.lemma_ + "_" + v.pos_,"chunk": " ".join([c.text for c in v.subtree])}
