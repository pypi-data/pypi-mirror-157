...
class TaskViewSet(BatchActionMixin, viewsets.ModelViewSet):
    ...
    def report(self, request, pk):
        obj = self.get_object()
        rd = request.data
        if rd.get('salt') != obj.salt:
            raise exceptions.PermissionDenied('验证码不正确。')
        obj.data = rd.get('data')
        obj.finish_time = datetime.now()
        obj.status = choices.STATUS_SUCCESS
        obj.save()
        from .signals import task_done
        task_done.send_robust(sender=None, task=obj)
        return response.Response(serializers.TaskSerializer(obj).data)