# coding=utf-8
# --------------------------------------------------------------------------
# Code generated by Microsoft (R) AutoRest Code Generator.
# Changes may cause incorrect behavior and will be lost if the code is
# regenerated.
# --------------------------------------------------------------------------

from msrest.serialization import Model


class CreateUtilityPlatformMeter(Model):
    """CreateUtilityPlatformMeter.

    All required parameters must be populated in order to send to Azure.

    :param meter_code: Required. The meter code <span
     class='property-internal'>Required</span> <span
     class='property-internal'>Must be between 0 and 32 characters</span>
    :type meter_code: str
    :param serial_number: Required. The meter serial number <span
     class='property-internal'>Required</span> <span
     class='property-internal'>Must be between 0 and 255 characters</span>
    :type serial_number: str
    """

    _validation = {
        'meter_code': {'required': True, 'max_length': 32, 'min_length': 0},
        'serial_number': {'required': True, 'max_length': 255, 'min_length': 0},
    }

    _attribute_map = {
        'meter_code': {'key': 'meterCode', 'type': 'str'},
        'serial_number': {'key': 'serialNumber', 'type': 'str'},
    }

    def __init__(self, *, meter_code: str, serial_number: str, **kwargs) -> None:
        super(CreateUtilityPlatformMeter, self).__init__(**kwargs)
        self.meter_code = meter_code
        self.serial_number = serial_number
