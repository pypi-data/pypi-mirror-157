...
class DisruptionProcess(object):
    ...
    **kwargs
    ...
    def __eq__(self, other):
        ...
        return (
            self._random_process_type == other._random_process_type
            and self._disruption_type == other._disruption_type
            and self._disruption_probability == other._disruption_probability
            and self._recovery_probability == other._recovery_probability
            and self._disruption_state_list == other._disruption_state_list
        )
    ...
    def __repr__(self):
        ...
        if self.random_process_type is None:
            ...
            param_str = (
                "disruption_probability={:.6f}, recovery_probability={:.6f}".format(
                    self.disruption_probability, self.recovery_probability
                )
            )
        elif self.random_process_type == "E":
            if (
                not is_list(self.disruption_state_list)
                or len(self.disruption_state_list) <= 8
            ):
                param_str = "disruption_state_list={}".format(
                    self.disruption_state_list
                )
            else:
                param_str = "disruption_state_list={}...".format(
                    self.disruption_state_list[0:8]
                )
        else:
            param_str = ""

        return "DisruptionProcess({:s}, {:s}: {:s})".format(
            self.disruption_type, self.random_process_type, param_str
        )
    ...
    def validate_parameters(self):
        ...
        """
        if self.random_process_type not in (None, "M", "E"):
            raise AttributeError(
                "Valid random_process_type in (None, 'M', 'E') must be provided"
            )
        if self.disruption_type not in (None, "SP", "OP", "TP", "RP"):
            raise AttributeError(
                "Valid disruption_type in (None, 'SP', 'OP', 'TP', 'RP') must be provided"
            )

        if self.random_process_type == "M":
            if self.disruption_probability is None:
                raise AttributeError(
                    "For 'M' (Markovian) disruptions, disruption_probability must be provided"
                )
            if self.disruption_probability < 0 or self.disruption_probability > 1:
                raise AttributeError(
                    "For 'M' (Markovian) disruptions, disruption_probability must be in [0,1]"
                )
            if self.recovery_probability is None:
                raise AttributeError(
                    "For 'M' (Markovian) disruptions, recovery_probability must be provided"
                )
            if self.recovery_probability < 0 or self.recovery_probability > 1:
                raise AttributeError(
                    "For 'M' (Markovian) disruptions, recovery_probability must be in [0,1]"
                )
        elif self.random_process_type == "E":
            if self.disruption_state_list is None:
                raise AttributeError(
                    "For 'E' (explicit) disruptions, disruption_probability_list must be provided"
                )
        ...
        if is_iterable(self.disruption_state_list):
            if period is None:
                ...
                return self.disruption_state_list[
                    period % len(self.disruption_state_list)
                ]
        else:
    ...
    def steady_state_probabilities(self):
        ...
        if self.random_process_type == "M":
            pi_up = self.recovery_probability / (
                self.disruption_probability + self.recovery_probability
            )
            pi_down = self.disruption_probability / (
                self.disruption_probability + self.recovery_probability
            )
        elif self.random_process_type == "E":
