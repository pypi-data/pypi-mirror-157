...
from stockpyl.helpers import (
    ...
    ensure_dict_for_nodes,
    ensure_list_for_nodes,
    is_iterable,
    is_list,
)
    ...
    def deep_equal_to(self, other, rel_tol=1e-8):
        ...
        for n_ind in sorted(self.node_indices):
            ...
            if other_node is None:
                return False
            if not self.get_node_from_index(n_ind).deep_equal_to(
                other_node, rel_tol=rel_tol
            ):
                return False

        return (
            self._period == other._period
            and self.max_max_replenishment_time == other.max_max_replenishment_time
        )
...
def network_from_edges(edges, node_order_in_lists=None, **kwargs):
    ...
    *or* the individual attributes of the demand source (``mean``, ``round_to_int``, etc.).
    ...
    for a in kwargs.keys():
        if (
            not hasattr(network.nodes[0], a)
            and not hasattr(network.nodes[0].demand_source, a)
            and not hasattr(network.nodes[0].inventory_policy, a)
            and not hasattr(network.nodes[0].disruption_process, a)
            and a not in ("demand_type", "policy_type")
        ):
    ...
    if node_order_in_lists is None:
        ...
        if set(node_order_in_lists) != set(network.node_indices):
            raise ValueError(
                "node_order_in_lists does not match nodes contained in edge list"
            )
    ...
    # Build data dict.
    data_dict = build_node_data_dict(
        attribute_dict=kwargs, node_order_in_lists=node_order_in_lists
    )
    ...
    for n in network.nodes:
        ...
        n.local_holding_cost_function = data_dict[n.index].get(
...
def serial_system(
    num_nodes, node_order_in_system=None, node_order_in_lists=None, **kwargs
):
    ...
    *or* the individual attributes of the demand source (``mean``, ``round_to_int``, etc.).
    ...
    edges = [
        (node_order_in_system[k], node_order_in_system[k + 1])
        for k in range(len(node_order_in_system) - 1)
    ]
    ...
    network = network_from_edges(
        edges=edges, node_order_in_lists=node_order_in_lists, **local_kwargs
    )
...
def owmr_system(
    num_retailers, node_order_in_system=None, node_order_in_lists=None, **kwargs
):
    ...
    *or* the individual attributes of the demand source (``mean``, ``round_to_int``, etc.).
    ...
    edges = [
        (node_order_in_system[0], node_order_in_system[k])
        for k in range(1, num_retailers + 1)
    ]
...
def mwor_system(
    num_warehouses, node_order_in_system=None, node_order_in_lists=None, **kwargs
):
    ...
    *or* the individual attributes of the demand source (``mean``, ``round_to_int``, etc.).
    ...
    edges = [
        (node_order_in_system[k], node_order_in_system[-1])
        for k in range(0, num_warehouses)
    ]
