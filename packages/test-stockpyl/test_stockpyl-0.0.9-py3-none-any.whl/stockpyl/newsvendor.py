    ...
    **Example** (Example 4.3):
    ...
    if base_stock_level is None:
        ...
        cost = (
            ...
            * stats.norm.pdf(stats.norm.ppf(alpha, 0, 1))
            * ltd_sd
        )
    else:
...
def newsvendor_normal_cost(
    base_stock_level, holding_cost, stockout_cost, demand_mean, demand_sd, lead_time=0
):
...
def newsvendor_poisson(holding_cost, stockout_cost, demand_mean, base_stock_level=None):
...
def newsvendor_poisson_cost(base_stock_level, holding_cost, stockout_cost, demand_mean):
    """Calculate the cost of using ``base_stock_level`` as the solution to the
    ...
    **Example** (Example 4.1):
    ...
    if demand_mean <= 0:
        raise ValueError("mean must be positive")
    if not is_integer(base_stock_level):
        raise ValueError("base_stock_level must be an integer")
...
def newsvendor_continuous(
    ...
    **Example** (Problem 4.8(b)):
    ...
    if demand_distrib is None and demand_pdf is None:
        raise ValueError("must provide demand_distrib or demand_pdf")

    # For now, raise error if only demand_pdf is provided. TODO: (Need to add this
    # capability.)
    assert (
        demand_distrib is not None
    ), "newsvendor_continuous() does not yet support demand distributions passed as pdf functions"
...
def newsvendor_discrete(
    ...
    **Example** (Example 4.7):
    ...
    # Check that either distribution or pmf have been supplied.
    if demand_distrib is None and demand_pmf is None:
        raise ValueError("must provide demand_distrib or demand_pmf")
...
def myopic_cost(
    ...
    **Example** (Example 4.1):
    ...
    g = newsvendor_normal_cost(
        base_stock_level, holding_cost, stockout_cost, demand_mean, demand_sd
    )

    return (
        purchase_cost * base_stock_level
        + g
...
def set_myopic_cost_to(
    ...
    **Example** (Example 4.1):
    ...
    if c_plus < -holding_cost or c_plus > stockout_cost:
        raise ValueError(
            "set_myopic_cost_to() requires -h_t <= c_t - gamma * c_{t+1} <= p_t"
        )
...
def newsvendor_normal_explicit(
    selling_revenue,
    purchase_cost,
    salvage_value,
    demand_mean,
    ...
    **Example** (Example 4.2):
    ...
    if demand_sd <= 0:
        raise ValueError("demand_sd must be positive")
    if selling_revenue <= purchase_cost:
        raise ValueError("selling_revenue must be > purchase_cost")
    if purchase_cost <= salvage_value:
        raise ValueError("purchase_cost must be > salvage_value")
    ...
    if base_stock_level is None:
        ...
        # Calculate profit.
        profit = (
            (selling_revenue - purchase_cost + stockout_cost) * base_stock_level
