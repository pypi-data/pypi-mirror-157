...
def finite_horizon_dp(
    ...
    **Example**:
    ...
    if num_periods <= 0 or not is_integer(num_periods):
        raise ValueError("num_periods must be a positive integer")
    if terminal_holding_cost < 0:
        raise ValueError("terminal_holding_cost must be non-negative")
    if terminal_stockout_cost < 0:
        raise ValueError("terminal_stockout_cost must be non-negative")

    # Replace scalar parameters with lists (multiple copies of scalar).
    holding_cost = np.array(
        ensure_list_for_time_periods(holding_cost, num_periods, var_name="holding_cost")
    )
    stockout_cost = np.array(
        ensure_list_for_time_periods(
            stockout_cost, num_periods, var_name="stockout_cost"
        )
    )
    purchase_cost = np.array(
        ensure_list_for_time_periods(
            purchase_cost, num_periods, var_name="purchase_cost"
        )
    )
    fixed_cost = np.array(
        ensure_list_for_time_periods(fixed_cost, num_periods, var_name="fixed_cost")
    )
    discount_factor = np.array(
        ensure_list_for_time_periods(
            discount_factor, num_periods, var_name="discount_factor"
        )
    )
    demand_mean = np.array(
        ensure_list_for_time_periods(demand_mean, num_periods, var_name="mean")
    )
    demand_sd = np.array(
        ensure_list_for_time_periods(demand_sd, num_periods, var_name="demand_sd")
    )

    # Validate other parameters.
    if not np.all(np.array(holding_cost[1:]) >= 0):
        raise ValueError("holding_cost must be non-negative")
    if not np.all(np.array(stockout_cost[1:]) >= 0):
        raise ValueError("stockout_cost must be non-negative")
    if not np.all(np.array(purchase_cost[1:]) >= 0):
        raise ValueError("purchase_cost must be non-negative")
    if not np.all(np.array(fixed_cost[1:]) >= 0):
        raise ValueError("fixed_cost must be non-negative")
    if not np.all(np.array(discount_factor[1:]) > 0) or not np.all(
        np.array(discount_factor[1:]) <= 1
    ):
        raise ValueError("discount_factor must be <0 and <=1")
    if not np.all(np.array(demand_mean[1:]) >= 0):
        raise ValueError("demand_mean must be non-negative")
    if not np.all(np.array(demand_sd[1:]) >= 0):
    ...
    d_min = int(
        max(0, round(np.min(demand_mean[1:]) - d_spread * np.max(demand_sd[1:])))
    )
    ...
    if np.any(prob > trunc_tol):
        warnings.warn(
            "Total probability of demand outside demand-truncation range exceeds trunc_tol for at least one "
            "period."
        )
    ...
    Q = [
        economic_order_quantity_with_backorders(
            fixed_cost[t], holding_cost[t], stockout_cost[t], demand_mean[t]
        )[0]
        for t in range(1, num_periods + 1)
    ]
    ...
    x_min = int(
        round(
            np.min(nv[1:])
            - np.max(demand_mean[1:])
            - np.max(demand_sd[1:]) * (s_spread + d_spread)
        )
    )
    x_max = int(
        round(np.max(nv[1:]) + np.max(Q[1:]) + np.max(demand_sd[1:]) * s_spread)
    )
    ...
    while not done:
        ...
        cost_matrix[num_periods + 1, :] = terminal_holding_cost * np.maximum(
            x_range, 0
        ...
        for t in range(num_periods, 0, -1):
            ...
            prob = norm.cdf(d_range + 0.5, demand_mean[t], demand_sd[t]) - norm.cdf(
                d_range - 0.5, demand_mean[t], demand_sd[t]
            )
            ...
            for y in range(x_min, x_max + 1):
                ...
                future_cost = discount_factor[t] * np.dot(
                    prob, cost_matrix[t + 1, y - d_eff - x_min]
                )
                    ...
                    if cost < best_cost:
                        ...
                        if y == x_max and x < x_max:
                            warnings.warn(
                                ...
                                "and retrying: t = {:d}, x = {:d}, y = {:d}.".format(
                                    t, x, y
                                )
                            )
                            abort = True
            ...
            if abort:
                ...
                while (
                    oul_matrix[t, reorder_points[t] + 1 - x_min]
                    == order_up_to_levels[t]
                    and reorder_points[t] < x_max
                ):
                    reorder_points[t] += 1
            ...
            prob_demand_below_range = 1 - norm.cdf(
                reorder_points[t] - x_min, demand_mean[t], demand_sd[t]
            )
            if prob_demand_below_range > trunc_tol:
                warnings.warn(
                    "Probability that demand brings IL below x-truncation range exceeds trunc_tol: t = {:d}, prob = {:f}".format(
                        t, prob_demand_below_range
                    )
                )
...
def myopic_bounds(
    ...
    **Example**:
    ...
    # Validate singleton parameters.
    assert num_periods > 0 and is_integer(
        num_periods
    ), "num_periods must be a positive integer."
    ...
    assert np.all(
        np.array(stockout_cost[1:]) >= 0
    ), "stockout_cost must be non-negative."
    assert np.all(
        np.array(purchase_cost[1:]) >= 0
    ), "purchase_cost must be non-negative."
    assert np.all(np.array(fixed_cost[1:]) >= 0), "fixed_cost must be non-negative."
    assert np.all(np.array(discount_factor[1:]) > 0) and np.all(
        np.array(discount_factor[1:]) <= 1
    ), "discount_factor must be <0 and <=1."
    ...
    for t in range(1, num_periods):
        c_plus[t] = c[t] - discount_factor[t] * c[t + 1]
        if c_plus[t] < -h[t] or c_plus[t] > p[t]:
            raise ValueError(
                "myopic policy requires -h_t <= c_t - gamma * c_{t+1} <= p_t for all t"
            )
    ...
    for t in range(1, num_periods + 1):
        ...
        S_underbar[t], G_S_underbar = myopic(
            h[t], p[t], c[t], c[t + 1], demand_mean[t], demand_sd[t], discount_factor[t]
        )

        # Set S_overbar to y >= S_underbar s.t. G_t(y) = G_t(S_underbar) + gamma_t * K_{t+1}.
        S_overbar[t] = set_myopic_cost_to(
            G_S_underbar + discount_factor[t] * K[t + 1],
            h[t],
            p[t],
            c[t],
            c[t + 1],
        ...
        s_underbar[t] = set_myopic_cost_to(
            G_S_underbar + K[t],
            h[t],
            p[t],
            c[t],
        ...
        if K[t] - discount_factor[t] * K[t + 1] >= 0:
            s_overbar[t] = set_myopic_cost_to(
                G_S_underbar + K[t] - discount_factor[t] * K[t + 1],
                h[t],
                p[t],
                c[t],
                c[t + 1],
