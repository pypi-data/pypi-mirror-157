    ...
    **kwargs
    ...
    def __repr__(self):
        ...
        if self.type is None:
            ...
            param_str = "lo={:.2f}, hi={:.2f}".format(self.lo, self.hi)
        elif self.type == "D":
    ...
    def validate_parameters(self):
        ...
        """
        if self.type not in (None, "N", "UD", "UC", "D", "CD"):
        ...
        if self.type == "N":
            if self.mean is None:
                raise AttributeError("For 'N' (normal) demand, mean must be provided")
            if self.mean < 0:
                raise AttributeError(
            ...
            if self.mean is None:
                raise AttributeError("For 'P' (Poisson) demand, mean must be provided")
            if self.mean < 0:
                raise AttributeError(
                    "For 'P' (Poisson) demand, mean must be non-negative"
                )
        elif self.type == "UD":
            if self.lo is None:
                raise AttributeError(
            ...
            if self.lo < 0 or not is_integer(self.lo):
                raise AttributeError(
            ...
            if self.hi < 0 or not is_integer(self.hi):
                raise AttributeError(
            ...
            if len(self.demand_list) != len(self.probabilities):
                raise AttributeError(
    ...
    def lead_time_demand_distribution(self, lead_time):
        ...
        if self.type in ("UC", "UD", "CD") and not is_integer(lead_time):
            raise ValueError(
                "lead_time must be an integer for 'UC', 'UD', or 'CD' demand"
            )

        # Get distribution object.
        if self.type == "N":
            return scipy.stats.norm(
                self.mean * lead_time, self.standard_deviation * np.sqrt(lead_time)
            )
        elif self.type == "P":
            return scipy.stats.poisson(self.mean * lead_time)
        elif self.type == "UC":
            distribution = sum_of_continuous_uniforms_distribution(
                lead_time, self.lo, self.hi
            )
        elif self.type == "UD":
            distribution = sum_of_discrete_uniforms_distribution(
                lead_time, self.lo, self.hi
            )
        elif self.type == "CD":
            ...
            for x in range(min_demand, max_demand + 1):
                if x in self.demand_list:
                    ...
                    p.append(0)
            distribution = sum_of_discretes_distribution(
                lead_time, min_demand, max_demand, p
            )
        else:
            return None
