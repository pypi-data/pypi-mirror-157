...
def economic_order_quantity_with_backorders(
    fixed_cost,
    holding_cost,
    stockout_cost,
    demand_rate,
    order_quantity=None,
    stockout_fraction=None,
):
    ...
    **Example** (Example 3.8):
    ...
    if order_quantity is not None and order_quantity <= 0:
        raise ValueError("order_quantity must be positive.")
    if stockout_fraction is not None and (
        stockout_fraction < 0 or stockout_fraction > 1
    ):
        raise ValueError("stockout_fraction must be between 0 and 1.")

    # Check that both or neither order_quantity and stockout_fraction are  provided.
    if (order_quantity is None and stockout_fraction is not None) or (
        order_quantity is not None and stockout_fraction is None
    ):
        raise ValueError(
            "You must provide both order_quantity and stockout_fraction or neither."
        )
    ...
    if order_quantity is None:
        ...
        # Caclulate cost.
        cost = (
            holding_cost * order_quantity * (1 - stockout_fraction) ** 2 / 2
...
def economic_production_quantity(
    fixed_cost, holding_cost, demand_rate, production_rate, order_quantity=None
):
    ...
    **Example**:
    ...
    if demand_rate >= production_rate:
        raise ValueError("demand_rate must be less than production_rate.")
    ...
    if order_quantity is None:
        ...
        cost = (
            fixed_cost * demand_rate / order_quantity
            + holding_cost * (1 - rho) * order_quantity / 2
        )

    return order_quantity, cost


def joint_replenishment_problem_silver_heuristic(
    shared_fixed_cost, individual_fixed_costs, holding_costs, demand_rates
):
    ...
    **Example**:
    ...
    # Check that parameters are non-negative/positive.
    if shared_fixed_cost < 0:
        raise ValueError("shared_fixed_cost must be non-negative")
    if not np.all(np.array(individual_fixed_costs) >= 0):
        raise ValueError("individual_fixed_costs must be non-negative.")
    if not np.all(np.array(holding_costs) > 0):
        raise ValueError("holding_costs must be non-negative.")
    if not np.all(np.array(demand_rates) > 0):
        raise ValueError("demand_rates must be non-negative.")
    if len(individual_fixed_costs) != len(holding_costs) or len(holding_costs) != len(
        demand_rates
    ):
    ...
    # Calculate ratios.
    ratio = [
        individual_fixed_costs[n] / (holding_costs[n] * demand_rates[n])
    ...
    const = (
        holding_costs[min_ratio_prod]
        * demand_rates[min_ratio_prod]
    ...
    term1 = shared_fixed_cost + np.sum(
        np.divide(individual_fixed_costs, order_multiples)
    )
    term2 = np.sum(
        [
            holding_costs[n] * order_multiples[n] * demand_rates[n]
            for n in range(num_prod)
        ]
    )
    ...
    order_quantities = [
        base_cycle_time * order_multiples[n] * demand_rates[n] for n in range(num_prod)
    ]
