...
def optimize_committed_service_times(
    num_nodes=None,
    ...
    *International Journal of Production Economics*, 24:103-113, 1991.
    ...
    if network is None and (
        num_nodes is None
        or local_holding_cost is None
        or processing_time is None
        or demand_bound_constant is None
        or external_outbound_cst is None
        or external_inbound_cst is None
        or (
            (demand_mean is None or demand_standard_deviation is None)
            and demand_source is None
        )
    ):
        raise ValueError(
            "You must provide either network or num_nodes, ..., demand_bound_constant"
        )
    ...
    if network:
        ...
        for n in range(1, num_nodes + 1):
            node = SupplyChainNode(
                index=n,
                network=network,
                local_holding_cost=ensure_dict_for_nodes(
                    local_holding_cost, node_indices=list(range(1, num_nodes + 1))
                )[n],
                processing_time=ensure_dict_for_nodes(
                    processing_time, node_indices=list(range(1, num_nodes + 1))
                )[n],
                demand_bound_constant=ensure_dict_for_nodes(
                    demand_bound_constant, node_indices=list(range(1, num_nodes + 1))
                )[n],
            )
            if n == 1:
...
def _cst_dp_serial(network):
    ...
    for k_index in range(num_nodes, 0, -1):
        ...
        if k_index == num_nodes:
            ...
            k.max_replenishment_time = (
                upstream.max_replenishment_time + k.processing_time
            )
    ...
    for k_index in range(1, num_nodes + 1):
        ...
        for SI in SI_range:
            if k_index == 1:
                ...
                theta[k_index][SI] = (
                    k.local_holding_cost
                    * k.demand_bound_constant
                    * sigma
                    * np.sqrt(max(0, SI + k.processing_time - k.external_outbound_cst))
                )
    ...
    # Get optimal cost.
    opt_cost = theta[num_nodes][
        network.get_node_from_index(num_nodes).external_inbound_cst
    ]
