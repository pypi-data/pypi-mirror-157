...
def dict_match(d1, d2, require_presence=False, rel_tol=1e-9, abs_tol=0.0):
    ...
    for key in d1.keys():
        if key in d2:
            ...
            if (
                not math.isclose(d1[key], 0, rel_tol=rel_tol, abs_tol=abs_tol)
                or require_presence
            ):
                match = False
    ...
    for key in d2.keys():
        if key in d2:
            ...
            if (
                not math.isclose(d2[key], 0, rel_tol=rel_tol, abs_tol=abs_tol)
                or require_presence
            ):
                match = False
    ...
    if is_iterable(x):
        if len(x) == num_periods + 1:
            ...
            if var_name is None:
                ...
                vname = var_name
            raise ValueError(
                "{:s} must be a singleton or a list of length num_periods or num_periods+1".format(
                    vname
                )
            )
    else:
    ...
    if x is None:
        ...
        if is_iterable(x):
            if len(list(x)) == num_nodes:
                ...
                raise ValueError(
                    "x must be a singleton, a list of length num_nodes, or None"
                )
        else:
            return [x] * num_nodes
    ...
    for a in attribute_dict:
        ...
        if attribute_dict[a] is None:
            ...
            if len(list(attribute_dict[a])) == len(node_order_in_lists):
                ...
                raise ValueError(
                    "attribute_dict[a] must be a singleton, dict, list with the same length as node_indices, or None for all a in attribute_names"
                )
    ...
    if type(x) == dict:
        ...
        if is_iterable(x):
            if len(list(x)) == len(node_indices):
                ...
                raise ValueError(
                    "x must be a singleton, dict, list with the same length as node_indices, or None"
                )
        else:
...
def sort_dict_by_keys(d, ascending=True, return_values=True):
    ...
    if return_values:
        ...
        return_list = [
            value
            for _, value in sorted(dict_without_none.items(), reverse=not ascending)
        ]
        ...
        return_list = [
            key for key, _ in sorted(dict_without_none.items(), reverse=not ascending)
        ]
...
def sum_of_discrete_uniforms_distribution(n, lo, hi):
    ...
    distribution = stats.rv_discrete(
        name="sum_of_discrete_uniforms", values=(list(pmf.keys()), list(pmf.values()))
    )
...
def sum_of_discretes_distribution(n, lo, hi, p):
    ...
    if len(p) != hi - lo + 1:
        raise ValueError(
            "p must have length hi - lo + 1. (If the support of the discrete distribution \
