    ...
    **kwargs
    ...
    def __eq__(self, other):
        ...
        * Does not check equality of ``_node``.
        ...
        return (
            ...
            and self._base_stock_level == other._base_stock_level
            and self._order_quantity == other._order_quantity
            and self._reorder_point == other._reorder_point
            and self._order_up_to_level == other._order_up_to_level
        )
    ...
    def __repr__(self):
        ...
        if self.type is None:
            ...
            param_str = "reorder_point={:.2f}, order_up_to_level={:.2f}".format(
                self.reorder_point, self.order_up_to_level
            )
        elif self.type == "rQ":
            param_str = "reorder_point={:.2f}, order_quantity={:.2f}".format(
                self.reorder_point, self.order_quantity
            )
        elif self.type == "FQ":
    ...
    def validate_parameters(self):
        ...
        """
        if self.type not in (None, "BS", "sS", "rQ", "EBS", "BEBS"):
            raise AttributeError(
                "Valid type in (None, 'BS', 'sS', 'rQ', 'EBS', 'BEBS') must be provided"
            )

        if self.type == "BS":
            if self.base_stock_level is None:
                raise AttributeError(
                    "For 'BS' (base-stock) policy, base_stock_level must be provided"
                )
        elif self.type == "sS":
            if self.reorder_point is None:
                raise AttributeError(
                    "For 'sS' (s,S) policy, reorder_point must be provided"
                )
            if self.order_up_to_level is None:
                raise AttributeError(
                    "For 'sS' (s,S) policy, order_up_to_level must be provided"
                )
            if self.reorder_point <= self.order_up_to_level:
                raise AttributeError(
                    "For 'sS' (s,S) policy, reorder_point must be <= order_up_to_level"
                )
        elif self.type == "rQ":
            if self.reorder_point is None:
                raise AttributeError(
                    "For 'rQ' (r,Q) policy, reorder_point must be provided"
                )
            if self.order_quantity is None:
                raise AttributeError(
                    "For 'rQ' (r,Q) policy, order_quantity must be provided"
                )
        if self.type == "EBS":
            if self.base_stock_level is None:
                raise AttributeError(
                    "For 'EBS' (echelon base-stock) policy, base_stock_level must be provided"
                )
        if self.type == "BEBS":
            if self.base_stock_level is None:
                raise AttributeError(
                    "For 'BEBS' (balanced echelon base-stock) policy, base_stock_level must be provided"
                )
    ...
    def get_order_quantity(
        ...
        if inventory_position is not None:
            ...
            if self.type == "FQ":
                ...
                # external demand.
                demand = self.node._get_attribute_total(
                    "inbound_order", self.node.network.period
                )
        ...
        if self.type == "BS":
            ...
            if echelon_inventory_position_adjusted is not None:
                ...
                if self.node.index == max(self.node.network.node_indices):
                    ...
                    partner_node = self.node.network.get_node_from_index(
                        self.node.index + 1
                    )
                    EIPA = (
                        partner_node.state_vars_current._echelon_inventory_position_adjusted()
                    )
    ...
    def _get_order_quantity_balanced_echelon_base_stock(
        self, echelon_inventory_position, echelon_inventory_position_adjusted
    ):
