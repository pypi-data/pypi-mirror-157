...
def optimize_base_stock_levels(
	...
	**Example** (Example 6.1):
    ...
    elif demand_source.type == "CD" and np.all(
        [is_integer(d) for d in demand_source.demand_list]
    ):
        discrete_distribution = True
    else:
    ...
    for j in range(1, N + 1):
        ...
        if j > 1:
            C_hat_lim2[j, :] = (
                h[j] * x_ext + C[j - 1, find_nearest(x, S_star[j - 1], True)]
            )
        else:
        ...
        fd = np.array(
            [
                ltd_dist.cdf(d_val + d_delta / 2) - ltd_dist.cdf(d_val - d_delta / 2)
                for d_val in d
            ]
        )
        ...
        for y in x:
            ...
            for d_ind in range(d.size):
                ...
                if y_minus_d < np.min(x):
                    the_cost[d_ind] = C_hat_lim1[
                        j, find_nearest(x_ext, y_minus_d, True)
                    ]
                elif y_minus_d > np.max(x):  # THIS SHOULD NEVER HAPPEN
                    the_cost[d_ind] = C_hat_lim2[
                        j, find_nearest(x_ext, y_minus_d, True)
                    ]
                else:
    ...
    temp_S_star = {
        n_ind: S_star[old_to_new_dict[n_ind]] for n_ind in old_to_new_dict.keys()
    }
    S_star = temp_S_star
...
def newsvendor_heuristic(
    ...
    **Example** (Example 6.1):
    ...
    for j in indices:
        sum_ltd_dist[j] = demand_source.lead_time_demand_distribution(
            float(np.sum(L[1 : (j + 1)]))
        )
    ...
    for j in indices:
        ...
        if demand_source.type == "N":
            ...
            ltd_distrib = demand_source.lead_time_demand_distribution(
                float(np.sum(L[1 : (j + 1)]))
            )
            # Calculate newsvendor quantities.
            ...
            raise ValueError(
                f"demand_source.type '{demand_source.type}' is not supported"
            )
    ...
    # Revert to original node indexing.
    temp_S_heur = {
        n_ind: S_heur[old_to_new_dict[n_ind]] for n_ind in old_to_new_dict.keys()
    }
    S_heur = temp_S_heur
...
def _preprocess_parameters(
    ...
    if network is None:
        if (
            num_nodes is None
            or echelon_holding_cost is None
            or lead_time is None
            or stockout_cost is None
        ):
            raise ValueError(
                "You must provide either network or num_nodes, ..., stockout_cost"
            )
    if (
        network is None
        and (demand_mean is None or demand_standard_deviation is None)
        and demand_source is None
    ):
        raise ValueError(
            "You must provide either demand_mean and demand_standard_deviation, or demand_source"
        )
    ...
    if network:
        ...
        if node_order_in_system is None:
            ...
            if num_nodes is None:
                raise ValueError(
                    "Either num_nodes, node_order_in_system, or network must be provided"
                )
        ...
        if demand_source is None:
            demand_source = DemandSource(
                type="N", mean=demand_mean, standard_deviation=demand_standard_deviation
            )
        # Build serial system.
    ...
    echelon_holding_cost_dict = {
        node.index: node.echelon_holding_cost for node in local_network.nodes
    }
