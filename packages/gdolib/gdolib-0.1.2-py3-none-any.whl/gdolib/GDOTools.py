...
try:
	import requests , user_agent , json , secrets , names , hashlib , urllib , uuid , re 
...
class Azoz_check_email:	
	...
	def facebook(email: str) -> str:
		...
		if str('The password you entered is incorrect. Please try again') in str(res):
 			return {'status':'Success','email':True}
...
class gdo_drow:
	...
	def get_email():
		...
		if '"protocol"' in pro.text or '"ip"' in pro.text or '"port"' in pro.text:				
			if str(pro.json()['protocol']) == 'socks5':
				...
				rem = requests.get('https://10minutemail.net/address.api.php',headers = {'cookie':'PHPSESSID='+str(secrets.token_hex(8)*2)},proxies={'socks5':proxy})				
				ema = rem.json()['mail_get_mail'].split("@")[0]					
				email = ema + domin					
				return email				
			else:					
				return names.get_first_name()+domin
		else:
			return names.get_first_name()+domin
	...
	def get_info_ip(ip: str) -> str:
        ...
        'sec-ch-ua-mobile': '?1',
        'save-data': 'on',
        'user-agent': 'Mozilla/5.0 (Linux; Android 8.0.0; SM-G930V) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.92 Mobile Safari/537.36',
        'sec-ch-ua-platform': '"Android"',
...
class login_IG:	
	...
	def Login_instagram(self):
		...
		elif ('challenge_required') in str(res.text):
...
class session_IG:
	...
	def login_user_pass(username: str, password: str) -> str:
		try:
			header = {"User-Agent": str(generate_user_agent())}
			with requests.Session() as azoz:
				url_tok = "https://www.instagram.com/"
				data = azoz.get(url_tok, headers=header).content
				token = re.findall('{"config":{"csrf_token":"(.*)","viewer"', str(data))[0]
			headers = {
	...
	def follower_session(user: str, session: str) -> str:
		...
		'content-length': '0',
		'content-type': 'application/x-www-form-urlencoded',
		'cookie': 'mid=YfHAnwALAAFCLfUn6sJVurIEyEfr; ig_did=DF84157E-69D8-46D5-B090-33F4741C808C; ig_nrcb=1; csrftoken='+str(gdo_drow.csrf_token()['csrf_token'])+'; ds_user_id='+str(session.split(':')[0])+'; sessionid='+str(session)+'; rur="CLN\05453352662133\0541684424578:01f726c10813bcb9113b0f4dcf1be24a5278c0fed11c90001622f46b9b5fdf7010e66eda"',
		'origin': 'https://www.instagram.com',
		...
		'user-agent': str(generate_user_agent()),
		'x-asbd-id': '198387',
		'x-csrftoken': str(gdo_drow.csrf_token()['csrf_token']),
		'x-ig-app-id': '936619743392459',
	...
	def like_post(url: str, session: str) -> str:
		id_ = gdo_drow.id_post(url)
		url = 'https://www.instagram.com/web/likes/{id_}/like/'
		headers = {
		'accept': '*/*',
		'accept-encoding': 'gzip, deflate, br',
		'accept-language': 'ar,en-US;q=0.9,en;q=0.8',
		'content-length': '0',
		'content-type': 'application/x-www-form-urlencoded',
		'cookie': 'mid=YfHAnwALAAFCLfUn6sJVurIEyEfr; ig_did=DF84157E-69D8-46D5-B090-33F4741C808C; ig_nrcb=1; shbid="19368\05452914264168\0541682536774:01f7c91f0322a7914c4967e12bcfa34c11317752f470517e402d83ba56c43dd701276cd8"; shbts="1651000774\05452914264168\0541682536774:01f704008a362d517796cf36108b67ea9ace8dbfcadcd2a319c9f940d90aa3647ebd9dbe"; csrftoken='+str(gdo_drow.csrf_token()['csrf_token'])+'; ds_user_id='+str(session.split(':')[0])+'; sessionid='+str(session)+'; rur="CLN\05452914264168\0541682560907:01f70b26a7341573483af51f1be279a94d451a4ebb66ffdf3419bec3f98f6850794bf7f7"',
		'origin': 'https://www.instagram.com',
		'referer': 'https://www.instagram.com/p/'+str(id_),
		'sec-ch-ua': '" Not A;Brand";v="99", "Chromium";v="100", "Google Chrome";v="100"',
		'sec-ch-ua-mobile': '?0',
		'sec-ch-ua-platform': '"Windows"',
		'sec-fetch-dest': 'empty',
