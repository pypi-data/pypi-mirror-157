    ...
    def _make_conn(self):
        try:
            ...
            pool = redis.ConnectionPool(
                host=self.host, password=self.password, port=self.port, db=self.db, decode_responses=True, max_connections=self.max_connections, socket_timeout=60, socket_connect_timeout=30, socket_keepalive=True)
...
class XRedisAsync(Singleton):
    ...
    async def _make_conn(self):
        try:
            ...
            print('创建新连接')
            pool = coredis.ConnectionPool(host=self.host, password=self.password, port=self.port, db=self.db, decode_responses=True, max_connections=self.max_connections, socket_timeout=60, socket_connect_timeout=30, socket_keepalive=True)
            self.conn = coredis.StrictRedis(connection_pool=pool)
