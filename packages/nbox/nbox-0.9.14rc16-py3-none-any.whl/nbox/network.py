...
def deploy_serving(*args, **kwargs):
  """Use the NBX-Deploy Infrastructure"""
  raise NotImplementedError("Changing the definition of NBX-Deploy")
...
def deploy_job(
  ...
  *,
  ...
  # zip all the files folder
  zip_path = U.join(gettempdir(), f"nbxjd_{job_id}@{workspace_id}.nbox")
  ...
  with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zip_file:
    ...
    # add exe.py file -> from nbox v0.9.14c16 this is now anonymously generated later to be migrated to Timeless
    assets = U.join(U.folder(__file__), "assets")
    path = U.join(assets, "exe.jinja")
    exe_path = U.join(gettempdir(), "exe.py")
    logger.debug(f"Writing exe to: {exe_path}")
    with open(path, "r") as f, open(exe_path, "w") as f2:
  ...
  job_proto.code.MergeFrom(JobProto.Code(
    size = max(os.stat(zip_path).st_size / (1024 ** 2), 1), # MiB
