...
class Applications(Base):
    ...
    def _add_fields(self, source: dict, destination: dict):
        ...
        """
        if Base.remove_fields_on_destination:
            self.log(f"Removing fields from application '{source['name']}' on destination instance")
            destination["fields"] = source["fields"]
            self.log(f"Successfully removed fields from application '{source['name']}' on destination instance")
        else:
        ...
        if not self._is_in_include_exclude_lists(application["name"], "applications"):
            ...
            if not dest_application:
                ...
                if not Base.dry_run:
                    ...
                    self.log(f"Updating destination application '{dest_application['name']}' layout.")
                    dest_application["layout"] = application["layout"]
                    self.log(f"Successfully update destination application '{dest_application['name']}' layout.")
