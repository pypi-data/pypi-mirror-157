...
class Tasks(Base):
    ...
    def _get_dest_plugin_dict(self):
        # TODO: Improve documentation and update logic
                ...
                if plugin:
                    ...
                    if _plugin.availableActionDescriptors:
                        for action in _plugin.availableActionDescriptors:
                            ...
                            # {"actionType": {"id": "adsfom", "imageId": "asdfsadfsd"}}
    ...
    def update_task_plugin_ids(self, task: Task):
        ...
        if "python" not in task.action.descriptor.actionType:
            ...
            if self._plugin_dict.get(plugin_name):
                if self._plugin_dict[plugin_name].get(action_type):
                    ...
                    self.log(
                        ...
                        f"'{self._plugin_dict[plugin_name]['fileId']}'"
                    )
                    task.action.descriptor.packageDescriptor.fileId = self._plugin_dict[plugin_name]["fileId"]
                    self.log(
                        f"Updating task '{task.name}' with correct imageId "
                        f"'{self._plugin_dict[plugin_name][action_type]['imageId']}'"
                    )
                    task.action.descriptor.imageId = self._plugin_dict[plugin_name][action_type]["imageId"]
                    self.log(
                        f"Updating task '{task.name}' with correct packageDescriptorId "
                        f"'{self._plugin_dict[plugin_name][action_type]['id']}'"
                    )
                    task.action.packageDescriptorId = self._plugin_dict[plugin_name][action_type]["id"]
        return task
