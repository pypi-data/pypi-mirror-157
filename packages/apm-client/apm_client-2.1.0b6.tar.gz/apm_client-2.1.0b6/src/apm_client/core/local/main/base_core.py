    ...
    try:
        ...
        for command in commands:
            if 'TagSpan' in command:
                ...
                utc = pytz.utc
                try:
                    start_time = datetime.strptime(command['timestamp'], '%Y-%m-%dT%H:%M:%S.%fZ').astimezone(pytz.utc)
                except:
                    start_time = datetime.strptime(command['timestamp'], '%Y-%m-%dT%H:%M:%SZ').astimezone(pytz.utc)
