import pandas as pd
from iwin_cli.utils import success, error
...
def validate_class(excel):
    ...
    try:
        ...
        if key_a.intersection(key_b) == key_b:
            ...
            error(f'[小组] 表单结构错误！！！ {key_a}')
        
        key_a = set(dfs['学员名单'].keys())
        key_b = set(['姓名', '邮箱', '学员登录ID（确保唯一性）', '初始密码', '简介', '小组'])
        if key_a.intersection(key_b) == key_b:
            success(f'[学员名单] 表单结构正确。====')
            l = len(dfs['学员名单'])
            print(f'学员人数：{l}')
        else:
            error(f'[学员名单] 表单结构错误！！！ {key_a}')
