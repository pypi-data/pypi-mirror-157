...
def get_pytz_type_info(pytz_type):
    if isinstance(pytz_type, pytz._FixedOffset):
        rkk__ovfio = pd.Timedelta(pytz_type._offset).value
    else:
        rkk__ovfio = pytz_type.zone
        if rkk__ovfio not in pytz.all_timezones_set:
            raise BodoError(
...
def nanoseconds_to_offset(nanoseconds):
    cpvpd__ycu = nanoseconds // (60 * 1000 * 1000 * 1000)
    return pytz.FixedOffset(cpvpd__ycu)
...
class PandasDatetimeArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        dgaa__btky = [('data', fe_type.data_array_type)]
        models.StructModel.__init__(self, dmm, fe_type, dgaa__btky)
...
def unbox_pd_datetime_array(typ, val, c):
    skjs__sonqn = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    fxdl__imd = c.pyapi.string_from_constant_string('datetime64[ns]')
    ktmuz__yixa = c.pyapi.call_method(val, 'to_numpy', (fxdl__imd,))
    skjs__sonqn.data = c.unbox(typ.data_array_type, ktmuz__yixa).value
    c.pyapi.decref(ktmuz__yixa)
    gyy__neyqw = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(skjs__sonqn._getvalue(), is_error=gyy__neyqw)
...
def box_pd_datetime_array(typ, val, c):
    skjs__sonqn = cgutils.create_struct_proxy(typ)(c.context, c.builder,
        value=val)
    c.context.nrt.incref(c.builder, typ.data_array_type, skjs__sonqn.data)
    vmmx__pkfvb = c.pyapi.from_native_value(typ.data_array_type,
        skjs__sonqn.data, c.env_manager)
    rpbkl__sdls = c.context.get_constant_generic(c.builder, types.
        unicode_type, 'ns')
    arnp__vdii = c.pyapi.from_native_value(types.unicode_type, rpbkl__sdls,
        c.env_manager)
    if isinstance(typ.tz, str):
        hugzq__fgcu = c.context.get_constant_generic(c.builder, types.
            unicode_type, typ.tz)
        dcsjz__npd = c.pyapi.from_native_value(types.unicode_type,
            hugzq__fgcu, c.env_manager)
    else:
        lrx__gmv = nanoseconds_to_offset(typ.tz)
        dcsjz__npd = c.pyapi.unserialize(c.pyapi.serialize_object(lrx__gmv))
    iwds__tuhh = c.context.insert_const_string(c.builder.module, 'pandas')
    slsk__fvwgq = c.pyapi.import_module_noblock(iwds__tuhh)
    luwho__hqh = c.pyapi.call_method(slsk__fvwgq, 'DatetimeTZDtype', (
        arnp__vdii, dcsjz__npd))
    jpabs__muphy = c.pyapi.object_getattr_string(slsk__fvwgq, 'arrays')
    enr__fqth = c.pyapi.call_method(jpabs__muphy, 'DatetimeArray', (
        vmmx__pkfvb, luwho__hqh))
    c.pyapi.decref(vmmx__pkfvb)
    c.pyapi.decref(arnp__vdii)
    c.pyapi.decref(dcsjz__npd)
    c.pyapi.decref(slsk__fvwgq)
    c.pyapi.decref(luwho__hqh)
    c.pyapi.decref(jpabs__muphy)
...
def init_pandas_datetime_array(typingctx, data, tz):
    ...
    def codegen(context, builder, sig, args):
        data, tz = args
        ryip__cqg = cgutils.create_struct_proxy(sig.return_type)(context,
            builder)
        ryip__cqg.data = data
        context.nrt.incref(builder, sig.args[0], data)
        return ryip__cqg._getvalue()
    if is_overload_constant_str(tz) or is_overload_constant_int(tz):
        hugzq__fgcu = get_literal_value(tz)
    else:
        raise BodoError('tz must be a constant string or Fixed Offset')
    herm__doc = DatetimeArrayType(hugzq__fgcu)
    sig = herm__doc(herm__doc.data_array_type, tz)
    return sig, codegen
...
def lower_constant_pd_datetime_arr(context, builder, typ, pyval):
    sdncr__uair = context.get_constant_generic(builder, typ.data_array_type,
        pyval.to_numpy('datetime64[ns]'))
    cfnmp__sfboq = lir.Constant.literal_struct([sdncr__uair])
    return cfnmp__sfboq
...
def overload_getitem(A, ind):
    ...
    if is_list_like_index_type(ind) and ind.dtype == types.bool_:
        ...
        def impl_bool(A, ind):
            ...
            qmya__miwj = ensure_contig_if_np(A._data[ind])
            return init_pandas_datetime_array(qmya__miwj, tz)
        return impl_bool
...
def unwrap_tz_array_equiv(self, scope, equiv_set, loc, args, kws):
    ...
    isi__alw = args[0]
    if equiv_set.has_shape(isi__alw):
        return ArrayAnalysis.AnalyzeResult(shape=isi__alw, pre=[])
    return None
