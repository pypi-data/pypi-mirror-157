...
def box_regex_flag(typ, val, c):
    gmngc__dgy = c.box(types.int64, val)
    otqix__fwcb = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(otqix__fwcb, (gmngc__dgy,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    gmngc__dgy = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, gmngc__dgy)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    eobn__mup = context.get_python_api(builder)
    krp__oavgq = builder.icmp_signed('==', val, eobn__mup.borrow_none())
    with builder.if_then(krp__oavgq):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    pcud__ojjsk = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(pcud__ojjsk)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        eagsv__qwh = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if eagsv__qwh > 1:
            typ = types.List(types.Tuple([string_type] * eagsv__qwh))
...
def overload_match_group(m, *args):
    ...
    vplgm__ezra = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            oorb__iaf = args[0]
            with numba.objmode(out=vplgm__ezra):
                out = m.group(oorb__iaf)
            return out
        return _match_group_impl_one
    uyagl__rcgb = 'tuple_str_{}'.format(len(args))
    setattr(types, uyagl__rcgb, types.Tuple([vplgm__ezra] * len(args)))
    xxg__xxl = ', '.join('group{}'.format(zqtvy__jwy + 1) for zqtvy__jwy in
        range(len(args)))
    nvcke__axm = 'def _match_group_impl(m, *args):\n'
    nvcke__axm += '  ({}) = args\n'.format(xxg__xxl)
    nvcke__axm += "  with numba.objmode(out='{}'):\n".format(uyagl__rcgb)
    nvcke__axm += '    out = m.group({})\n'.format(xxg__xxl)
    nvcke__axm += '  return out\n'
    fox__vxkl = {}
    exec(nvcke__axm, globals(), fox__vxkl)
    jdlrq__dlp = fox__vxkl['_match_group_impl']
    return jdlrq__dlp
...
def overload_match_groups(m, default=None):
    umpj__udme = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=umpj__udme):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(occp__wxib is None for occp__wxib in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=vplgm__ezra):
