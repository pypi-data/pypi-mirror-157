...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        xrgmu__xjja = self.overloads[sig]
        gcq__zejy = xrgmu__xjja.metadata.get('distributed_diagnostics', None)
        if gcq__zejy is None:
            ghhrm__srmah = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(ghhrm__srmah)
        gcq__zejy.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        dzjw__psq = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(dzjw__psq)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        gtru__gbbnh = options.pop('distributed')
        pipeline_class = (pipeline_class if gtru__gbbnh else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        nmog__ghimc = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if nmog__ghimc else
            pipeline_class)
    wehjb__sltcl = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(wehjb__sltcl, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(wehjb__sltcl)
        else:
            return master_mode_wrapper(wehjb__sltcl)
    else:
        return wehjb__sltcl
