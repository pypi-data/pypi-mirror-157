...
def decode_if_dict_array_overload(A):
    if isinstance(A, types.BaseTuple):
        notrr__ijgxh = len(A.types)
        xdi__nteuq = 'def f(A):\n'
        uzef__kaqon = ','.join(f'decode_if_dict_array(A[{i}])' for i in
            range(notrr__ijgxh))
        xdi__nteuq += '  return ({}{})\n'.format(uzef__kaqon, ',' if 
            notrr__ijgxh == 1 else '')
        uzk__cddrn = {}
        exec(xdi__nteuq, {'decode_if_dict_array': decode_if_dict_array},
            uzk__cddrn)
        impl = uzk__cddrn['f']
        return impl
    if isinstance(A, types.List):
        ...
        def impl(A):
            ...
            for a in A:
                notrr__ijgxh += 1
            tczut__ter = []
            for i in range(notrr__ijgxh):
                tczut__ter.append(decode_if_dict_array(A[i]))
            return tczut__ter
        return impl
    ...
    if isinstance(A, bodo.SeriesType):
        ...
        def impl(A):
            meaz__ftyvq = bodo.hiframes.pd_series_ext.get_series_data(A)
            pri__mrmn = bodo.hiframes.pd_series_ext.get_series_index(A)
            name = bodo.hiframes.pd_series_ext.get_series_name(A)
            xiuc__chy = decode_if_dict_array(meaz__ftyvq)
            return bodo.hiframes.pd_series_ext.init_series(xiuc__chy,
                pri__mrmn, name)
        return impl
...
def to_str_arr_if_dict_array(t):
    ...
    if isinstance(t, bodo.TableType):
        conw__jeg = tuple(to_str_arr_if_dict_array(t) for t in t.arr_types)
        return bodo.TableType(conw__jeg, t.has_runtime_cols)
...
class BodoError(NumbaError):
    ...
    def __init__(self, msg, loc=None, locs_in_msg=None):
        ...
        ybygi__zhh = numba.core.errors.termcolor().errmsg
        super(BodoError, self).__init__(ybygi__zhh(msg), loc)
...
def raise_bodo_error(msg, loc=None):
    if bodo.transforms.typing_pass.in_partial_typing:
        ...
        zszox__wjmc = [] if loc is None else [loc]
        raise BodoError(msg, locs_in_msg=zszox__wjmc)
...
class FileInfo:
    ...
    def get_schema(self, fname):
        vgp__skcbf = self.get_full_filename(fname)
        return self._get_schema(vgp__skcbf)
...
class FilenameModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        vqfr__fvja = dmm.lookup(bodo.typeof(fe_type.fname))
        zzpf__lip = [(a, dkerp__vfpvs) for a, dkerp__vfpvs in zip(
            vqfr__fvja._fields, vqfr__fvja._members)]
        super().__init__(dmm, fe_type, zzpf__lip)
...
def is_initial_value_type(t):
    if not isinstance(t, types.InitialValue) or t.initial_value is None:
        return False
    pduek__ygtv = t.initial_value
    if isinstance(pduek__ygtv, dict):
        pduek__ygtv = pduek__ygtv.values()
    return not any(isinstance(vyvyx__dtj, (types.Poison, numba.core.
        interpreter._UNKNOWN_VALUE)) for vyvyx__dtj in pduek__ygtv)
...
def is_overload_constant_dict(val):
    return isinstance(val, types.LiteralStrKeyDict) and all(is_literal_type
        (vyvyx__dtj) for vyvyx__dtj in val.types
...
def is_overload_bool_list(val):
    return is_overload_constant_list(val) and all(is_overload_constant_bool
        (vyvyx__dtj) for vyvyx__dtj in get_overload_const_list(val))
...
def get_overload_const(val):
    ...
    if isinstance(val, types.LiteralList):
        hkknu__vfd = []
        for vyvyx__dtj in val.literal_value:
            wmv__iye = get_overload_const(vyvyx__dtj)
            if wmv__iye == NOT_CONSTANT:
                return NOT_CONSTANT
            else:
                hkknu__vfd.append(wmv__iye)
        return hkknu__vfd
    ...
    if isinstance(val, types.BaseTuple):
        hkknu__vfd = []
        for vyvyx__dtj in val.types:
            wmv__iye = get_overload_const(vyvyx__dtj)
            if wmv__iye == NOT_CONSTANT:
                return NOT_CONSTANT
            else:
                hkknu__vfd.append(wmv__iye)
        return tuple(hkknu__vfd)
...
def ensure_constant_arg(fname, arg_name, val, const_type):
    wmv__iye = get_overload_const(val)
    ygpd__ytgkp = _const_type_repr.get(const_type, str(const_type))
    if not isinstance(wmv__iye, const_type):
        raise BodoError(
            f"{fname}(): argument '{arg_name}' should be a constant {ygpd__ytgkp} not {val}"
            )


def ensure_constant_values(fname, arg_name, val, const_values):
    wmv__iye = get_overload_const(val)
    if wmv__iye not in const_values:
        raise BodoError(
            f"{fname}(): argument '{arg_name}' should be a constant value in {const_values} not '{wmv__iye}'"
            )
...
def check_unsupported_args(fname, args_dict, arg_defaults_dict,
    ...
    rmq__msyh = False
    for a in args_dict:
        rlcic__zvpjn = get_overload_const(args_dict[a])
        sux__jrl = arg_defaults_dict[a]
        if (rlcic__zvpjn is NOT_CONSTANT or rlcic__zvpjn is not None and 
            sux__jrl is None or rlcic__zvpjn is None and sux__jrl is not
            None or rlcic__zvpjn != sux__jrl or rlcic__zvpjn is not
            _no_input and sux__jrl is _no_input or rlcic__zvpjn is
            _no_input and sux__jrl is not _no_input):
            fpkl__vuwk = (
                f'{fn_str}: {a} parameter only supports default value {sux__jrl}'
                )
            rmq__msyh = True
            break
    if rmq__msyh and package_name == 'pandas':
        if module_name == 'IO':
            fpkl__vuwk += """
...
def get_overload_constant_dict(val):
    if isinstance(val, types.LiteralStrKeyDict):
        return {get_literal_value(fazb__tnilf): get_literal_value(
            vyvyx__dtj) for fazb__tnilf, vyvyx__dtj in val.literal_value.
            items()}
    ...
    if isinstance(val, types.DictType):
        ...
        return val.initial_value
    cpcn__obyy = [get_overload_const(vyvyx__dtj) for vyvyx__dtj in val.
        types[1:]]
    return {cpcn__obyy[2 * i]: cpcn__obyy[2 * i + 1] for i in range(len(
        cpcn__obyy) // 2)}
...
def get_overload_const_list(val):
    ...
    if is_initial_value_list_type(val):
        return val.initial_value
    if isinstance(val, types.LiteralList):
        return [get_literal_value(vyvyx__dtj) for vyvyx__dtj in val.
            literal_value]
...
def parse_dtype(dtype, func_name=None):
    ...
    if isinstance(dtype, types.Number) or dtype == bodo.string_type:
        return dtype
    try:
        czqpm__soloj = get_overload_const_str(dtype)
        if czqpm__soloj.startswith('Int') or czqpm__soloj.startswith('UInt'):
            return bodo.libs.int_arr_ext.typeof_pd_int_dtype(pd.api.types.
                pandas_dtype(czqpm__soloj), None)
        if czqpm__soloj == 'boolean':
            return bodo.libs.bool_arr_ext.boolean_dtype
        if czqpm__soloj == 'str':
            return bodo.string_type
        return numba.np.numpy_support.from_dtype(np.dtype(czqpm__soloj))
    except:
        pass
...
def get_index_names(t, func_name, default_name):
    ...
    cdnoh__gctvd = '{}: index name should be a constant string'.format(
        func_name)
    if isinstance(t, MultiIndexType):
        cvsg__cbgtu = []
        for i, txwb__evx in enumerate(t.names_typ):
            if txwb__evx == types.none:
                cvsg__cbgtu.append('level_{}'.format(i))
                continue
            if not is_overload_constant_str(txwb__evx):
                raise BodoError(cdnoh__gctvd)
            cvsg__cbgtu.append(get_overload_const_str(txwb__evx))
        return tuple(cvsg__cbgtu)
...
if types.SliceLiteral in numba.core.pythonapi._boxers.functions:
    ...
    def box_slice_literal(typ, val, c):
        pddw__gdnjr = typ.literal_value
        gtufl__anikt = []
        for ggrg__odl in ('start', 'stop', 'step'):
            fjz__lhc = getattr(typ.literal_value, ggrg__odl)
            gor__qmg = c.pyapi.make_none(
                ) if fjz__lhc is None else c.pyapi.from_native_value(types.
                literal(fjz__lhc), fjz__lhc, c.env_manager)
            gtufl__anikt.append(gor__qmg)
        aud__ztouc = c.pyapi.unserialize(c.pyapi.serialize_object(slice))
        kbb__alw = c.pyapi.call_function_objargs(aud__ztouc, gtufl__anikt)
        for a in gtufl__anikt:
            c.pyapi.decref(a)
        c.pyapi.decref(aud__ztouc)
        return kbb__alw
...
def box_list_literal(typ, val, c):
    kudqa__iqbkj = typ.literal_value
    dft__zlog = [c.pyapi.from_native_value(types.literal(vyvyx__dtj),
        vyvyx__dtj, c.env_manager) for vyvyx__dtj in kudqa__iqbkj]
    jwqem__tctn = c.pyapi.list_pack(dft__zlog)
    for a in dft__zlog:
        c.pyapi.decref(a)
    return jwqem__tctn
...
def list_literal_to_list(context, builder, fromty, toty, val):
    ...
    wnyvy__ghn = types.List(toty.dtype)
...
def is_literal_type(t):
    ...
    return isinstance(t, (types.Literal, types.Omitted)) and not isinstance(t,
        ...
        is_literal_type(vyvyx__dtj) for vyvyx__dtj in t.types
...
def get_literal_value(t):
    ...
    if isinstance(t, types.Literal):
        if isinstance(t, types.LiteralStrKeyDict):
            return {get_literal_value(fazb__tnilf): get_literal_value(
                vyvyx__dtj) for fazb__tnilf, vyvyx__dtj in t.literal_value.
                items()}
        if isinstance(t, types.LiteralList):
            return [get_literal_value(vyvyx__dtj) for vyvyx__dtj in t.
                literal_value]
        return t.literal_value
    if isinstance(t, types.Omitted):
        return t.value
    if isinstance(t, types.BaseTuple):
        return tuple(get_literal_value(vyvyx__dtj) for vyvyx__dtj in t.types)
...
def dtype_to_array_type(dtype):
    ...
    if isinstance(dtype, (types.Number, types.Boolean, types.NPDatetime,
        types.NPTimedelta)):
        meaz__ftyvq = types.Array(dtype, 1, 'C')
        if otwdc__ovyo:
            return to_nullable_type(meaz__ftyvq)
        return meaz__ftyvq
...
def get_udf_out_arr_type(f_return_type, return_nullable=False):
    ...
    kybm__ippik = dtype_to_array_type(f_return_type)
    kybm__ippik = to_nullable_type(kybm__ippik
        ) if return_nullable else kybm__ippik
    return kybm__ippik
...
def types_equality_exists(t1, t2):
    uchui__zojgf = numba.core.registry.cpu_target.typing_context
    try:
        uchui__zojgf.resolve_function_type(operator.eq, (t1, t2), {})
        return True
    except:
        return False
...
def is_hashable_type(t):
    whitelist_types = (types.UnicodeType, types.StringLiteral, types.
        ...
        PandasTimestampType)
    btve__unb = (types.bool_, bodo.datetime64ns, bodo.timedelta64ns, bodo.
        pd_timedelta_type)
    if isinstance(t, whitelist_types) or t in btve__unb:
        return True
    uchui__zojgf = numba.core.registry.cpu_target.typing_context
    try:
        uchui__zojgf.resolve_function_type(hash, (t,), {})
        return True
    except:
        return False
...
def to_nullable_type(t):
    ...
    if isinstance(t, DataFrameType):
        fnls__ipi = tuple(to_nullable_type(t) for t in t.data)
        return DataFrameType(fnls__ipi, t.index, t.columns, t.dist, t.
            is_table_format)
...
def is_common_scalar_dtype(scalar_types):
    mcg__kaal, nepn__jdg = get_common_scalar_dtype(scalar_types)
    return nepn__jdg
...
def get_common_scalar_dtype(scalar_types):
    ...
    try:
        hbsn__bgxda = np.find_common_type([numba.np.numpy_support.as_dtype(
            t) for t in scalar_types], [])
        if hbsn__bgxda != object:
            return numba.np.numpy_support.from_dtype(hbsn__bgxda), True
    except numba.core.errors.NumbaNotImplementedError as arj__gfhlu:
        pass
    ...
    katkl__gqy = itertools.groupby(scalar_types)
    if next(katkl__gqy, True) and not next(katkl__gqy, False):
...
def get_nullable_and_non_nullable_types(array_of_types):
    ...
    for typ in array_of_types:
        if typ == bodo.libs.bool_arr_ext.boolean_array:
            zxhdo__qradv.append(types.Array(types.bool_, 1, 'C'))
        elif isinstance(typ, bodo.libs.int_arr_ext.IntegerArrayType):
            zxhdo__qradv.append(types.Array(typ.dtype, 1, 'C'))
        elif isinstance(typ, types.Array):
            if typ.dtype == types.bool_:
                zxhdo__qradv.append(bodo.libs.bool_arr_ext.boolean_array)
            if isinstance(typ.dtype, types.Integer):
                zxhdo__qradv.append(bodo.libs.int_arr_ext.IntegerArrayType(
                    typ.dtype))
        zxhdo__qradv.append(typ)
    return zxhdo__qradv
...
def _gen_objmode_overload(func, output_type, attr_name=None, is_function=
    ...
    if is_function:
        atohh__xxbj = getfullargspec(func)
        assert atohh__xxbj.varargs is None, 'varargs not supported'
        assert atohh__xxbj.varkw is None, 'varkw not supported'
        defaults = [] if atohh__xxbj.defaults is None else atohh__xxbj.defaults
        nec__ffpsv = len(atohh__xxbj.args) - len(defaults)
        args = atohh__xxbj.args[1:] if attr_name else atohh__xxbj.args[:]
        fvtqe__zqgp = []
        for i, vnri__vxjf in enumerate(atohh__xxbj.args):
            if i < nec__ffpsv:
                fvtqe__zqgp.append(vnri__vxjf)
            elif str(defaults[i - nec__ffpsv]) != '<deprecated parameter>':
                fvtqe__zqgp.append(vnri__vxjf + '=' + str(defaults[i -
                    nec__ffpsv]))
            else:
                args.remove(vnri__vxjf)
        if atohh__xxbj.kwonlyargs is not None:
            for vnri__vxjf in atohh__xxbj.kwonlyargs:
                args.append(f'{vnri__vxjf}={vnri__vxjf}')
                fvtqe__zqgp.append(
                    f'{vnri__vxjf}={str(atohh__xxbj.kwonlydefaults[vnri__vxjf])}'
                    )
        sig = ', '.join(fvtqe__zqgp)
        args = ', '.join(args)
    else:
        sig = 'self'
    ...
    yjc__tuy = f'self.{attr_name}' if attr_name else f'{func_name}'
    xdi__nteuq = f'def overload_impl({sig}):\n'
    xdi__nteuq += f'    def impl({sig}):\n'
    if single_rank:
        xdi__nteuq += f'        if bodo.get_rank() == 0:\n'
        pbe__mytvo = '    '
    else:
        pbe__mytvo = ''
    xdi__nteuq += (
        f"        {pbe__mytvo}with numba.objmode(res='{type_name}'):\n")
    if is_function:
        xdi__nteuq += f'            {pbe__mytvo}res = {yjc__tuy}({args})\n'
    else:
        xdi__nteuq += f'            {pbe__mytvo}res = {yjc__tuy}\n'
    xdi__nteuq += f'        return res\n'
    xdi__nteuq += f'    return impl\n'
    uzk__cddrn = {}
    ddvfk__vdgc = globals()
    if not attr_name:
        ddvfk__vdgc[func_name] = func
    exec(xdi__nteuq, ddvfk__vdgc, uzk__cddrn)
    rrgo__bqnw = uzk__cddrn['overload_impl']
    return rrgo__bqnw


def gen_objmode_func_overload(func, output_type=None, single_rank=False):
    try:
        rrgo__bqnw = _gen_objmode_overload(func, output_type, is_function=
            True, single_rank=single_rank)
        overload(func, no_unliteral=True)(rrgo__bqnw)
    except Exception as arj__gfhlu:
        pass
...
def gen_objmode_method_overload(obj_type, method_name, method, output_type=
    ...
    try:
        rrgo__bqnw = _gen_objmode_overload(method, output_type, method_name,
            True, single_rank)
        overload_method(obj_type, method_name, no_unliteral=True)(rrgo__bqnw)
    except Exception as arj__gfhlu:
        pass
...
def gen_objmode_attr_overload(obj_type, attr_name, attr, output_type=None,
    single_rank=False):
    try:
        rrgo__bqnw = _gen_objmode_overload(attr, output_type, attr_name, 
            False, single_rank)
        overload_attribute(obj_type, attr_name, no_unliteral=True)(rrgo__bqnw)
    except Exception as arj__gfhlu:
        pass
...
class NumTypeStaticGetItem(AbstractTemplate):
    ...
    def generic(self, args, kws):
        val, ucpo__ewai = args
        if isinstance(ucpo__ewai, slice) and (isinstance(val, types.
            NumberClass) or isinstance(val, types.TypeRef) and isinstance(
            val.instance_type, (types.NPDatetime, types.NPTimedelta))):
            return signature(types.TypeRef(val.instance_type[ucpo__ewai]),
                *args)
...
def construct_pysig(arg_names, defaults):
    xdi__nteuq = f'def stub('
    for vnri__vxjf in arg_names:
        xdi__nteuq += vnri__vxjf
        if vnri__vxjf in defaults:
            if isinstance(defaults[vnri__vxjf], str):
                xdi__nteuq += f"='{defaults[vnri__vxjf]}'"
            else:
                xdi__nteuq += f'={defaults[vnri__vxjf]}'
        xdi__nteuq += ', '
    xdi__nteuq += '):\n'
    xdi__nteuq += '    pass\n'
    uzk__cddrn = {}
    exec(xdi__nteuq, {}, uzk__cddrn)
    did__vsis = uzk__cddrn['stub']
    return numba.core.utils.pysignature(did__vsis)


def fold_typing_args(func_name, args, kws, arg_names, defaults,
    unsupported_arg_names=()):
    kws = dict(kws)
    pupuw__sgkzb = len(arg_names)
    jxk__iti = len(args) + len(kws)
    if jxk__iti > pupuw__sgkzb:
        xnptr__ewzus = 'argument' if pupuw__sgkzb == 1 else 'arguments'
        qglz__sbsr = 'was' if jxk__iti == 1 else 'were'
        raise BodoError(
            f'{func_name}(): Too many arguments specified. Function takes {pupuw__sgkzb} {xnptr__ewzus}, but {jxk__iti} {qglz__sbsr} provided.'
            )
    xtr__nxtw = bodo.utils.typing.construct_pysig(arg_names, defaults)
    try:
        mxscs__bbkuw = bodo.utils.transform.fold_argument_types(xtr__nxtw,
            args, kws)
    except Exception as aqp__bdqxu:
        raise_bodo_error(f'{func_name}(): {aqp__bdqxu}')
    if unsupported_arg_names:
        ...
        for i, arg_name in enumerate(arg_names):
            if arg_name in unsupported_arg_names:
                ...
                yynx__qmas[arg_name] = mxscs__bbkuw[i]
                ieynk__wpwpx[arg_name] = defaults[arg_name]
        check_unsupported_args(func_name, yynx__qmas, ieynk__wpwpx)
    return xtr__nxtw, mxscs__bbkuw
...
class BodoArrayIteratorModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        zzpf__lip = [('index', types.EphemeralPointer(types.uintp)), (
            'array', fe_type.arr_type)]
        super(BodoArrayIteratorModel, self).__init__(dmm, fe_type, zzpf__lip)


@lower_builtin('iternext', BodoArrayIterator)
@iternext_impl(RefType.NEW)
def iternext_bodo_array(context, builder, sig, args, result):
    [yxxak__bxgbz] = sig.args
    [xoy__hevpu] = args
    sffjd__rgzc = context.make_helper(builder, yxxak__bxgbz, value=xoy__hevpu)
    lpai__ldt = signature(types.intp, yxxak__bxgbz.arr_type)
    yrt__har = context.compile_internal(builder, lambda a: len(a),
        lpai__ldt, [sffjd__rgzc.array])
    pri__mrmn = builder.load(sffjd__rgzc.index)
    jdjz__civmf = builder.icmp_signed('<', pri__mrmn, yrt__har)
    result.set_valid(jdjz__civmf)
    with builder.if_then(jdjz__civmf):
        ldpw__ssifk = signature(yxxak__bxgbz.yield_type, yxxak__bxgbz.
            arr_type, types.intp)
        value = context.compile_internal(builder, lambda a, i: a[i],
            ldpw__ssifk, [sffjd__rgzc.array, pri__mrmn])
        result.yield_(value)
        tlub__emrmi = cgutils.increment_index(builder, pri__mrmn)
        builder.store(tlub__emrmi, sffjd__rgzc.index)


def index_typ_from_dtype_name_arr(elem_dtype, name, arr_typ):
    emph__dovt = type(get_index_type_from_dtype(elem_dtype))
    if name is None:
        tyxx__ejbp = None
    else:
        tyxx__ejbp = types.StringLiteral(name)
    if emph__dovt == bodo.hiframes.pd_index_ext.NumericIndexType:
        fva__jjhw = emph__dovt(elem_dtype, tyxx__ejbp, arr_typ)
    elif emph__dovt == bodo.hiframes.pd_index_ext.CategoricalIndexType:
        fva__jjhw = emph__dovt(bodo.CategoricalArrayType(elem_dtype),
            tyxx__ejbp, arr_typ)
    else:
        fva__jjhw = emph__dovt(tyxx__ejbp, arr_typ)
    return fva__jjhw
...
def check_objmode_output_type(ret_tup, ret_type):
    return tuple(_check_objmode_type(vyvyx__dtj, t) for vyvyx__dtj, t in
...
def _fix_objmode_df_type(val, val_typ, typ):
    ...
    if val_typ != typ:
        zypj__ppmpm = pd.Index(val_typ.columns)
        vwax__ldhb = pd.Index(typ.columns)
        pbfor__wwiu = zypj__ppmpm.argsort()
        pte__btxb = vwax__ldhb.argsort()
        htpry__tfp = val_typ.copy(data=tuple(np.array(val_typ.data)[
            pbfor__wwiu]), columns=tuple(zypj__ppmpm[pbfor__wwiu]))
        duq__rkqlq = typ.copy(data=tuple(np.array(typ.data)[pte__btxb]),
            columns=tuple(vwax__ldhb[pte__btxb]))
        if htpry__tfp == duq__rkqlq:
            val_typ = typ
