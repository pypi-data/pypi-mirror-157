...
def get_new_null_mask_bool_index(old_mask, ind, n):
    obr__xnlsy = n + 7 >> 3
    sincj__cnp = np.empty(obr__xnlsy, np.uint8)
    llt__siy = 0
    for mlrrg__nikp in range(len(ind)):
        if ind[mlrrg__nikp]:
            pfzsp__nyk = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask,
                mlrrg__nikp)
            bodo.libs.int_arr_ext.set_bit_to_arr(sincj__cnp, llt__siy,
                pfzsp__nyk)
            llt__siy += 1
    return sincj__cnp
...
def array_getitem_bool_index(A, ind):
    ...
    ejf__wneg = A._data[ind]
    n = len(ejf__wneg)
    sincj__cnp = get_new_null_mask_bool_index(old_mask, ind, n)
    return ejf__wneg, sincj__cnp
...
def get_new_null_mask_int_index(old_mask, ind, n):
    ...
    llt__siy = 0
    for mlrrg__nikp in range(len(ind)):
        pfzsp__nyk = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask, ind
            [mlrrg__nikp])
        bodo.libs.int_arr_ext.set_bit_to_arr(sincj__cnp, llt__siy, pfzsp__nyk)
        llt__siy += 1
    return sincj__cnp
...
def array_getitem_int_index(A, ind):
    gnjr__vgvgy = bodo.utils.conversion.coerce_to_ndarray(ind)
    old_mask = A._null_bitmap
    ejf__wneg = A._data[gnjr__vgvgy]
    n = len(ejf__wneg)
    sincj__cnp = get_new_null_mask_int_index(old_mask, gnjr__vgvgy, n)
    return ejf__wneg, sincj__cnp


@register_jitable
def get_new_null_mask_slice_index(old_mask, ind, n):
    rrrm__pgm = numba.cpython.unicode._normalize_slice(ind, n)
    wrwca__ptlmo = numba.cpython.unicode._slice_span(rrrm__pgm)
    obr__xnlsy = wrwca__ptlmo + 7 >> 3
    sincj__cnp = np.empty(obr__xnlsy, np.uint8)
    llt__siy = 0
    for mlrrg__nikp in range(rrrm__pgm.start, rrrm__pgm.stop, rrrm__pgm.step):
        pfzsp__nyk = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask,
            mlrrg__nikp)
        bodo.libs.int_arr_ext.set_bit_to_arr(sincj__cnp, llt__siy, pfzsp__nyk)
        llt__siy += 1
...
def array_getitem_slice_index(A, ind):
    n = len(A._data)
    old_mask = A._null_bitmap
    ejf__wneg = np.ascontiguousarray(A._data[ind])
    sincj__cnp = get_new_null_mask_slice_index(old_mask, ind, n)
    return ejf__wneg, sincj__cnp
...
def array_setitem_int_index_overload(A, idx, val):
    if bodo.utils.utils.is_array_typ(val
        ...
        def impl_arr(A, idx, val):
            ...
            n = len(val._data)
            for mlrrg__nikp in range(n):
                A._data[idx[mlrrg__nikp]] = val._data[mlrrg__nikp]
                pfzsp__nyk = bodo.libs.int_arr_ext.get_bit_bitmap_arr(val.
                    _null_bitmap, mlrrg__nikp)
                bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap, idx[
                    mlrrg__nikp], pfzsp__nyk)
        return impl_arr
    if bodo.utils.typing.is_scalar_type(val):

        def impl_scalar(A, idx, val):
            for mlrrg__nikp in idx:
                A._data[mlrrg__nikp] = val
...
def array_setitem_bool_index_overload(A, idx, val):
    if bodo.utils.utils.is_array_typ(val
        ...
        def impl_arr(A, idx, val):
            ...
            znn__dbz = 0
            for mlrrg__nikp in range(n):
                if not bodo.libs.array_kernels.isna(idx, mlrrg__nikp) and idx[
                    mlrrg__nikp]:
                    A._data[mlrrg__nikp] = val._data[znn__dbz]
                    pfzsp__nyk = bodo.libs.int_arr_ext.get_bit_bitmap_arr(val
                        ._null_bitmap, znn__dbz)
                    bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
                        mlrrg__nikp, pfzsp__nyk)
                    znn__dbz += 1
        return impl_arr
...
def setitem_slice_index_null_bits(dst_bitmap, src_bitmap, idx, n):
    rrrm__pgm = numba.cpython.unicode._normalize_slice(idx, n)
    znn__dbz = 0
    for mlrrg__nikp in range(rrrm__pgm.start, rrrm__pgm.stop, rrrm__pgm.step):
        pfzsp__nyk = bodo.libs.int_arr_ext.get_bit_bitmap_arr(src_bitmap,
            znn__dbz)
        bodo.libs.int_arr_ext.set_bit_to_arr(dst_bitmap, mlrrg__nikp,
            pfzsp__nyk)
        znn__dbz += 1
...
def array_setitem_slice_index_overload(A, idx, val):
    ...
    if bodo.utils.typing.is_scalar_type(val):
        ...
        def impl_scalar(A, idx, val):
            rrrm__pgm = numba.cpython.unicode._normalize_slice(idx, len(A))
            for mlrrg__nikp in range(rrrm__pgm.start, rrrm__pgm.stop,
                rrrm__pgm.step):
                A._data[mlrrg__nikp] = val
                bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
                    mlrrg__nikp, 1)
    ...
    if not bodo.utils.utils.is_array_typ(A, False):
        ...
        if isinstance(idx, types.Integer):
            ...
            def setitem_none_int_arr(A, idx, val):
                ...
                for mlrrg__nikp in idx:
                    bodo.libs.array_kernels.setna(A, mlrrg__nikp)
            ...
            if A == bodo.string_array_type:
                ...
                def string_arr_impl(A, idx, val):
                    n = len(A)
                    idx = bodo.utils.conversion.coerce_to_ndarray(idx)
                    phe__tcp = bodo.libs.str_arr_ext.pre_alloc_string_array(n,
                        -1)
                    for mlrrg__nikp in numba.parfors.parfor.internal_prange(n):
                        if idx[mlrrg__nikp] or bodo.libs.array_kernels.isna(A,
                            mlrrg__nikp):
                            phe__tcp[mlrrg__nikp] = ''
                            bodo.libs.str_arr_ext.str_arr_set_na(phe__tcp,
                                mlrrg__nikp)
                        else:
                            phe__tcp[mlrrg__nikp] = A[mlrrg__nikp]
            ...
            def setitem_none_bool_arr(A, idx, val):
                ...
                n = len(idx)
                for mlrrg__nikp in range(n):
                    if not bodo.libs.array_kernels.isna(idx, mlrrg__nikp
                        ) and idx[mlrrg__nikp]:
                        bodo.libs.array_kernels.setna(A, mlrrg__nikp)
            ...
            def setitem_optional_int_arr(A, idx, val):
                ...
                for mlrrg__nikp in idx:
                    if val is None:
                        bodo.libs.array_kernels.setna(A, mlrrg__nikp)
                        continue
                    A[mlrrg__nikp] = bodo.utils.indexing.unoptional(val)
...
def unoptional(typingctx, val_t=None):
    ...
    def codegen(context, builder, signature, args):
        xlfth__eclz = context.make_helper(builder, val_t, args[0])
        gew__kjki = xlfth__eclz.data
        context.nrt.incref(builder, val_t.type, gew__kjki)
        return gew__kjki
