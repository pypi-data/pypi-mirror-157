...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    tbico__gsa = pq_node.out_vars[0].name
    pcf__olevc = pq_node.out_vars[1].name
    if tbico__gsa not in lives and pcf__olevc not in lives:
        return None
    elif tbico__gsa not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.out_used_cols = []
    elif pcf__olevc not in lives:
