...
class SeriesIatType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        owjz__mjlt = 'SeriesIatType({})'.format(stype)
        super(SeriesIatType, self).__init__(owjz__mjlt)
...
class SeriesIatModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        zpe__ktg = [('obj', fe_type.stype)]
        super(SeriesIatModel, self).__init__(dmm, fe_type, zpe__ktg)
...
def init_series_iat(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        ihoor__fxpxr, = args
        qsdav__srlc = signature.return_type
        uduz__rxj = cgutils.create_struct_proxy(qsdav__srlc)(context, builder)
        uduz__rxj.obj = ihoor__fxpxr
        context.nrt.incref(builder, signature.args[0], ihoor__fxpxr)
        return uduz__rxj._getvalue()
...
def overload_series_iat_setitem(I, idx, val):
    if isinstance(I, SeriesIatType):
        ...
        if I.stype.dtype == types.NPTimedelta('ns'
            ...
            def impl_dt(I, idx, val):
                jgscc__wuq = (bodo.hiframes.datetime_timedelta_ext.
                    _to_nanoseconds(val))
...
class SeriesIlocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        owjz__mjlt = 'SeriesIlocType({})'.format(stype)
        super(SeriesIlocType, self).__init__(owjz__mjlt)
...
class SeriesIlocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        zpe__ktg = [('obj', fe_type.stype)]
        super(SeriesIlocModel, self).__init__(dmm, fe_type, zpe__ktg)
...
def init_series_iloc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        ihoor__fxpxr, = args
        lqr__fna = signature.return_type
        gdrs__yme = cgutils.create_struct_proxy(lqr__fna)(context, builder)
        gdrs__yme.obj = ihoor__fxpxr
        context.nrt.incref(builder, signature.args[0], ihoor__fxpxr)
        return gdrs__yme._getvalue()
...
def overload_series_iloc_getitem(I, idx):
    if isinstance(I, SeriesIlocType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl(I, idx):
                S = I._obj
                ysky__kzvu = bodo.utils.conversion.coerce_to_ndarray(idx)
                nqo__yod = bodo.hiframes.pd_series_ext.get_series_data(S)[
                    ysky__kzvu]
                cqgtp__kvub = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    ysky__kzvu]
                owjz__mjlt = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(nqo__yod,
                    cqgtp__kvub, owjz__mjlt)
            return impl
        if isinstance(idx, types.SliceType):

            def impl_slice(I, idx):
                S = I._obj
                nqo__yod = bodo.hiframes.pd_series_ext.get_series_data(S)[idx]
                cqgtp__kvub = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    idx]
                owjz__mjlt = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(nqo__yod,
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            if is_scalar_type(val):
                if I.stype.dtype == types.NPDatetime('ns'
                    ...
                    def impl_dt(I, idx, val):
                        s = integer_to_dt64(val.value)
                        ysky__kzvu = bodo.utils.conversion.coerce_to_ndarray(
                            idx)
                ...
                if I.stype.dtype == types.NPTimedelta('ns'
                    ...
                    def impl_dt(I, idx, val):
                        jgscc__wuq = (bodo.hiframes.datetime_timedelta_ext.
                            _to_nanoseconds(val))
                        s = (bodo.hiframes.pd_timestamp_ext.
                            integer_to_timedelta64(jgscc__wuq))
                        ysky__kzvu = bodo.utils.conversion.coerce_to_ndarray(
                            idx)
                        bodo.hiframes.pd_series_ext.get_series_data(I._obj)[
                            ysky__kzvu] = s
            ...
            def impl_arr(I, idx, val):
                ysky__kzvu = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(I._obj)[ysky__kzvu
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
class SeriesLocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        owjz__mjlt = 'SeriesLocType({})'.format(stype)
        super(SeriesLocType, self).__init__(owjz__mjlt)
...
class SeriesLocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        zpe__ktg = [('obj', fe_type.stype)]
        super(SeriesLocModel, self).__init__(dmm, fe_type, zpe__ktg)
...
def init_series_loc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        ihoor__fxpxr, = args
        dwkqw__nnxg = signature.return_type
        lbujz__ohvp = cgutils.create_struct_proxy(dwkqw__nnxg)(context, builder
            )
        lbujz__ohvp.obj = ihoor__fxpxr
        context.nrt.incref(builder, signature.args[0], ihoor__fxpxr)
        return lbujz__ohvp._getvalue()
...
def overload_series_loc_getitem(I, idx):
    ...
    if isinstance(idx, types.Integer) and isinstance(I.stype.index,
        ...
        def impl_range_index_int(I, idx):
            ...
            nqo__yod = bodo.hiframes.pd_series_ext.get_series_data(S)
            cqgtp__kvub = bodo.hiframes.pd_series_ext.get_series_index(S)
            fywc__iqdt = (idx - cqgtp__kvub._start) // cqgtp__kvub._step
            return nqo__yod[fywc__iqdt]
        ...
        if isinstance(idx, types.Integer):
            ...
            if isinstance(S.index, RangeIndexType):
                ...
                def impl(S, idx):
                    ...
                    ysky__kzvu = idx * I._step + I._start
                    return bodo.utils.conversion.box_if_dt64(nqo__yod[
                        ysky__kzvu])
                return impl
        ...
        if idx == bodo.string_type or is_overload_constant_str(idx):
            if isinstance(S.index, bodo.hiframes.pd_index_ext.StringIndexType):
                ...
                def impl_str_getitem(S, idx):
                    hya__ylsov = bodo.hiframes.pd_series_ext.get_series_index(S
                        )
                    uhzk__vjjil = hya__ylsov.get_loc(idx)
                    if uhzk__vjjil is None:
                        raise IndexError
                    else:
                        val = bodo.hiframes.pd_series_ext.get_series_data(S)[
                            bodo.utils.indexing.unoptional(uhzk__vjjil)]
                    return val
    ...
    if isinstance(S, SeriesType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl_arr(S, idx, val):
                ysky__kzvu = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(S)[ysky__kzvu
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
def overload_const_index_series_getitem(S, idx):
    if isinstance(S, (SeriesType, HeterogeneousSeriesType)) and isinstance(S
        ...
        index.data):
        jbv__qiojr = get_overload_const_tuple(S.index.data)
        if isinstance(idx, types.Integer) and not any(isinstance(unp__mqnp,
            int) for unp__mqnp in jbv__qiojr):
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[idx]
        if is_literal_type(idx):
            febvf__myk = get_literal_value(idx)
            if febvf__myk not in jbv__qiojr:
                raise_bodo_error(
                    f"Series label-based getitem: '{febvf__myk}' not in {jbv__qiojr}"
                    )
            arr_ind = jbv__qiojr.index(febvf__myk)
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[arr_ind]
...
def cast_series_iat(context, builder, fromty, toty, val):
    uduz__rxj = cgutils.create_struct_proxy(fromty)(context, builder, val)
    vnuug__lkyy = context.cast(builder, uduz__rxj.obj, fromty.stype, toty.stype
        )
    uihi__hlwsv = cgutils.create_struct_proxy(toty)(context, builder)
    uihi__hlwsv.obj = vnuug__lkyy
    return uihi__hlwsv._getvalue()
