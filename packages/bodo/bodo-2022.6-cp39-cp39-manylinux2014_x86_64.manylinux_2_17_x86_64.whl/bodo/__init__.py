...
def _global_except_hook(exctype, value, traceback):
    ...
    llaqn__pga = MPI.COMM_WORLD
    wtz__hbn = llaqn__pga.Get_rank()
    gpwkn__sfjl = 3.0
    qzbu__jyh = True
    uded__hui = llaqn__pga.Ibarrier()
    qnzq__epk = time.time()
    while time.time() - qnzq__epk < gpwkn__sfjl:
        time.sleep(0.1)
        if uded__hui.Test():
            qzbu__jyh = False
            break
    try:
        ...
        if qzbu__jyh:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {wtz__hbn}. \n')
