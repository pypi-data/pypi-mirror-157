...
def decimal_to_str_codegen(context, builder, signature, args, scale):
    ...
    mrco__kxqp = cgutils.create_struct_proxy(types.unicode_type)(context,
        builder)
    vyy__ojk = lir.FunctionType(lir.VoidType(), [lir.IntType(128), lir.
        IntType(8).as_pointer().as_pointer(), lir.IntType(64).as_pointer(),
        lir.IntType(32)])
    vwhlr__bwdcr = cgutils.get_or_insert_function(builder.module, vyy__ojk,
        name='decimal_to_str')
    builder.call(vwhlr__bwdcr, [val, mrco__kxqp._get_ptr_by_name('meminfo'),
        mrco__kxqp._get_ptr_by_name('length'), scale])
    mrco__kxqp.kind = context.get_constant(types.int32, numba.cpython.
        unicode.PY_UNICODE_1BYTE_KIND)
    mrco__kxqp.is_ascii = context.get_constant(types.int32, 1)
    mrco__kxqp.hash = context.get_constant(numba.cpython.unicode._Py_hash_t, -1
        )
    mrco__kxqp.data = context.nrt.meminfo_data(builder, mrco__kxqp.meminfo)
    mrco__kxqp.parent = cgutils.get_null_value(mrco__kxqp.parent.type)
    return mrco__kxqp._getvalue()
...
def str_to_decimal_codegen(context, builder, signature, args):
    val, paml__lckp, paml__lckp = args
    val = bodo.libs.str_ext.gen_unicode_to_std_str(context, builder, val)
    vyy__ojk = lir.FunctionType(lir.IntType(128), [lir.IntType(8).as_pointer()]
        )
    vwhlr__bwdcr = cgutils.get_or_insert_function(builder.module, vyy__ojk,
        name='str_to_decimal')
    wgow__zcz = builder.call(vwhlr__bwdcr, [val])
    return wgow__zcz
...
def decimal128type_to_int64_tuple(typingctx, val):
    ...
    def codegen(context, builder, signature, args):
        zfxd__kqp = cgutils.alloca_once(builder, lir.ArrayType(lir.IntType(
            64), 2))
        builder.store(args[0], builder.bitcast(zfxd__kqp, lir.IntType(128).
            as_pointer()))
        return builder.load(zfxd__kqp)
...
def decimal128type_cmp(typingctx, val1, scale1, val2, scale2, func_name):
    ...
    ogwdq__vgwfo = get_overload_const_str(func_name)

    def codegen(context, builder, signature, args):
        val1, scale1, val2, scale2, paml__lckp = args
        vyy__ojk = lir.FunctionType(lir.IntType(1), [lir.IntType(128), lir.
            IntType(64), lir.IntType(128), lir.IntType(64)])
        vwhlr__bwdcr = cgutils.get_or_insert_function(builder.module,
            vyy__ojk, name=ogwdq__vgwfo)
        return builder.call(vwhlr__bwdcr, (val1, scale1, val2, scale2))
...
def decimal_create_cmp_op_overload(op):
    ...
    def overload_cmp(lhs, rhs):
        if isinstance(lhs, Decimal128Type) and isinstance(rhs, Decimal128Type):
            ogwdq__vgwfo = 'decimal_cmp_' + op.__name__
            scale1 = lhs.scale
            scale2 = rhs.scale
...
def lower_constant_decimal(context, builder, ty, pyval):
    ujq__zktc = numba.njit(lambda v: decimal128type_to_int64_tuple(v))(pyval)
    xjeh__covl = [context.get_constant_generic(builder, types.int64, v) for
        v in ujq__zktc]
    atum__dqir = cgutils.pack_array(builder, xjeh__covl)
    zfxd__kqp = cgutils.alloca_once(builder, lir.IntType(128))
    builder.store(atum__dqir, builder.bitcast(zfxd__kqp, lir.ArrayType(lir.
...
def unbox_decimal(typ, val, c):
    vyy__ojk = lir.FunctionType(lir.VoidType(), [lir.IntType(8).as_pointer(
        ), lir.IntType(128).as_pointer()])
    vwhlr__bwdcr = cgutils.get_or_insert_function(c.builder.module,
        vyy__ojk, name='unbox_decimal')
    zfxd__kqp = cgutils.alloca_once(c.builder, c.context.get_value_type(
        int128_type))
    c.builder.call(vwhlr__bwdcr, [val, zfxd__kqp])
    uowga__cjwh = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    vvbav__rxwsp = c.builder.load(zfxd__kqp)
    return NativeValue(vvbav__rxwsp, is_error=uowga__cjwh)


@box(Decimal128Type)
def box_decimal(typ, val, c):
    qbhy__nyrae = decimal_to_str_codegen(c.context, c.builder, bodo.
        string_type(typ), (val,), typ.scale)
    hsik__ifhd = c.pyapi.from_native_value(bodo.string_type, qbhy__nyrae, c
        .env_manager)
    trp__lmzp = c.context.insert_const_string(c.builder.module, 'decimal')
    htyrh__qmf = c.pyapi.import_module_noblock(trp__lmzp)
    zfxd__kqp = c.pyapi.call_method(htyrh__qmf, 'Decimal', (hsik__ifhd,))
    c.pyapi.decref(hsik__ifhd)
    c.pyapi.decref(htyrh__qmf)
    return zfxd__kqp
...
class DecimalArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        vkk__qknl = [('data', data_type), ('null_bitmap', nulls_type)]
        models.StructModel.__init__(self, dmm, fe_type, vkk__qknl)
...
def init_decimal_array(typingctx, data, null_bitmap, precision_tp, scale_tp
    ...
    def codegen(context, builder, signature, args):
        ohgv__iizm, eiqo__uwsae, paml__lckp, paml__lckp = args
        dbmbu__wep = cgutils.create_struct_proxy(signature.return_type)(context
            , builder)
        dbmbu__wep.data = ohgv__iizm
        dbmbu__wep.null_bitmap = eiqo__uwsae
        context.nrt.incref(builder, signature.args[0], ohgv__iizm)
        context.nrt.incref(builder, signature.args[1], eiqo__uwsae)
        return dbmbu__wep._getvalue()
    precision = get_overload_const_int(precision_tp)
    scale = get_overload_const_int(scale_tp)
    xempw__mao = DecimalArrayType(precision, scale)
    vdli__brzwk = xempw__mao(data, null_bitmap, precision_tp, scale_tp)
    return vdli__brzwk, codegen


@lower_constant(DecimalArrayType)
def lower_constant_decimal_arr(context, builder, typ, pyval):
    n = len(pyval)
    ythm__hdjn = context.get_constant(types.int64, n)
    bbb__dxet = bodo.utils.utils._empty_nd_impl(context, builder, types.
        Array(int128_type, 1, 'C'), [ythm__hdjn])
    ornaw__rbw = np.empty(n + 7 >> 3, np.uint8)

    def f(arr, idx, val):
        arr[idx] = decimal128type_to_int128(val)
    for lqqt__tqutb, yjcef__gkyy in enumerate(pyval):
        rlh__mkywq = pd.isna(yjcef__gkyy)
        bodo.libs.int_arr_ext.set_bit_to_arr(ornaw__rbw, lqqt__tqutb, int(
            not rlh__mkywq))
        if not rlh__mkywq:
            context.compile_internal(builder, f, types.void(types.Array(
                int128_type, 1, 'C'), types.int64, Decimal128Type(typ.
                precision, typ.scale)), [bbb__dxet._getvalue(), context.
                get_constant(types.int64, lqqt__tqutb), context.
                get_constant_generic(builder, Decimal128Type(typ.precision,
                typ.scale), yjcef__gkyy)])
    vsq__cyfu = context.get_constant_generic(builder, nulls_type, ornaw__rbw)
    dbmbu__wep = context.make_helper(builder, typ)
    dbmbu__wep.data = bbb__dxet._getvalue()
    dbmbu__wep.null_bitmap = vsq__cyfu
    return dbmbu__wep._getvalue()


@numba.njit(no_cpython_wrapper=True)
def alloc_decimal_array(n, precision, scale):
    ems__jzh = np.empty(n, dtype=int128_type)
    mtwwj__tgwu = np.empty(n + 7 >> 3, dtype=np.uint8)
    return init_decimal_array(ems__jzh, mtwwj__tgwu, precision, scale)
...
def box_decimal_arr(typ, val, c):
    thfjn__zidlc = cgutils.create_struct_proxy(typ)(c.context, c.builder, val)
    ems__jzh = c.context.make_array(types.Array(int128_type, 1, 'C'))(c.
        context, c.builder, thfjn__zidlc.data)
    bvhfo__kmga = c.context.make_array(types.Array(types.uint8, 1, 'C'))(c.
        context, c.builder, thfjn__zidlc.null_bitmap).data
    n = c.builder.extract_value(ems__jzh.shape, 0)
    scale = c.context.get_constant(types.int32, typ.scale)
    vyy__ojk = lir.FunctionType(c.pyapi.pyobj, [lir.IntType(64), lir.
        IntType(128).as_pointer(), lir.IntType(8).as_pointer(), lir.IntType
        (32)])
    ivgf__sskp = cgutils.get_or_insert_function(c.builder.module, vyy__ojk,
        name='box_decimal_array')
    trr__yxrtj = c.builder.call(ivgf__sskp, [n, ems__jzh.data, bvhfo__kmga,
        scale])
...
def unbox_decimal_arr(typ, val, c):
    dbmbu__wep = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    uypqa__rvbvq = c.pyapi.call_method(val, '__len__', ())
    n = c.pyapi.long_as_longlong(uypqa__rvbvq)
    c.pyapi.decref(uypqa__rvbvq)
    fcvfb__lvv = c.builder.udiv(c.builder.add(n, lir.Constant(lir.IntType(
        64), 7)), lir.Constant(lir.IntType(64), 8))
    bbb__dxet = bodo.utils.utils._empty_nd_impl(c.context, c.builder, types
        .Array(int128_type, 1, 'C'), [n])
    negbr__yrjr = bodo.utils.utils._empty_nd_impl(c.context, c.builder,
        types.Array(types.uint8, 1, 'C'), [fcvfb__lvv])
    vyy__ojk = lir.FunctionType(lir.VoidType(), [lir.IntType(8).as_pointer(
        ), lir.IntType(64), lir.IntType(128).as_pointer(), lir.IntType(8).
        as_pointer()])
    vwhlr__bwdcr = cgutils.get_or_insert_function(c.builder.module,
        vyy__ojk, name='unbox_decimal_array')
    c.builder.call(vwhlr__bwdcr, [val, n, bbb__dxet.data, negbr__yrjr.data])
    dbmbu__wep.null_bitmap = negbr__yrjr._getvalue()
    dbmbu__wep.data = bbb__dxet._getvalue()
    uowga__cjwh = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(dbmbu__wep._getvalue(), is_error=uowga__cjwh)
...
def decimal_arr_getitem(A, ind):
    ...
    if is_list_like_index_type(ind) and ind.dtype == types.bool_:
        ...
        def impl(A, ind):
            cmq__kms, wckz__sztag = array_getitem_bool_index(A, ind)
            return init_decimal_array(cmq__kms, wckz__sztag, precision, scale)
        return impl
    if is_list_like_index_type(ind) and isinstance(ind.dtype, types.Integer):
        ...
        def impl(A, ind):
            cmq__kms, wckz__sztag = array_getitem_int_index(A, ind)
            return init_decimal_array(cmq__kms, wckz__sztag, precision, scale)
        return impl
    if isinstance(ind, types.SliceType):
        ...
        def impl_slice(A, ind):
            cmq__kms, wckz__sztag = array_getitem_slice_index(A, ind)
            return init_decimal_array(cmq__kms, wckz__sztag, precision, scale)
        return impl_slice
    raise BodoError(
