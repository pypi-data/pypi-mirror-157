...
class TupleArrayType(types.ArrayCompatible):
    ...
    @property
    def dtype(self):
        return types.BaseTuple.from_types(tuple(nifwu__qms.dtype for
            nifwu__qms in self.data))
...
class TupleArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        atudy__pkvp = [('data', StructArrayType(fe_type.data))]
        models.StructModel.__init__(self, dmm, fe_type, atudy__pkvp)
...
def init_tuple_arr(typingctx, data_typ=None):
    ...
    joup__alk = TupleArrayType(data_typ.data)

    def codegen(context, builder, sig, args):
        fonlp__rtswd, = args
        qdt__ghyeh = context.make_helper(builder, joup__alk)
        qdt__ghyeh.data = fonlp__rtswd
        context.nrt.incref(builder, data_typ, fonlp__rtswd)
        return qdt__ghyeh._getvalue()
    return joup__alk(data_typ), codegen


@unbox(TupleArrayType)
def unbox_tuple_array(typ, val, c):
    data_typ = StructArrayType(typ.data)
    zxvoh__woun = unbox_struct_array(data_typ, val, c, is_tuple_array=True)
    fonlp__rtswd = zxvoh__woun.value
    qdt__ghyeh = c.context.make_helper(c.builder, typ)
    qdt__ghyeh.data = fonlp__rtswd
    vdtve__zoga = zxvoh__woun.is_error
    return NativeValue(qdt__ghyeh._getvalue(), is_error=vdtve__zoga)


@box(TupleArrayType)
def box_tuple_arr(typ, val, c):
    data_typ = StructArrayType(typ.data)
    qdt__ghyeh = c.context.make_helper(c.builder, typ, val)
    arr = box_struct_arr(data_typ, qdt__ghyeh.data, c, is_tuple_array=True)
    return arr
...
def tuple_arr_getitem(arr, ind):
    ...
    if isinstance(ind, types.Integer):
        oojwl__img = 'def impl(arr, ind):\n'
        buuin__tfs = ','.join(f'get_data(arr._data)[{chvdi__qqrpb}][ind]' for
            chvdi__qqrpb in range(len(arr.data)))
        oojwl__img += f'  return ({buuin__tfs})\n'
        egp__cnya = {}
        exec(oojwl__img, {'get_data': bodo.libs.struct_arr_ext.get_data},
            egp__cnya)
        bldt__hpecd = egp__cnya['impl']
        return bldt__hpecd
...
def tuple_arr_setitem(arr, ind, val):
    ...
    if val == types.none or isinstance(val, types.optional):
        return
    if isinstance(ind, types.Integer):
        dyfa__rrrpn = len(arr.data)
        oojwl__img = 'def impl(arr, ind, val):\n'
        oojwl__img += '  data = get_data(arr._data)\n'
        oojwl__img += '  null_bitmap = get_null_bitmap(arr._data)\n'
        oojwl__img += '  set_bit_to_arr(null_bitmap, ind, 1)\n'
        for chvdi__qqrpb in range(dyfa__rrrpn):
            oojwl__img += (
                f'  data[{chvdi__qqrpb}][ind] = val[{chvdi__qqrpb}]\n')
        egp__cnya = {}
        exec(oojwl__img, {'get_data': bodo.libs.struct_arr_ext.get_data,
            'get_null_bitmap': bodo.libs.struct_arr_ext.get_null_bitmap,
            'set_bit_to_arr': bodo.libs.int_arr_ext.set_bit_to_arr}, egp__cnya)
        bldt__hpecd = egp__cnya['impl']
        return bldt__hpecd
