...
def _install_mpl_types():
    fffdv__avjvi = [('mpl_figure_type', matplotlib.figure.Figure), (
    ...
    for tyiwx__gcyd, mqkcr__ldq in fffdv__avjvi:
        install_py_obj_class(types_name=tyiwx__gcyd, python_type=mqkcr__ldq,
            module=this_module)
...
def generate_matplotlib_signature(return_typ, args, kws, obj_typ=None):
    kws = dict(kws)
    uyrru__yrek = ', '.join(f'e{vbfz__fzx}' for vbfz__fzx in range(len(args)))
    if uyrru__yrek:
        uyrru__yrek += ', '
    ohlvi__vkjy = ', '.join(f"{asmy__onaf} = ''" for asmy__onaf in kws.keys())
    rund__ksnz = 'matplotlib_obj, ' if obj_typ is not None else ''
    srjvr__tnbc = f'def mpl_stub({rund__ksnz} {uyrru__yrek} {ohlvi__vkjy}):\n'
    srjvr__tnbc += '    pass\n'
    vwtq__iahrn = {}
    exec(srjvr__tnbc, {}, vwtq__iahrn)
    udyc__oykq = vwtq__iahrn['mpl_stub']
    bdg__tzta = numba.core.utils.pysignature(udyc__oykq)
    eakq__jkb = ((obj_typ,) if obj_typ is not None else ()) + args + tuple(kws
        .values())
    return signature(return_typ, *unliteral_all(eakq__jkb)).replace(pysig=
        bdg__tzta)


def generate_axes_typing(mod_name, nrows, ncols):
    ara__wsd = '{}.subplots(): {} must be a constant integer >= 1'
    if not is_overload_constant_int(nrows):
        raise_bodo_error(ara__wsd.format(mod_name, 'nrows'))
    if not is_overload_constant_int(ncols):
        raise_bodo_error(ara__wsd.format(mod_name, 'ncols'))
    blpcu__grynp = get_overload_const_int(nrows)
    ccntt__kqr = get_overload_const_int(ncols)
    if blpcu__grynp < 1:
        raise BodoError(ara__wsd.format(mod_name, 'nrows'))
    if ccntt__kqr < 1:
        raise BodoError(ara__wsd.format(mod_name, 'ncols'))
    if blpcu__grynp == 1 and ccntt__kqr == 1:
        rkv__fho = types.mpl_axes_type
    else:
        if ccntt__kqr == 1:
            znhts__ndztw = types.mpl_axes_type
        else:
            znhts__ndztw = types.Tuple([types.mpl_axes_type] * ccntt__kqr)
        rkv__fho = types.Tuple([znhts__ndztw] * blpcu__grynp)
    return rkv__fho


def generate_pie_return_type(args, kws):
    yznx__vjbeo = args[4] if len(args) > 5 else kws.get('autopct', types.none)
    if yznx__vjbeo == types.none:
...
def generate_xcorr_return_type(func_mod, args, kws):
    dwmi__tmnu = args[4] if len(args) > 5 else kws.get('usevlines', True)
    if not is_overload_constant_bool(dwmi__tmnu):
        raise_bodo_error(
            f'{func_mod}.xcorr(): usevlines must be a constant boolean')
    if is_overload_true(dwmi__tmnu):
...
class SubplotsTyper(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        cyyp__szpu = generate_axes_typing('matplotlib.pyplot', nrows, ncols)
        return generate_matplotlib_signature(types.Tuple([types.
            mpl_figure_type, cyyp__szpu]), args, kws)
...
def overload_subplots(fig, nrows=1, ncols=1, sharex=False, sharey=False,
    ...
    cyyp__szpu = generate_axes_typing('matplotlib.figure.Figure', nrows, ncols)
    tyiwx__gcyd = str(cyyp__szpu)
    if not hasattr(types, tyiwx__gcyd):
        tyiwx__gcyd = f'objmode_type{ir_utils.next_label()}'
        setattr(types, tyiwx__gcyd, cyyp__szpu)
    srjvr__tnbc = f"""def impl(
        fig,
        nrows=1,
        ncols=1,
        sharex=False,
        ...
        with numba.objmode(axes="{tyiwx__gcyd}"):
            axes = fig.subplots(
                nrows=nrows,
                ncols=ncols,
                sharex=sharex,
    ...
    """
    vwtq__iahrn = {}
    exec(srjvr__tnbc, {'numba': numba, 'np': np}, vwtq__iahrn)
    impl = vwtq__iahrn['impl']
    return impl
