...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        eueq__mmdj = context.get_abi_sizeof(context.get_data_type(dtype.dtype))
        return context.get_constant(types.intp, eueq__mmdj)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            zqygs__xwo = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(zqygs__xwo.dtype)
            file_write(unicode_to_utf8(fname), zqygs__xwo.ctypes, 
                dtype_size * zqygs__xwo.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            zqygs__xwo = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(zqygs__xwo.dtype)
            mpjyi__mtc = dtype_size * bodo.libs.distributed_api.get_tuple_prod(
                zqygs__xwo.shape[1:])
            _file_write_parallel(unicode_to_utf8(fname), zqygs__xwo.ctypes,
                start, count, mpjyi__mtc)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            fxo__odf = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                fxo__odf = min(fxo__odf, count * dtype_size)
            if fxo__odf < 0:
                return -1
            return fxo__odf
        return impl
