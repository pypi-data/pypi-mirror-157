...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        taktw__dvf = self.overloads[sig]
        idh__qep = taktw__dvf.metadata.get('distributed_diagnostics', None)
        if idh__qep is None:
            nslo__teqln = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(nslo__teqln)
        idh__qep.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        evz__eycf = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(evz__eycf)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        onn__okm = options.pop('distributed')
        pipeline_class = (pipeline_class if onn__okm else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        ncy__ymab = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if ncy__ymab else
            pipeline_class)
    ixcsm__onpww = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(ixcsm__onpww, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(ixcsm__onpww)
        else:
            return master_mode_wrapper(ixcsm__onpww)
    else:
        return ixcsm__onpww
