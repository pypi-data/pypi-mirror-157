...
def _install_mpl_types():
    glryv__lzr = [('mpl_figure_type', matplotlib.figure.Figure), (
    ...
    for lrj__fofq, ktxky__naw in glryv__lzr:
        install_py_obj_class(types_name=lrj__fofq, python_type=ktxky__naw,
            module=this_module)
...
def generate_matplotlib_signature(return_typ, args, kws, obj_typ=None):
    kws = dict(kws)
    iuoa__cenx = ', '.join(f'e{fyble__ydqzr}' for fyble__ydqzr in range(len
        (args)))
    if iuoa__cenx:
        iuoa__cenx += ', '
    vaw__njlvj = ', '.join(f"{diqti__mfxp} = ''" for diqti__mfxp in kws.keys())
    eqw__xvtq = 'matplotlib_obj, ' if obj_typ is not None else ''
    nqb__efwjh = f'def mpl_stub({eqw__xvtq} {iuoa__cenx} {vaw__njlvj}):\n'
    nqb__efwjh += '    pass\n'
    gpu__kfm = {}
    exec(nqb__efwjh, {}, gpu__kfm)
    iuzy__kthk = gpu__kfm['mpl_stub']
    ldmdg__ica = numba.core.utils.pysignature(iuzy__kthk)
    kvn__gnse = ((obj_typ,) if obj_typ is not None else ()) + args + tuple(kws
        .values())
    return signature(return_typ, *unliteral_all(kvn__gnse)).replace(pysig=
        ldmdg__ica)


def generate_axes_typing(mod_name, nrows, ncols):
    qtgfh__mwps = '{}.subplots(): {} must be a constant integer >= 1'
    if not is_overload_constant_int(nrows):
        raise_bodo_error(qtgfh__mwps.format(mod_name, 'nrows'))
    if not is_overload_constant_int(ncols):
        raise_bodo_error(qtgfh__mwps.format(mod_name, 'ncols'))
    pzr__uptv = get_overload_const_int(nrows)
    nek__mhj = get_overload_const_int(ncols)
    if pzr__uptv < 1:
        raise BodoError(qtgfh__mwps.format(mod_name, 'nrows'))
    if nek__mhj < 1:
        raise BodoError(qtgfh__mwps.format(mod_name, 'ncols'))
    if pzr__uptv == 1 and nek__mhj == 1:
        iskc__fkxfa = types.mpl_axes_type
    else:
        if nek__mhj == 1:
            kbh__daisy = types.mpl_axes_type
        else:
            kbh__daisy = types.Tuple([types.mpl_axes_type] * nek__mhj)
        iskc__fkxfa = types.Tuple([kbh__daisy] * pzr__uptv)
    return iskc__fkxfa


def generate_pie_return_type(args, kws):
    ynx__tapni = args[4] if len(args) > 5 else kws.get('autopct', types.none)
    if ynx__tapni == types.none:
...
def generate_xcorr_return_type(func_mod, args, kws):
    fhxa__jwo = args[4] if len(args) > 5 else kws.get('usevlines', True)
    if not is_overload_constant_bool(fhxa__jwo):
        raise_bodo_error(
            f'{func_mod}.xcorr(): usevlines must be a constant boolean')
    if is_overload_true(fhxa__jwo):
...
class SubplotsTyper(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        fri__zbxqs = generate_axes_typing('matplotlib.pyplot', nrows, ncols)
        return generate_matplotlib_signature(types.Tuple([types.
            mpl_figure_type, fri__zbxqs]), args, kws)
...
def overload_subplots(fig, nrows=1, ncols=1, sharex=False, sharey=False,
    ...
    fri__zbxqs = generate_axes_typing('matplotlib.figure.Figure', nrows, ncols)
    lrj__fofq = str(fri__zbxqs)
    if not hasattr(types, lrj__fofq):
        lrj__fofq = f'objmode_type{ir_utils.next_label()}'
        setattr(types, lrj__fofq, fri__zbxqs)
    nqb__efwjh = f"""def impl(
        fig,
        nrows=1,
        ncols=1,
        sharex=False,
        ...
        with numba.objmode(axes="{lrj__fofq}"):
            axes = fig.subplots(
                nrows=nrows,
                ncols=ncols,
                sharex=sharex,
    ...
    """
    gpu__kfm = {}
    exec(nqb__efwjh, {'numba': numba, 'np': np}, gpu__kfm)
    impl = gpu__kfm['impl']
    return impl
