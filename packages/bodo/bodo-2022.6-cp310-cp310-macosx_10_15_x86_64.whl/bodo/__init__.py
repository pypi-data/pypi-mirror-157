...
def _global_except_hook(exctype, value, traceback):
    ...
    ydo__jye = MPI.COMM_WORLD
    tmy__bbaz = ydo__jye.Get_rank()
    xlp__vokt = 3.0
    zbl__zkf = True
    qvteq__xnymc = ydo__jye.Ibarrier()
    evgzt__zgoaa = time.time()
    while time.time() - evgzt__zgoaa < xlp__vokt:
        time.sleep(0.1)
        if qvteq__xnymc.Test():
            zbl__zkf = False
            break
    try:
        ...
        if zbl__zkf:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {tmy__bbaz}. \n')
