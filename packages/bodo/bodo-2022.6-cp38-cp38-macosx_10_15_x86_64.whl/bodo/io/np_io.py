...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        nxgw__bcu = context.get_abi_sizeof(context.get_data_type(dtype.dtype))
        return context.get_constant(types.intp, nxgw__bcu)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            gbh__ekep = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(gbh__ekep.dtype)
            file_write(unicode_to_utf8(fname), gbh__ekep.ctypes, dtype_size *
                gbh__ekep.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            gbh__ekep = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(gbh__ekep.dtype)
            gzux__bzvy = dtype_size * bodo.libs.distributed_api.get_tuple_prod(
                gbh__ekep.shape[1:])
            _file_write_parallel(unicode_to_utf8(fname), gbh__ekep.ctypes,
                start, count, gzux__bzvy)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            rpog__tirjz = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                rpog__tirjz = min(rpog__tirjz, count * dtype_size)
            if rpog__tirjz < 0:
                return -1
            return rpog__tirjz
        return impl
