...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    addw__lvdzf = pq_node.out_vars[0].name
    ejzu__xqcqr = pq_node.out_vars[1].name
    if addw__lvdzf not in lives and ejzu__xqcqr not in lives:
        return None
    elif addw__lvdzf not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.out_used_cols = []
    elif ejzu__xqcqr not in lives:
