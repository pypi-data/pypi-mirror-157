...
class YbConnector:
    ...
    def dfToYb(self,src,target,distribute_on="random",clobber=False,ybTypes=None,verbose=False,ignoreLoadErrors=False):
        ...
        yb_exe="ybload"
        try:
            #Is yb_load on the path? Fine.
            _=subprocess.call(f"{yb_exe} --version")
        except:
            #Is yb_load in the usual Windows-y place?
            yb_exe=r"C:\Program Files\Yellowbrick Data\Client Tools\bin\ybload.exe"
            _=subprocess.call(f'"{yb_exe}" --version')
        
        cmd=f"""{yb_exe} --host {pb.host} --username {pb.user} --dbname {pb.db} --max-bad-rows {maxbad} --csv-skip-header-line true --table {target} {src}"""
