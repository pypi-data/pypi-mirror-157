...
async def gateTypeDifferentiation(x, y, z, p, zoneAccessType):
    ...
    # special case, krokotopia obelisks
        ...
        while await p.is_in_npc_range():
            ...
            await p.send_key(Keycode.X, 0.1)
            await p.send_key(Keycode.X, 0.1)
        logger.debug("Waiting a long ass time for the mouth thing to open")
        await asyncio.sleep(20)
        logger.debug("If you haven't changed zones by now, manually turn on the obelisks and walk to the zone gate.  The script should continue after you change zones")
...
async def goToNewWorld(p, destinationWorld):
    ...
    await p.root_window.debug_print_ui_tree()
...
async def goToDestination(p, destinationZone, p1WorldName, bigStackDestinations, interactiveTeleportersOriginal):
    ...
    if currentWorld != destinationWorld:
        ...
        logger.debug(f'Returning to Spiral Door in: {zoneContainingZoneDoor}')
        ...
        logger.debug(f'User in wrong world - heading to {destinationWorld}')
    ...
    if currentZone.strip() != destinationZone.strip():
        ...
        if currentZoneInZoneMap == False:
            logger.debug(f'Zone not found in zonemap - returning to Hub to reconnect')
    ...
    if teleportedToZone == False:
        if currentZone != destinationZone.strip():
            ...
            if len(pathToCurrentZoneStack) > 0:
                logger.debug('Going back to most recent common zone')
                while i >= 0:
        ...
        if len(pathToDestinationStack) != 0:
            logger.debug(f'Heading to destination zone {destinationZone}')
        ...
        while i < len(pathToDestinationStack):
            ...
            if zone != destinationZone.strip():
                if p1WorldName in ['Empyrea', 'Karamelle', 'Lemuria']:
                    logger.debug(f'Physical gate not found.  Attempting interactive teleport')
...
async def toZone(clients, destinationZone):
    ...
    logger.debug(f'reached destination zone: {destinationZone}.')
...
# Credit to Notfaj and Slackaduts for this function from UniversalBotSF
@logger.catch()
...
async def main(clientHandler):
    ...
    # Hook activation
    for p in clients:
        logger.debug(f"[{p.title}] Activating Hooks")
