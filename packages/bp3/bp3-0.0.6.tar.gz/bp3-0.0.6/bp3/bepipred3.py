        ...
        try: #only using this for biolib implementation 
            if self.run_esm_model_local is not None:
                if self.run_esm_model_local.is_file():
                    subprocess.check_call(['python3', ESM_LOCAL_SCRIPT_PATH, self.run_esm_model_local, fastaPath, self.esm1b_encoding_dir, "--include", "per_tok", "--truncate"])
                else:
                    sys.exit(f"Could not find local model: {self.run_esm_model_local}.")
            else:
                subprocess.check_call(['python3', ESM_SCRIPT_PATH, "esm1b_t33_650M_UR50S", fastaPath, self.esm1b_encoding_dir, "--include", "per_tok", "--truncate"])
