...
# 数据集转换
def transform(data_dir, task, form, output=None, uie_file=None):
    if task == "obj_det":
...
# 预测结果评估
def eva(submit_file, gold_file, task, origin_image_dir=None, long_text_categories=None, output_dir=None):
    if output_dir and not os.path.exists(output_dir):
        os.makedirs(output_dir, exist_ok=True)
    if task == "obj_det":
        det_eva(submit_file, gold_file, origin_image_dir=origin_image_dir, output_dir=output_dir)
    elif task == "text_entity_extra":
        entity_eva(submit_file, gold_file, long_text_categories, output=output_dir)
    elif task == "text_entity_relation_extra":
        ...
        spo_eva(submit_file, gold_file, output=output_dir)


# 数据集分析
def eda(data_dir, task, output_dir, form=None):
    if task == "obj_det":
        det_eda(data_dir, form, output_dir)
    elif task == "text_entity_extra":
        entity_eda(data_dir, output_dir)
