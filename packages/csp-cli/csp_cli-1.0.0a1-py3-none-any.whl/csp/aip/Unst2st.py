...
class Unst2st:
    ...
    def remove_watermark(self, file, output):
        ...
        # import re
        # pattern = r'\((\d{1,3})\)$'
        # re_result = re.search(pattern, name_split[0])
        # if re_result:
        #     re_d = re_result.group(1)
        #     re_all = re_result.group()
        #     output_name = name_split[0].replace(re_all, "") + "(" + str(int(re_d) + 1) + ")" + name_split[1]
        # else:
