...
def authenticate():
    ...
    if r_auth.status_code != 200:
        click.secho(
            textwrap.fill(
                "Error ({}:{}) connecting to Remember the Milk. "
                "Please try again later.".format(r_auth.status_code, r_auth.reason),
                fg="red",
            )
        )
        sys.exit(1)
    ...
    params = {"api_key": api_key, "method": "rtm.auth.getToken", "format": "json", "frob": frob}
    ...
    click.echo("")
    click.echo(
        "Congrats, {}, your account is authenticated!".format(config["USER SETTINGS"]["name"])
    )
    click.echo("")
...
def handle_response(r):
    ...
    if r.status_code != 200:
        click.secho(
            textwrap.fill(
                "Error ({}:{}) connecting to Remember the "
                "Milk. Please try again later.".format(r.status_code, r.reason)
            ),
            fg="red",
        )
...
def create_Task_list(rtm_lists, tag="", dates={}, status=""):
    ...
    if dates.get("due"):
        tasks = [
            ...
            if task.due
            and task.due != "never"
            and arrow.get(task.due).date() == human_date_to_arrow(dates["due"], "due")
        ]
    if dates.get("due_before"):
        tasks = [
            ...
            if task.due
            and task.due != "never"
            and arrow.get(task.due).date() < human_date_to_arrow(dates["due_before"], "due")
        ]
    if dates.get("due_after"):
        tasks = [
            ...
            if task.due
            and task.due != "never"
            and arrow.get(task.due).date() > human_date_to_arrow(dates["due_after"], "due")
        ]
...
def display_tasks(method, list_name, tag, tasks, status, filename=""):
    ...
    if status and status == "incomplete":
        ...
        if method == "export":
            ...
            if incomplete_tasks:
                story.append(
                    Paragraph(str(len(incomplete_tasks)) + " incomplete tasks", styles["Heading2"])
                )
                t = Table(incomplete_tasks_as_lists, colWidths=(col1_width, col2_width))
                t.setStyle(table_style)
                story.append(t)
            ...
            if completed_tasks:
                story.append(
                    Paragraph(str(len(completed_tasks)) + " completed tasks", styles["Heading2"])
                )
                t = Table(completed_tasks_as_lists, colWidths=(col1_width, col2_width))
                t.setStyle(table_style)
                story.append(t)
            ...
            if len(incomplete_tasks_as_lists) > 1:
                click.secho("\nIncomplete Tasks", fg="green")
                print(
                    tabulate(incomplete_tasks_as_lists, headers="firstrow", tablefmt="fancy_grid")
                )
            else:
                print("")
...
@click.option(
    ...
    flag_value="print",
    default=True,
    help="Print tasks to terminal (default).",
)
...
@click.option(
    ...
    "-f",
    default="RTM tasks",
    help='Name of file to create when exporting to pdf (defaults to "RTM tasks").',
)
...
@click.option(
    "--incomplete", "-i", "status", flag_value="incomplete", help="Incomplete tasks only."
)
...
@click.option(
    "--completed_before", "-cb", default="", help="Tasks completed before a particular date."
)
@click.option(
    "--completed_after", "-ca", default="", help="Tasks completed after a particular date."
)
def tasks(
    method,
    list_name,
