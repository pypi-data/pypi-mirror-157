...
class ConnectionModelType(sgqlc.types.Enum):
    ...
    * `DATABRICKS_DELTA`: Databricks Delta
    '''
    __schema__ = schema
    __choices__ = ('ATHENA', 'BIGQUERY', 'DATABRICKS_DELTA', 'DATABRICKS_METASTORE', 'DBT_CLOUD', 'GLUE', 'HIVE', 'HIVE_MYSQL', 'HIVE_S3', 'LOOKER', 'LOOKER_GIT', 'LOOKER_GIT_CLONE', 'LOOKER_GIT_SSH', 'PRESTO', 'PRESTO_S3', 'REDSHIFT', 'S3', 'S3_AIRFLOW_LOG_EVENTS', 'S3_METADATA_EVENTS', 'S3_QL_EVENTS', 'SNOWFLAKE', 'SPARK')
...
class CustomRuleComparisonInput(sgqlc.types.Input):
    __schema__ = schema
    __field_names__ = ('comparison_type', 'full_table_id', 'mcon', 'field', 'metric', 'operator', 'threshold', 'baseline_agg_function', 'baseline_interval_minutes', 'is_threshold_relative', 'threshold_ref', 'min_buffer', 'max_buffer', 'number_of_agg_periods')
...
class CustomRuleComparison(sgqlc.types.Type):
    __schema__ = schema
    __field_names__ = ('comparison_type', 'full_table_id', 'field', 'metric', 'operator', 'threshold', 'baseline_agg_function', 'baseline_interval_minutes', 'is_threshold_relative', 'threshold_ref', 'min_buffer', 'max_buffer', 'number_of_agg_periods', 'data_collection_interval_minutes', 'rule_interval_minutes')
    ...
    '''The uppder bound buffer to modify the alert threshold'''
...
class DbtConnectionStatus(sgqlc.types.Type):
    __schema__ = schema
    __field_names__ = ('uuid', 'last_model_import', 'last_run_result_import')
    uuid = sgqlc.types.Field(UUID, graphql_name='uuid')
    '''The primary identifier for the dbt project'''

    last_model_import = sgqlc.types.Field(Date, graphql_name='lastModelImport')
    '''The date of the last model import we know about'''

    last_run_result_import = sgqlc.types.Field(Date, graphql_name='lastRunResultImport')
    '''The date of the last run result import we know about'''



class DbtConnectionStatusesConnection(sgqlc.types.relay.Connection):
    '''Shows the last known model import and run steps import dates'''
    __schema__ = schema
    __field_names__ = ('page_info', 'edges')
    page_info = sgqlc.types.Field(sgqlc.types.non_null('PageInfo'), graphql_name='pageInfo')
    '''Pagination data for this connection.'''

    edges = sgqlc.types.Field(sgqlc.types.non_null(sgqlc.types.list_of('DbtConnectionStatusesEdge')), graphql_name='edges')
    '''Contains the nodes in this connection.'''



class DbtConnectionStatusesEdge(sgqlc.types.Type):
    '''A Relay edge containing a `DbtConnectionStatuses` and its cursor.'''
    __schema__ = schema
    __field_names__ = ('node', 'cursor')
    node = sgqlc.types.Field(DbtConnectionStatus, graphql_name='node')
    '''The item at the end of the edge'''
...
class DomainOutput(sgqlc.types.Type):
    ...
    __schema__ = schema
    __field_names__ = ('uuid', 'name', 'assignments')
    uuid = sgqlc.types.Field(UUID, graphql_name='uuid')
    '''Domain UUID'''
...
class ICustomRulesMonitor(sgqlc.types.Interface):
    __schema__ = schema
    __field_names__ = ('has_custom_rule_name', 'rule_description', 'rule_comparisons', 'rule_notes', 'rule_variables', 'is_snoozed', 'snooze_until_time', 'slack_snooze_user', 'breach_rate')
...
class IMonitor(sgqlc.types.Interface):
    __schema__ = schema
    __field_names__ = ('uuid', 'monitor_type', 'created_time', 'creator_id', 'resource_id', 'entities', 'schedule_type', 'rule_name', 'is_snoozeable', 'is_paused', 'is_template_managed', 'namespace', 'next_execution_time', 'prev_execution_time', 'is_transitioning_data_provider')
    uuid = sgqlc.types.Field(sgqlc.types.non_null(UUID), graphql_name='uuid')
...
class Query(sgqlc.types.Type):
    ...
    * `last` (`Int`)None
    ...
    get_dbt_connection_statuses = sgqlc.types.Field(DbtConnectionStatusesConnection, graphql_name='getDbtConnectionStatuses', args=sgqlc.types.ArgDict((
        ('uuids', sgqlc.types.Arg(sgqlc.types.list_of(sgqlc.types.non_null(UUID)), graphql_name='uuids', default=None)),
        ('before', sgqlc.types.Arg(String, graphql_name='before', default=None)),
        ('after', sgqlc.types.Arg(String, graphql_name='after', default=None)),
        ('first', sgqlc.types.Arg(Int, graphql_name='first', default=None)),
        ('last', sgqlc.types.Arg(Int, graphql_name='last', default=None)),
...
class SearchResult(sgqlc.types.Type):
    ...
    __schema__ = schema
    __field_names__ = ('mcon', 'lineage_node_id', 'object_type', 'object_id', 'display_name', 'parent_mcon', 'path', 'project_id', 'dataset', 'table_id', 'properties', 'resource_id', 'warehouse_display_name', 'description', 'field_type', 'highlight', 'highlight_properties', 'field_names')
    mcon = sgqlc.types.Field(sgqlc.types.non_null(String), graphql_name='mcon')
    '''Monte Carlo full identifier for an entity'''
...
class DbtProject(sgqlc.types.Type, Node):
    __schema__ = schema
    __field_names__ = ('created_time', 'updated_time', 'uuid', 'account_id', 'project_name', 'source', 'generates_incidents', 'dbt_nodes', 'dbt_edges', 'dbt_runs')
    created_time = sgqlc.types.Field(sgqlc.types.non_null(DateTime), graphql_name='createdTime')
...
class DbtRun(sgqlc.types.Type, Node):
    __schema__ = schema
    __field_names__ = ('created_time', 'updated_time', 'uuid', 'account_id', 'dbt_project', 'dbt_job_definition_id', 'dbt_run_id', 'git_branch', 'run_logs', 'generated_at', 'dbt_run_steps')
    created_time = sgqlc.types.Field(sgqlc.types.non_null(DateTime), graphql_name='createdTime')
