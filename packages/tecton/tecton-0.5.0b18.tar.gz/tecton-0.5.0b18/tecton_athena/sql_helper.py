...
from tecton_athena.data_catalog_helper import register_feature_view_as_athena_table
...
def _get_athena_force_register_fv_in_catalog():
    return conf.get_bool("ATHENA_FORCE_REGISTER_FEATURE_VIEW_IN_CATALOG")
...
def _feature_view_select_all_sql(feature_definition: FeatureDefinition, start_time, end_time):
    athena_table = register_feature_view_as_athena_table(
        feature_definition, session=_get_session(), force_register=_get_athena_force_register_fv_in_catalog()
    )
    ...
    partition_lower_bound = _timestamp_to_partition(
        start_time, athena_table.partition_size_timedelta, athena_table.partition_type
    )
    partition_upper_bound = _timestamp_to_partition(
        end_time, athena_table.partition_size_timedelta, athena_table.partition_type
    )
    ...
    if feature_definition.is_temporal_aggregate:
        ...
        if tecton_spark.offline_store.ANCHOR_TIME in athena_table.hive_columns:
            ...
            del athena_table.hive_columns[tecton_spark.offline_store.ANCHOR_TIME]

        select_columns = list(athena_table.hive_columns.keys())
