...
from tecton_athena.errors import TectonAthenaNotImplementedError
...
class AthenaTable:
    ...
    partition_by: str
    partition_type: str
    partition_size_timedelta: timedelta


# Todo: Refactor. Fairly ugly function
def register_feature_view_as_athena_table(
    feature_definition: FeatureDefinition, session: AthenaSession, force_register: bool
) -> AthenaTable:
    ...
    if store_type == "delta":
        ...
        partition_size_timedelta = offline_store_config.delta.time_partition_size.ToTimedelta()
        if partition_size_timedelta % timedelta(days=1) != timedelta(0):
        ...
        partition_size_timedelta = timedelta(seconds=feature_definition.batch_materialization_schedule.total_seconds())
        ...
        partition_by_interval = int(partition_size_timedelta.total_seconds() * SECONDS_TO_NANOSECONDS)
    else:
    ...
    hive_columns = {}
    for (col, type_name) in materialization_schema.column_name_raw_spark_types():
        hive_columns[col] = SPARK_TO_HIVE_TYPES[type_name]
    ...
    register_table = force_register or not session.does_table_exist(database=athena_database, table=name)
    if register_table:
        logger.info(f"Dropping FeatureView {name} table from Glue Catalog if it exists...")
        session.sql(f"DROP TABLE IF EXISTS {name}")

        logger.info(f"Registering FeatureView {name} table in Glue Catalog database {athena_database}...")
        sql = CREATE_TABLE_TEMPLATE.render(
            ...
            partition_by_range_from=partition_by_range_from,
            partition_by_range_to=partition_by_range_to,
    ...
    return AthenaTable(
        ...
        partition_size_timedelta=partition_size_timedelta,
    )
