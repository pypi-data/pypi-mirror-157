...
class AlphaDatabase(AlphaDatabaseCore):
    ...
    def get_table(self, schema: str, table: str):
        registered_classes = [
            x for x in self.Model._decl_class_registry.values()
        ]
    ...
    def process_entries(self, bind, table, values: list, headers: list = None):
        if headers is not None:
            ...
            entries = [
                table(
                    **{
                        headers[i]: database_lib.convert_value(value)
        ...
        for entry in entries:
            self.session.merge(entry, bind=bind)
        self.session.commit(bind=bind)
