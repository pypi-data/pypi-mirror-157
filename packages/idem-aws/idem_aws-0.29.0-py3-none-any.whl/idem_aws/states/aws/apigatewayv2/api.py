...
async def present(
    ...
    Args:
        ...
        api_key_selection_expression(string, optional): An API key selection expression. Supported only for WebSocket APIs.
            cors_configuration(Dict, optional): A CORS configuration. Supported only for HTTP APIs. See Configuring CORS
            for more information. Defaults to None.
            * (AllowCredentials, optional): Specifies whether credentials are included in the CORS request. Supported
                only for HTTP APIs.
            * (AllowHeaders, optional): Represents a collection of allowed headers. Supported only for HTTP APIs.
            * (AllowMethods, optional): Represents a collection of allowed HTTP methods. Supported only for HTTP APIs.
            * (AllowOrigins, optional): Represents a collection of allowed origins. Supported only for HTTP APIs.
            * (ExposeHeaders, optional): Represents a collection of exposed headers. Supported only for HTTP APIs.
            * (MaxAge, optional): The number of seconds that the browser should cache preflight request results.
