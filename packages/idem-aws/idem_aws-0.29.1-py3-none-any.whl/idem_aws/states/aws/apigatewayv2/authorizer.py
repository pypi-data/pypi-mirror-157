...
async def present(
    ...
    Args:
        ...
        jwt_configuration(Dict, optional): Represents the configuration of a JWT authorizer. Required for the JWT authorizer type.
            ...
            * (Audience, optional): A list of the intended recipients of the JWT. A valid JWT must provide an aud that matches at
                least one entry in this list. See RFC 7519. Supported only for HTTP APIs.
            * (Issuer, optional): The base domain of the identity provider that issues JSON Web Tokens. For example, an Amazon
