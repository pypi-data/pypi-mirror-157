...
def get_invoker_level(invoker_name):
    ...
    wallet_token_info = api.Wallet(invoker_name).get_token(TOKEN_NAME)
...
def main():
    ...
    for op in blockchain.stream(opNames=['comment'], start=start_block, threading=False, thread_num=1):
        ...
        bot_balance = float(api.Wallet(ACCOUNT_NAME).get_token(TOKEN_NAME)['balance'])
        ...
        if ENABLE_TRANSFERS:
            ...
            wallet = api.Wallet(ACCOUNT_NAME, blockchain_instance=HIVE)
