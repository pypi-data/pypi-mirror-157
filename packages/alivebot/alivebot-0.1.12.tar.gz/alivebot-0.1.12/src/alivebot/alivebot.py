...
def get_invoker_level(invoker_name):
    ...
    wallet_token_info = Wallet(invoker_name, api = apiUrl).get_token(TOKEN_NAME)
...
def main():
    ...
    for op in blockchain.stream(opNames=['comment'], start=start_block, threading=False, thread_num=1):
        ...
        bot_balance = float(Wallet(ACCOUNT_NAME, api = apiUrl).get_token(TOKEN_NAME)['balance'])
        ...
        if ENABLE_TRANSFERS:
            ...
            wallet = Wallet(ACCOUNT_NAME, api = apiUrl, blockchain_instance=HIVE)
