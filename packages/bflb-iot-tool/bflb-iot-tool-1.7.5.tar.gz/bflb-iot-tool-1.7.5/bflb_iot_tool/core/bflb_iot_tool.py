...
#         if 'use_romfs' in config["check_box"] and config["check_box"]['use_romfs'] is True:
#             romfs_dir = config['input_path']['romfs_dir_input']
#             if romfs_dir is not None and len(romfs_dir) > 0:
#                 ret = generate_romfs_img(romfs_dir,
#                                          os.path.join(chip_path, chipname, "img_create_iot", "media.bin"))
...
#                 media_bin_d = True
        ...
        if 'mfg_download' in config["check_box"] and config["check_box"]['mfg_download'] is True:
            if 'mfg_addr' in pt_parcel:
                ...
                bflb_utils.printf("Error: mfg is not in partition table")                      
        return self.bl60x_mfg_flasher_eflash_loader_cfg_old(chipname, chiptype, bin_d, boot2_d, ro_params_d,
                pt_parcel, media_bin_d, mfg_bin_d, d0_bin_d, imtb_bin_d, kv_bin_d, yocboot_bin_d, dtb_bin_d)
