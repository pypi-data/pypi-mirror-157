...
class MultiDownloader():
    def __init__(self, url, thread_num, save_path, proxy, contentType=None): #video/mp4
        self.url = url
        self.num = thread_num
        self.name = save_path
        ...
        if self.contentType and contentType != self.contentType:
            raise Exception(f"和指定Content-Type不匹配:{self.contentType}-{contentType}")
