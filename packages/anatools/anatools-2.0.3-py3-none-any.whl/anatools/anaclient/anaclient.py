...
class client:
            ...
            while self.user is None:
                ...
                try: self.user = self.ana_api.login(self.email, self.__password)
                    ...
                    if failcount < 5:
                        ...
                        failcount += 1
                    else: print(f'\rInvalid username or password, consider resetting your password at {envs[environment]["website"]}/forgot-password.'); return
    ...
    def refresh_token(self):
        import time
        from anatools.anaclient.api import api
        if int(time.time())-self.__timer > int(self.user['expires']):
            self.__timer = int(time.time())
