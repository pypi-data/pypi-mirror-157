...
def go_back_test(strategy, rule_type, data_path, face_value, c_rate, slippage, leverage_rate, min_margin_ratio):
    ...
    df = df[df['candle_begin_time'] >= pd.to_datetime('2022-3-01')]
    df = df[df['candle_begin_time'] < pd.to_datetime('2022-7-02')]
    df.reset_index(inplace=True, drop=True)
    ...
    rtn.to_csv(os.path.join(absolute_path, 'result.csv'), index=False)
