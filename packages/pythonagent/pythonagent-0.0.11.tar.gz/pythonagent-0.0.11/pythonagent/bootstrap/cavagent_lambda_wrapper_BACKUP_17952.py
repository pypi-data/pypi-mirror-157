...
try:
    ...
    _agent.logger.error("Cavisson PythonAgent Initialized")
...
def get_handler():
    _agent.logger.info("inside get_handler function first {}".format(sys.executable))
    modandpackage = os.path.abspath('.')
    mod = sys.modules[__name__]

    _agent.logger.info("inside get_handler function {}".format(mod))
    try:
    ...
    try:
        ...
        _agent.logger.info("inside get_handler function module_path {} handler name {}:".format(module_path,handler_name))
    ...
    try:
        ...
        _agent.logger.info("inside get_handler function module_path {}, file_handle {}, pathname {}, desc {}".format(module_path, file_handle, pathname, desc))
        module = imp.load_module(module_path, file_handle, pathname, desc)
        _agent.logger.info("inside get_handler function module {} in cavagent_lambda_wrapper".format(module))
        

    except Exception as e:
        _agent.logger.error("Module not found")
    ...
    try:
        ...
        _agent.logger.info("inside get_handler function handler {}".format(handler))
    except AttributeError:
        _agent.logger.info("NO HANDLER FOUND")
...
def local_customer_application(event, context):
    ...
    _agent.logger.info("inside local_customer_application function Lambda Handler Function")
...
def handler(event, context):
    ...
    _agent.logger.info("function name inside context object in cavwrapper handler {}".format(context_obj.function_name))
    ...
    try:
        ...
        _agent.logger.error("Cavisson PythonAgent Business transation started")
    except Exception as e:
        _agent.logger.error("Error occurred in Start Business transaction Call {}".format(e))


    try:
        handle, handler_name, modandpackage = wrapped_handler(event, context, is_lambda)
    except Exception as e:
        _agent.logger.error("Error occurred in Calling Wrapped Handler {}".format(e))


    _agent.logger.info("inside handler function handle {}, handler_name {}, modandpackage {} IN CAVAGENT_LAMBDA_WRAPPER".format(handle, handler_name, modandpackage))
    ...
    if is_lambda:
        try:
            ...
            _agent.logger.info("inside handler function RESPONSE IN LAMBDA IN CAVAGENT_LAMBDA_WRAPPER response {}".format(response))
        except Exception as e:
            _agent.logger.error("Error while invoking handler {}".format(e))
            _agent.method_exit(bt, fqmmethodentry, 503)
            _agent.set_current_status_code(503)
            rc = _agent.end_business_transaction(bt)
            _agent.logger.error("Cavisson PythonAgent Business Transaction ended")
    ...
    try:
        ...
        _agent.logger.info("Inside handler function: response inside get_handler function first status code {}".format(status_code))

    except Exception as e:
        _agent.logger.error("Error occurred in get status code {}".format(e))
    ...
    try:
        _agent.logger.debug("Url request: {} ".format(context_obj.url_path))
        havoc_monitor.apply_inbound_service_failure(context_obj.url_path)

    except Exception as e:
        if isinstance(e, NDHavocException):
            _agent.logger.error("Havoc Exception {}".format(e))
            _agent.method_exit(bt, fqmmethodentry, 503)
            _agent.set_current_status_code(503)
            rc = _agent.end_business_transaction(bt)
            _agent.logger.error("Cavisson PythonAgent Business Transaction ended")
            ...
            _agent.logger.error("Non-Havoc Exception {}".format(e))
...
###############################FOR UNDERSTANDING HAVOC ISSUES##############################
    ...
    try:
        ...
        _agent.logger.error("Error occurred in End Business transaction Call {}".format(e))
