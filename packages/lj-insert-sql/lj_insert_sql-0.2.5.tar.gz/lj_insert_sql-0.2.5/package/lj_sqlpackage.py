...
class PackageInfo():
    def __init__(self, package_name, package_type, description, home_page, repository_url, license, license_key):
...
class PackageDeps():
    def __init__(self, package_name, package_type, lj_dependency_version_expression, dependency_type, version, dependency_version=None):
...
class SqlTool(object):
        ...
        if pack_id:
            update_package = f"update package_info set `package_name`='%s', `description`='%s', `home_page`='%s', " \
                             f"`repository_url`='%s',`license`='%s', `package_type`='%s', `license_key`= '%s' where `package_name`='%s'" % (
                                 ...
                                 package.license, package.package_type, package.license_key, package.package_name,
                             )
            ...
            sql = f"insert into `package_info`(`package_name`, `description` ,`home_page` ,`repository_url` ," \
                  "`license` ,`package_type`, `license_key`) values('%s', '%s', '%s', '%s', '%s', '%s', '%s')" \
                  % (package.package_name, package.description, package.home_page, package.repository_url,
                     package.license, package.package_type, package.license_key)
            return self.conn_local.execute(sql)
    ...
    def package_version(self, version: PackageVersion):
        ...
        if not data:
            info_sqls = f"insert into `package_info`(`package_name`, `package_type`) values('%s', '%s')" % (
                version.package_name, version.package_type
            )
    ...
    def package_deps(self, deps: PackageDeps):
        ...
        for require in deps.requirement:
            ...
            deps_sql = f"select id from package_dependencies where package_id=%d and dependency_package_name='%s' and " \
                       f"dependency_package_id=%d"%(
                int(_data['id']),deps_id['package_name'],int(deps_id['id'])
            ...
            if dep:
                sql = f"update package_dependencies set dependency_version_expression=%s, lj_dependency_version_expression= %s," \
                      ...
                      f"package_id=%d and dependency_package_name='%s' and " \
                       f"dependency_package_id=%d"
                args_list.append((require['requirement'], '',
                                  '', deps.dependency_type, _data['package_name'],
                                  deps.version, require['app'],
                                  deps_id['id'], _data['id'], _data['package_type'], int(_data['id']),deps_id['package_name'],int(deps_id['id'])))
            elif deps.dependency_version:
                ...
                args_list.append((require['requirement'], '',
                                  deps.dependency_version, deps.dependency_type, _data['package_name'],
                                  deps.version, require['app'],
