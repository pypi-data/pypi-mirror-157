...
class SqlTool(object):
    ...
    def package_info(self, package: PackageInfo):
        ...
        if pack_id:
            ...
            update_package = f"insert into `package_info`(`package_name`, `description` ,`home_page` ,`repository_url` ," \
                  "`license` ,`package_type`, `license_key`) values('%s', '%s', '%s', '%s', '%s', '%s', '%s')" \
                  % (package.package_name, package.description, package.home_page, package.repository_url,
                     verified_license, package.package_type, package.license_key)
    ...
    def package_version(self, version: PackageVersion):
        ...
        if version_id:
            ...
            update_version = "insert into `package_version`(`version`, `license`, `publish_time`, `package_name`, " \
                             "`package_id`, `package_type`, `license_key`) values" "('%s', '%s', '%s', '%s', %d, '%s', '%s')" % \
                             (version.version, verified_license, version.publish_time,
    ...
    def package_Type(self, types: PackageType):
        type_sql = f"select id from package_type where package_type='%s' and name='%s' and `language`='%s' and modify_time='%s'"%(
