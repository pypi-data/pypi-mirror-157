...
def filter_on_start_and_end_date(data, start_time, end_time):
    data['START_DATE'] = pd.to_datetime(data['START_DATE'],
                                        format='%Y/%m/%d %H:%M:%S')
    data['END_DATE'] = np.where(data['END_DATE'] == '2999/12/31 00:00:00',
                                '2100/12/31 00:00:00', data['END_DATE'])
    data['END_DATE'] = pd.to_datetime(data['END_DATE'],
                                      format='%Y/%m/%d %H:%M:%S')
    data = data[(data['START_DATE'] < end_time)
                & (data['END_DATE'] > start_time)]
    return data
...
def filter_on_settlementdate(data, start_time, end_time):
    data['SETTLEMENTDATE'] = pd.to_datetime(data['SETTLEMENTDATE'],
                                            format='%Y/%m/%d %H:%M:%S')
    data = data[(data['SETTLEMENTDATE'] > start_time)
                & (data['SETTLEMENTDATE'] <= end_time)]
    return data
...
def filter_on_interval_datetime(data, start_time, end_time):
    data['INTERVAL_DATETIME'] = pd.to_datetime(data['INTERVAL_DATETIME'],
                                               format='%Y/%m/%d %H:%M:%S')
    data = data[(data['INTERVAL_DATETIME'] > start_time)
...
def filter_on_last_changed(data, start_time, end_time):
    data['LASTCHANGED'] = pd.to_datetime(data['LASTCHANGED'],
                                         format='%Y/%m/%d %H:%M:%S')
...
def construct_interval_datetime_from_period_id(data):
    data['SETTLEMENTDATE'] =\
        np.vectorize(date_2_interval_datetime)(data['SETTLEMENTDATE'],
                                               data['PERIODID'])
    return data
...
def date_2_interval_datetime(date, period):
    ...
    datetime_obj = datetime(year=datetime_obj.year, month=datetime_obj.month,
