    ...
    def run(self):
        ...
        try:
            if config["action"] == "delete":
                ...
                createRepo(repo, self.config)
            else:
                listRepo(repo)
        except Exception as e:
            logger.warning(e)

def createRepo(repo: Path, config):
    if not repo.exists():
        return
...
def main(argv = None):
    ...
    if argv is None:
        argv = sys.argv[1:]
    gp.parseArgs(argv)
    if not sys.platform.startswith('linux') and not gp.config["windows"]:
        logger.warning("Sorry, this script can only run on linux systems.")
        return

    if not is_root() and not gp.config["normal_user"]:
        logger.warning(
            "Try running this script as normal user.\n---------------")
        gp.parser.print_help()
        return
    gp.run()
