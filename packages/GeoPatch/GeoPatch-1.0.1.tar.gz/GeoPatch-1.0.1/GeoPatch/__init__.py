# -*- coding: utf-8 -*-
"""
Created on Thu Jun 23 15:05:26 2022
...
img="D:/LandScene/axu-data/china-s2.tif"
lbl="D:/LandScene/axu-data/china-inv.tif"
...
class Generator:
    ...
    def visualize(self,folder_name="npy",patches_to_show=1, band_num=1, fig_size=(10,20), dpi=96):
        ...
        print(f'image and label for the following patches will be displayed: \n'
              f'{idx}')
        patch_dir=os.listdir(os.path.join(folder_name, "patch"))
        if patch_dir[0][-3:]=="tif":
            fig,ax= plt.subplots(len(idx),2, figsize=fig_size, dpi=dpi)
            for i in range(len(idx)):
                file=os.path.join(os.getcwd(),folder_name,"patch")+"/"+str(idx[i])+"_img.tif"
                print(file)
                file_=os.path.join(os.getcwd(),folder_name,"label")+"/"+str(idx[i])+"_lbl.tif"
                img=rs.open(file)
                img=img.read()
                lbl=rs.open(file_)
                lbl=lbl.read(1)
        ...
        if patch_dir[0][-3:]=="npy":
            fig,ax= plt.subplots(len(idx),2, figsize=fig_size, dpi=dpi)
            for i in range(len(idx)):
                file=os.path.join(os.getcwd(),folder_name,"patch")+"/"+str(idx[i])+"_img.npy"
                print(file)
                file_=os.path.join(os.getcwd(),folder_name,"label")+"/"+str(idx[i])+"_lbl.npy"
                img=np.load(file)
                lbl=np.load(file_)
                if patches_to_show<2:
                    ax[i].imshow(img[:,:,band_num])
