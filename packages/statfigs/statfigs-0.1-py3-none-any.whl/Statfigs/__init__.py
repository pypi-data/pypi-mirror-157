#Virtual Enviornment (location on your computer where python runs and stores its code)
#To create, go to workplace and add new workplace folder. Select the folder you want the new Venv to be saved 
#Delete any previous terminals and open a new terminal selecting the new workplace hosting the new venv

#Directory (location within Folders, path)
import os
...
def bar_upttest(df, para, grp, title, ymax):
    ...
    cat1 = df[df[grp]==[l1[0]]] #subgroup 1 RAW values
    cat2 = df[df[grp]==[l1[1]]] #subgroup 2 RAW values
    results = ttest_ind(cat1[value], cat2[value]) #ttest function
