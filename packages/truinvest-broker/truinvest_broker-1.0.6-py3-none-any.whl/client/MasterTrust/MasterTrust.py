...
class MasterTrustBroker(TradingInterface):
    ...
    def Login(self):
        ...
        response = requests.request("POST", BASE_URL + ROUTE_LOGIN, headers=headers, data=json.dumps(payload))
    ...
    def Positions(self) -> list:
        ...
        response = requests.request("GET", BASE_URL + ROUTE_POSITION, data=payload, headers=headers, params=querystring)
    ...
    def GenerateSession(self, mPin: str):
        ...
        response = requests.request("POST", BASE_URL + ROUTE_TWOFA, headers=headers, data=json.dumps(payload))
        if response.status_code == 200:
            res = response.json()
