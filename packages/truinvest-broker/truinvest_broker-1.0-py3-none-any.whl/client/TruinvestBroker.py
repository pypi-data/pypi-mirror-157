...
class TruinvestBroker(TradingInterface.TradingInterface):
    ...
    def __init__(self, api_key, api_secret, broker, truinvest_secret, is_session_generated=True,
        ...
        if self.broker_name == BROKER_KITE:
            self.broker_obj = KiteTrading.KiteTrading(self.api_key, self.api_secret, self.request_token,
            ...
            self.broker_obj = uTradeBroker.UTradeBroker(username=self.api_key, password=self.api_secret)
        elif self.broker_name == BROKER_MASTER_TRUST:
            self.broker_obj = MasterTrust.MasterTrustBroker(login_id=api_key, password=api_secret,
                                                            truinvest_secret=truinvest_secret)
