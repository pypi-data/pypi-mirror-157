...
def create_view_on_clause(on_clause_view_name, prepared_dataset_name, mapper_table, mapper_id, model_code_view, **kwargs):
...
class sto_framework():
    ...
    def create_training_process(self, id_process, prepared_dataset_name,  id_mapper, feature_store, **kwargs):

        on_clause_view_name = f"{self.database}.V_STO_{prepared_dataset_name.split('.')[1]}"
        mapper_table = self.mapper_training
    ...
    def run_training_process(self, id_process, feature_store, train_fold = 'train'):
        ...
        if type(train_fold) == 'str':
            filter_ = f"WHERE STO_FOLD_ID = '{selected_fold}'"
        elif type(train_fold) == 'list':
    ...
    def insert_new_scoring_process(self,id_process, final_view, on_clause,
                                    prepared_dataset, id_mapper, feature_store, **kwargs):


        process_type = 'sto_scoring'
        mapper_table = self.mapper_training
        model_code_view = self.code_model_view
        process_catalog = feature_store.process_catalog
        sto_database = self.SEARCHUIFDBPATH
