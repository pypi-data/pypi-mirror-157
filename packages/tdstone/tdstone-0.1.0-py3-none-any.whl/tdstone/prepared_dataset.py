...
def preparedataset_from_teradata_dataframe(dataset,ID_Columns,Partition,FoldID):
    ...
    new_cols = {
        'STO_ROW_ID'       : tdml.sqlalchemy.literal_column(f"CAST(JSON_COMPOSE({','.join(ID_Columns)}) AS VARCHAR(2000) CHARACTER SET UNICODE)"),
...
def preparedataset_from_table(schema, view_name, input_table_name, input_schema_name, ID_Columns,Partition,FoldID):
    ...
    query = f"""
        ...
        SEL
            A.*
        ,   CAST(JSON_COMPOSE({','.join(ID_Columns)}) AS VARCHAR(2000) CHARACTER SET UNICODE) AS STO_ROW_ID
