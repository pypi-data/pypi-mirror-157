...
def create_process_catalog(process_catalog):
    ...
    (
        ...
        FINAL_VIEW VARCHAR(2000) CHARACTER SET UNICODE NOT CASESPECIFIC,
        ...
        MAPPER_TABLE VARCHAR(2000) CHARACTER SET UNICODE NOT CASESPECIFIC,
        MAPPER_ID BIGINT,
...
def create_view_process_input_output(process_input_output, process_catalog, calculated_feature_view):
    # create a view that describes the input and the output of the process

    sql_create_view = f"""
        -- GET THE INPUT AND THE TARGET FOR A GIVEN PROCESS
        REPLACE VIEW {process_input_output} AS
        ...
        ,	PROCESS.MAPPER_TABLE
        ,	PROCESS.MAPPER_ID
        ,	PROCESS.MODEL_CODE_VIEW
        ,	PROCESS.STO_DATABASE
        ,	FEATURE.LOCATION AS EFS_LOCATION
        FROM {process_catalog} PROCESS
        LEFT JOIN {calculated_feature_view} FEATURE
        ON PROCESS.PROCESS_ID = FEATURE.PROCESS_ID;    
    """

    return sql_create_view

def insert_new_process(id_process, process_type, final_view, on_clause,
                       prepared_dataset, mapper_table, mapper_id, model_code_view,
                       process_catalog, sto_database, **kwargs):
    ...
    res = tdml.get_context().execute(f'sel count(*) from {process_catalog} where process_id = {id_process}').fetchall()[0][0]
    if res == 0:
        sql_insert_query = f"""
        CURRENT VALIDTIME INSERT INTO {process_catalog}
        (PROCESS_ID, PROCESS_TYPE , FINAL_VIEW, ON_CLAUSE, PREPARED_DATASET, MAPPER_TABLE, MAPPER_ID, MODEL_CODE_VIEW, STO_DATABASE)
        VALUES
        ({id_process}, '{process_type}', '{final_view}','{on_clause}',
        '{prepared_dataset}', '{mapper_table}', {mapper_id}, '{model_code_view}'
        , '{sto_database}'
        );
        """
    elif len(final_view)>0:
        sql_insert_query = f"""
...
class feature_store():
    ...
    def setup(self):
        ...
        sql_queries['process_input_output'] = create_view_process_input_output(
            self.process_input_output,
            self.process_catalog,
            self.calculated_feature_view)

        execute_querydictionary(sql_queries)
        return

    def clean(self):
        tdml.db_drop_view(self.process_input_output.split('.')[1])
    ...
    def register_object(self,ID, object_feature):
        ...
        sql_insert = f"""
        INSERT INTO {object_description_table} 
    ...
    def register_feature(self,ID, feature, feature_description_table):
        # create a feature with its description
        # ID : unique ID of the feature
        # object: a dictionary with the name and description fields
        # feature_description_table : the destination table
    ...
    def insert_new_calculated_feature(self,id_process, map_features_process, **kwargs):
        ...
        #
        # map_features_process : list of dictionary. [{CALCULATED_FEATURE_ID, FEATURE_NAME, PROCESS_ID}]
        ...
        (CALCULATED_FEATURE_ID, FEATURE_ID, PROCESS_ID)
        SEL 
    ...
    def get_process_parameters(self, process_id):
        ...
        sql_sel_query = f"""
            ...
            , EFS_LOCATION
            , STO_DATABASE
            FROM {self.process_input_output}
        ...
        try:
            res['efs_root_name'] = res['efs_root_name'].replace('_FLOAT','').replace('_INTEGER','')
        except Exception as e:
            res['efs_root_name'] = res['efs_root_name']
