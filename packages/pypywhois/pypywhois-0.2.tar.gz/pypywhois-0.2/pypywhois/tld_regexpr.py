...
# The .de NIC pypywhois servers no longer provide any PII data for domains in the TLD.
...
# The Japanese pypywhois servers always return English unless a Japanese locale is specified in the user's LANG environmental variable.
# See: https://www.computerhope.com/unix/uwhois.htm
# Additionally, pypywhois qeuries can explicitly request english:
# 	To suppress Japanese output, add'/e' at the end of command, e.g. 'pypywhois -h pypywhois.jprs.jp xxx/e'.
#
jp = {
    "domain_name": r"\[Domain Name\]\s?(.+)",
    #    'registrar':                None,
...
pt = {
    ...
    # manual lookup: use the website at pypywhois.dns.pt
...
# WHOIS server: pypywhois.donuts.co
_donuts = {
    "extend": "com",
    "_server": "pypywhois.donuts.co",
    "registrant": r"Registrant Organization:\s?(.+)",
    "status": r"Domain Status:\s?(.+)",
}
...
# WHOIS server: pypywhois.centralnic.com
_centralnic = {
    "extend": "com",
    "_server": "pypywhois.centralnic.com",
    "domain_name": r"Domain Name:\s?(.+)",
    "registrar": r"Registrar:\s*(.+)",
    "creation_date": r"Creation Date:\s?(.+)",
    "expiration_date": r"Registry Expiry Date:\s?(.+)",
...
# mboot added start
# note i extract the pypywhois server for each toplevel domain using: https://github.com/jophy/iana_tld_list
...
bet = {
    "extend": "ac",
    "_server": "pypywhois.nic.bet",
}

bg = {
    "extend": None,
    "_server": "pypywhois.register.bg",
...
bid = {
    "extend": "ac",
    "_server": "pypywhois.nic.bid",
}

# Benin
# WHOIS server: pypywhois.nic.bj
# by: https://github.com/LickosA

bj = {
    "_server": "pypywhois.nic.bj",
    "extend": "com",
    "domain_name": r"Domain Name:\s?(.+)",
    "registrar": r"Registrar:\s*(.+)",
    "creation_date": r"Creation Date:\s?(.+)",
...
srl = {
    "_server": "pypywhois.afilias-srs.net",
    "extend": "ac",
    "registrant_country": r"Registrant Country:\s+(.+)",
}
...
# WHOIS server: pypywhois.gtld.knet.cn
_gtldKnet = {
    "extend": "com",
    "_server": "pypywhois.gtld.knet.cn",
...
accountant = {"extend": "com", "_server": "pypywhois.nic.accountant"}
cricket = {"extend": "com", "_server": "pypywhois.nic.cricket"}
date = {"extend": "com", "_server": "pypywhois.nic.date"}
faith = {"extend": "com", "_server": "pypywhois.nic.faith"}
hiphop = {"extend": "com", "_server": "pypywhois.nic.hiphop"}
loan = {"extend": "com", "_server": "pypywhois.nic.loan"}
party = {"extend": "com", "_server": "pypywhois.nic.party"}
racing = {"extend": "com", "_server": "pypywhois.nic.racing"}
ren = {"extend": "com", "_server": "pypywhois.nic.ren"}
review = {"extend": "com", "_server": "pypywhois.nic.review"}
science = {"extend": "com", "_server": "pypywhois.nic.science"}
webcam = {"extend": "com", "_server": "pypywhois.nic.webcam"}

# Registry operator: UNR Corp.
# WHOIS server: pypywhois.uniregistry.net
_uniregistry = {
    "extend": "com",
    "_server": "pypywhois.uniregistry.net",
}
...
# WHOIS server: pypywhois.teleinfo.cn
_teleinfo = {
    "extend": "com",
    "_server": "pypywhois.teleinfo.cn",
}
...
# some actually dont have a working pypywhois server
...
va = {"extend": "_privateReg"}  # This TLD has no pypywhois server.
vu = {"extend": "_privateReg"}  # all dates 1970 , no furter relevant info
vn = {"extend": "_privateReg"}
#
zw = {"extend": "_privateReg"}  # Zimbabwe ; # This TLD has no pypywhois server
com_zw = {"extend": "zw"}
org_zw = {"extend": "zw"}
...
np = {
    "extend": "_privateReg"
}  # This TLD has no pypywhois server, but you can access the pypywhois database at https://www.mos.com.np/
com_np = {"extend": "np"}
...
hopto_org = {"extend": "_privateReg"}  # dynamic dns without any pypywhois
duckdns_org = {"extend": "_privateReg"}  # dynamic dns without any pypywhois
# changeip_com = {"extend": "_privateReg"}  # dynamic dns without any pypywhois
# dnsdynamic_org = {"extend": "_privateReg"}  # dynamic dns without any pypywhois
# noip_com = {"extend": "_privateReg"}  # dynamic dns without any pypywhois
# freedns_afraid_org = {"extend": "_privateReg"}  # dynamic dns without any pypywhois
# dyndns_com = {"extend": "_privateReg"}  # dynamic dns without any pypywhois
# sitelutions_com = {"extend": "_privateReg"}  # dynamic dns without any pypywhois
# 3322_org = {"extend": "_privateReg"}  # dynamic dns without any pypywhois
