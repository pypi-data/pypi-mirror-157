"""
    ...
    >>> print(domain.__dict__)  # print(pypywhois.get('google.com'))
...
def query(
    ...
    server:             if set use the pypywhois server explicitly for making the query:
                        propagates on linux to "pypywhois -h <server> <domain>"
                        propagates on Windows to pypywhois.exe <domain> <server>
    ...
    if thisTld.get("_privateRegistry"):
        msg = "This tld has either no pypywhois server or responds only with minimal information"
        raise WhoisPrivateRegistry(msg)

    # allow explicit pypywhois server usage
