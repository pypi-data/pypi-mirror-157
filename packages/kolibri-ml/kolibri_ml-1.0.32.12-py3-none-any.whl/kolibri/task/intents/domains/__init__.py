...
self.intent_generator=IntentExpression(intents={})

def get_intent_expression(text, langauge):
    return self.intent_generator.get_intent_expression(text, langauge)"""

for domain, file in __available_doamins().items():
    import_code(code=code_template.format(file), name=domain)


g=payments.get_intent_expression("ma facture est tres elevée", "fr")