...
def opt_tt(Y, nswp_outer=2, nswp=5, r=20, e=1.E-8, log=False):
    ...
    is_max_better = not (y_min < 0 and -y_min > y_max)
    r_curr = erank(Y)

    _log(y_min, y_max, r=r_curr, is_outer=True, with_log=log)
    ...
    for swp in range(nswp_outer):
        ...
        i_min, y_min, i_max, y_max = _opt_tt_iter(Y, i_min, y_min, i_max, y_max,
            not is_max_better, nswp, r, e, log)
        i_min, y_min, i_max, y_max = _opt_tt_iter(Y, i_min, y_min, i_max, y_max,
...
def _log(y_min=None, y_max=None, swp=None, r=None, is_max=False, is_outer=False, y_new=None, with_log=False):
    if not with_log:
        return
...
def _opt_tt_iter(Y, i_min, y_min, i_max, y_max, is_max, nswp, r, e, log=False):
    ...
    for swp in range(nswp):
        ...
        if (is_max and y_opt_new > y_opt) or (not is_max and y_opt_new < y_opt):
            i_opt = i_opt_new.copy()
            y_opt = y_opt_new

        _log(
            y_min if is_max else y_opt,
            y_opt if is_max else y_max,
            swp, r_curr, is_max, y_new=y_opt_new, with_log=log)
