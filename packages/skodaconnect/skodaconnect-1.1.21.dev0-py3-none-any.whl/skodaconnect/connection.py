            ...
            # Modify timers if action is on, off or schedule
                ...
                # Set departure schedule
                    ...
                    if data.get('schedule', {}).get('recurring', False):
                        _LOGGER.debug('Schedule to set is cyclic')
                        timers[timerid]['timerFrequency'] = 'cyclic'
                        timers[timerid]['departureWeekdayMask'] = data.get('schedule', {}).get('days', 'ynnynny')
                        timers[timerid]['departureTimeOfDay'] = data.get('schedule', {}).get('time', '12:30')
                        timers[timerid].pop('departureDateTime', None)
                    else:
                        _LOGGER.debug('Schedule to set is single departure')
                        timers[timerid]['timerFrequency'] = 'single'
                        #timers[timerid]['departureWeekdayMask'] = 'ynnynny'
                        #timers[timerid]['departureTimeOfDay'] = '12:30'
                        timers[timerid].pop('departureWeekdayMask', None)
                        timers[timerid].pop('departureTimeOfDay', None)
