...
class HopscotchDict(MutableMapping[Hashable, Any]):
    ...
    @staticmethod
    def _get_displaced_neighbors(
        lookup_idx: int, nbhd: int, nbhd_size: int, max_size: int
    ) -> list[int]:
        """
    ...
    def _get_lookup_index_info(self, lookup_idx: int) -> tuple[int, list[int]]:
        ...
        neighbors = self._get_displaced_neighbors(
            lookup_idx, nbhd, self._nbhd_size, self._size
        )
            ...
            if nbr_data_idx < 0:
                raise RuntimeError(
                    (
                        ...
                        "free index"
                    ).format(abs(hash(key)) % self._size)
                )
    ...
    def _set_lookup_index_info(
        self, lookup_idx: int, data: int | None = None, nbhd: int | None = None
    ) -> None:
        """
        ...
        lookup_offset = calcsize(self._pack_fmt) * lookup_idx
        data_idx, neighbors = unpack_from(
            self._pack_fmt, self._lookup_table, lookup_offset
        )
        ...
        pack_into(
            self._pack_fmt, self._lookup_table, lookup_offset, data_idx, neighbors
        )
