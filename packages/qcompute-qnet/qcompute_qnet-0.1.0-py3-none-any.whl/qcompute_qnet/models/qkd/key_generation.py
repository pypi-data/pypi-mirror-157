...
class PrepareAndMeasure(KeyGeneration, ABC):
    ...
    def receive_message(self, msg: "ClassicalMessage") -> None:
        r"""Node receives a classical message.

        Args:
            msg (ClassicalMessage): message used for the prepare-and-measure protocol
        """
        ...
        if msg.src == self.peer and self.status == KeyGeneration.Status.WORKING:
            ...
            if self.role == PrepareAndMeasure.Role.TRANSMITTER:
                self.transmitter_receive_message(msg)
            elif self.role == PrepareAndMeasure.Role.RECEIVER:
                self.receiver_receive_message(msg)

    @abstractmethod
    def transmitter_receive_message(self, msg: "ClassicalMessage") -> None:
        r"""Transmitter receives a classical message from the receiver.

        Args:
            msg (ClassicalMessage): message for the prepare-and-measure protocol
        """
        pass

    @abstractmethod
    def receiver_receive_message(self, msg: "ClassicalMessage") -> None:
...
class BB84(PrepareAndMeasure):
    ...
    class BB84Message(ClassicalMessage):
        r"""Class for the classical control messages in BB84 protocol.
        """

        def __init__(self, src: "Node", dst: "Node", protocol: type, data: Dict):
            r"""Constructor for BB84Message class.

            Args:
                src (Node): source of BB84Message
                dst (Node): destination of BB84Message
                protocol (type): protocol of BB84Message
    ...
    def transmitter_start_protocol(self) -> None:
        ...
        start_msg = BB84.BB84Message(src=self.node,
                                     dst=self.peer,
                                     protocol=BB84,
                                     data={'type': BB84.BB84Message.Type.START,
                                           'clock': clock,
    ...
    def receiver_stop_reception(self) -> None:
        ...
        if self.status == KeyGeneration.Status.WORKING:
            ...
            request_bases_msg = BB84.BB84Message(src=self.node,
                                                 dst=self.peer,
                                                 protocol=BB84,
                                                 data={'type': BB84.BB84Message.Type.BASES_REQUEST,
                                                       'round': self.round})
    ...
    def transmitter_receive_message(self, msg: "BB84.BB84Message") -> None:
        r"""Transmitter receives a BB84 message from the receiver.

        Args:
            msg (BB84.BB84Message): message for BB84 protocol
        """
        photon_round = msg.data['round']

        if msg.data['type'] == BB84.BB84Message.Type.BASES_REQUEST:
            ...
            bases_msg = BB84.BB84Message(src=self.node,
                                         dst=self.peer,
                                         protocol=BB84,
                                         data={'type': BB84.BB84Message.Type.BASES,
                                               'peer_bases': bases,
                                               'round': photon_round})
            self.node.send_classical_msg(dst=self.peer, msg=bases_msg)

        elif msg.data['type'] == BB84.BB84Message.Type.MATCHING:
    ...
    def receiver_receive_message(self, msg: "BB84.BB84Message") -> None:
        r"""Receiver receives a BB84 message from the transmitter.

        Args:
            msg (BB84.BB84Message): message for BB84 protocol
        """
        if msg.data['type'] == BB84.BB84Message.Type.START:
        ...
        elif msg.data['type'] == BB84.BB84Message.Type.BASES:
            photon_round = msg.data['round']
            ...
            matching_msg = BB84.BB84Message(src=self.node,
                                            dst=self.peer,
                                            protocol=BB84,
                                            data={'type': BB84.BB84Message.Type.MATCHING,
...
class DecoyBB84(PrepareAndMeasure):
    ...
    class DecoyBB84Message(ClassicalMessage):
        r"""Class for the classical control messages in DecoyBB84 protocol.
        """

        def __init__(self, src: "Node", dst: "Node", protocol: type, data: Dict):
            r"""Constructor for DecoyBB84Message class.

            Args:
                src (Node): source of DecoyBB84Message
                dst (Node): destination of DecoyBB84Message
                protocol (type): protocol of DecoyBB84Message
                data (Dict): message content
            """
            super().__init__(src, dst, protocol, data)
        ...
        @unique
        class Type(Enum):
            r"""Class for DecoyBB84Message types.
            """
    ...
    def transmitter_start_protocol(self) -> None:
        ...
        start_msg = DecoyBB84.DecoyBB84Message(src=self.node,
                                               dst=self.peer,
                                               protocol=DecoyBB84,
                                               data={'type': DecoyBB84.DecoyBB84Message.Type.START,
                                                     'clock': clock,
                                                     'source_frequency': self.node.photon_source.frequency,
                                                     'light_duration': self.light_duration,
                                                     'emission_rest': self.emission_rest})
    ...
    def transmitter_start_emission(self) -> None:
        ...
        if self.status == KeyGeneration.Status.WORKING:
            ...
            self.scheduler.schedule_now(handler)
            finish_emission_msg = DecoyBB84.DecoyBB84Message(src=self.node,
                                                             dst=self.peer,
                                                             protocol=DecoyBB84,
                                                             data={'type': DecoyBB84.DecoyBB84Message.Type.FINISH_EMISSION,
                                                                   'round': self.round})
    ...
    def receiver_stop_reception(self) -> None:
        ...
        if self.status == KeyGeneration.Status.WORKING:
            ...
            bases_msg = DecoyBB84.DecoyBB84Message(src=self.node,
                                                   dst=self.peer,
                                                   protocol=DecoyBB84,
                                                   data={'type': DecoyBB84.DecoyBB84Message.Type.BASES,
                                                         'peer_bases': bases,
                                                         'round': self.round})
            self.node.send_classical_msg(dst=self.peer, msg=bases_msg)
    ...
    def transmitter_receive_message(self, msg: "DecoyBB84.DecoyBB84Message") -> None:
        r"""Transmitter receives a DecoyBB84 message from the receiver.

        Args:
            msg (DecoyBB84.DecoyBB84Message): message for DecoyBB84 protocol
        """
        photon_round = msg.data['round']

        # if self.node.env.now < self.end_time:
        if msg.data['type'] == DecoyBB84.DecoyBB84Message.Type.BASES:
            self.node.env.logger.debug(f"{self.node.name} received bases from {self.peer.name}"
                                       f" (Round {photon_round})")
            ...
            matching_msg = DecoyBB84.DecoyBB84Message(src=self.node,
                                                      dst=self.peer,
                                                      protocol=DecoyBB84,
                                                      data={'type': DecoyBB84.DecoyBB84Message.Type.MATCHING,
                                                            'matching_list': matching_list,
    ...
    def receiver_receive_message(self, msg: "DecoyBB84.DecoyBB84Message") -> None:
        r"""Receiver receives a DecoyBB84 message from the transmitter.

        Args:
            msg (DecoyBB84.DecoyBB84Message): message for DecoyBB84 protocol
        """
        if msg.data['type'] == DecoyBB84.DecoyBB84Message.Type.START:
            self.node.env.logger.debug(f"{self.node.name} received message 'START' from {self.peer.name}")
        ...
        elif msg.data['type'] == DecoyBB84.DecoyBB84Message.Type.FINISH_EMISSION:
            # Stop receiving photons after a light duration
            handler = EventHandler(self, "receiver_stop_reception")
            self.scheduler.schedule_now(handler)

        elif msg.data['type'] == DecoyBB84.DecoyBB84Message.Type.MATCHING:
            photon_round = msg.data['round']
            self.node.env.logger.debug(f"{self.node.name} received matching indices from {self.peer.name} "
                                       f"(Round: {photon_round})")
