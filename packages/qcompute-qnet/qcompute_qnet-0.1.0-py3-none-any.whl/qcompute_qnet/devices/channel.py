...
class ClassicalChannel(Channel, ABC):
...
class QuantumChannel(Channel, ABC):
...
class ClassicalFiberChannel(ClassicalChannel):
    ...
    Note:
        The lossy probability of a clasical fiber channel is set to be zero for simplicity.

    Examples:
        We can create a classical fiber channel between two nodes (e.g. alice and bob) by two approaches.
        The first approach is to instantiate a ``ClassicalFiberChannel`` with given ``name`` and ``distance``.
        Then using the ``connect`` method to make a connection to alice and bob.

        >>> c_a2b = ClassicalFiberChannel(name="c_a2b", distance=1e3)
...
class QuantumFiberChannel(QuantumChannel):
    ...
    Examples:
        We can create a quantum fiber channel between two nodes (e.g. alice and bob) by two approaches.
        The first approach is to instantiate a ``QuantumFiberChannel`` with given ``name`` and ``distance``.
        Then using the ``connect`` method to make a connection to alice and bob.

        >>> q_a2b = QuantumFiberChannel(name="q_a2b", distance=1e3)
