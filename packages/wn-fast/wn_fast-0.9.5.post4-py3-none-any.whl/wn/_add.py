...
def _insert_lexicon(
    lexicon: _AnyLexicon, info: dict, cur: sqlite3.Cursor, progress: ProgressHandler
) -> Tuple[int, int]:
...
def _insert_entries(
    ...
    for batch in _batch(_local_entries(entries)):
        data = (
            (entry["id"], lexid, entry["lemma"]["partOfSpeech"], entry["meta"])
            for entry in batch
        )
        cur.executemany(query, data)
        progress.update(len(batch))
...
def _insert_forms(
    ...
    for batch in _batch(entries):
        forms: List[
            Tuple[Optional[str], int, str, int, str, Optional[str], Optional[str], int]
        ] = []
        for entry in batch:
            eid = entry["id"]
...
def remove(
    lexicon: str, progress_handler: Optional[Type[ProgressHandler]] = ProgressBar
) -> None:
