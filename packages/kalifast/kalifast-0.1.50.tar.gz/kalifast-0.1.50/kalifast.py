...
class KFT :
    ...
    def getAllPosition(self, img=False, timeout=DefaultTimeout, onError=False, ref=CENTER, at=[0,0], correlation=DefaultCorrelation, parentZone=DefaultParentZone) :
        ...
        try :
            ...
            if len(zones) > 1 :
                ...
                #      position_y = zone[0][1]
                if ref == self.LEFT :
                    positions = [[ int(zones[i][0][0]+at[0]), int(zones[i][0][1]+ ((zones[i][1][1] - zones[i][0][1])/2)+at[1]) ] for i in range(len(zones[0]))]

                #      position_x = zone[0][0]
                #      position_y = zone[0][1]+(height/2)
                if ref == self.CENTER :
                    positions = [[ int(zones[i][0][0]+((zones[i][1][0] - zones[i][0][0])/2)+at[0]), int(zones[i][0][1]+ ((zones[i][1][1] - zones[i][0][1])/2)+at[1]) ] for i in range(len(zones[0]))]
                ...
                if ref == self.BOT_LEFT :
                    positions = [[ int(zones[i][0][0]+at[0]), int(zones[i][0][1]+ (zones[i][1][1] - zones[i][0][1])+at[1]) ] for i in range(len(zones[0]))]

                #     position_x = zone[0][0]
                #     position_y = zone[0][1]+height
                if ref == self.BOT_RIGHT :
                    positions = [[ int(zones[i][1][0]+at[0]), int(zones[i][1][1]+at[1]) ] for i in range(len(zones[0]))]

                #     position_x = zone[1][0]
                #     position_y = zone[1][1]
                if ref == self.BOT :
                    positions = [[ zones[i][0][0]+((int(zones[i][1][0] - zones[i][0][0])/2)+at[0]), int(zones[i][0][1]+ (zones[i][1][1] - zones[i][0][1])+at[1]) ] for i in range(len(zones[0]))]
    ...
    def getOCRText(self, zone=ENTIRE_SCREEN, onError=False, tesseract_exe='C:\\Program Files\\Tesseract-OCR\\tesseract.exe', readOnlyTransp=True) :
        ...
        if self._checkTypeZone(zone) == False and self._checkTypeZones(zone) == False and zone != self.ENTIRE_SCREEN:
            if onError != False :
                return onError
        ...
        if self._checkTypeZones(zone) == True :
            for z in zone :
                ...
                img = ImageOps.expand(img, border=50, fill=colorHex)
