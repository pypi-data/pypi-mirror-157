...
class NonparametricKernel(torch.nn.Module):
    def __init__(self, n_samples: int, n_members: int, n_classes: int, parameters: np.ndarray = None):
        ...
        if parameters is None:
            ...
            self._params = torch.nn.Parameter(torch.from_numpy(np.expand_dims(parameters, 0).repeat(n_samples, 0)), requires_grad=True)
...
class NormalKernel(torch.nn.Module):
    ...
    def __init__(self, n_samples: int, n_members: int, n_classes: int, parameters: np.ndarray = None):
        ...
        self.n_classes = n_classes
        if parameters is None:
            self._params = torch.nn.Parameter(torch.randn(n_samples, self.N_PARAMETERS, n_members)+5.0, requires_grad=True)
        else:
            assert parameters.ndim == 2
