...
class ScrapyPlaywrightDownloadHandler(HTTPDownloadHandler):
    def __init__(self, crawler: Crawler) -> None:
        ...
        if "PLAYWRIGHT_DEFAULT_NAVIGATION_TIMEOUT" in crawler.settings:
            with suppress(TypeError, ValueError):
                logger.info("Set default navigation timeout")
    ...
    async def _download_request(self, request: Request, spider: Spider) -> Response:
        ...
        try:
            ...
            logger.info(f"[{request.url}] Start finally")
            try:
                if page is not None:
                    if not page.is_closed():
                        logger.info(f"[{request.url}] Close page")
                        try:
                            await asyncio.wait_for(page.close(), timeout=10)
                        except asyncio.TimeoutError:
                            await self._plan_user_agent_rotation()
                        logger.info(f"[{request.url}] Page closed")
            except:
                logger.error(f"[{request.url}] Error on page close")
                raise
            if self.context_semaphores['default'].locked():
                logger.info(f"[{request.url}] Release semaphore")
                self.context_semaphores['default'].release()
