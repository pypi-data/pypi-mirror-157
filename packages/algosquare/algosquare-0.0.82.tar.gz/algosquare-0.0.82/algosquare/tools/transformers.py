...
class BinaryTransformer(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, nan_unknowns = True):
        ...
        self._nan_unknowns = nan_unknowns
...
class CategoricalImputer(BaseEstimator, TransformerMixin):
    """
    Imputes categoricals with constant and transforms unknown as missing.
    ...
    def transform(self, X):
        """
        Replaces missing and unknowns with fill_value.
        ...
        X[pd.concat(idx_unknowns, axis=1) | pd.isnull(X)] = self._fill_value
        return X
