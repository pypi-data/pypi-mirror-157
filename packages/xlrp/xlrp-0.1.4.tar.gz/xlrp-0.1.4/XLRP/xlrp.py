...
def _log_path():
    path_dir = Path().cwd() / 'Xlrp_Log'
    path_dir.mkdir(exist_ok=True)
    file = path_dir / 'xlrp.log'
...
class StepName:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        global _failed_count, _passed_count, _error_count
    ...
    def __call__(self, func):
        ...
        @wraps(func)
        def decorator(*args, **kwargs):
            global _passed_count, _failed_count, _error_count
            ...
            try:
                ...
                if type(res) is bool:
                    ...
                    if case_status is False:
                        ...
                        _failed_count += 1
                        Logger.error(log)
                        _write_log(log_file, 'The custom assertion failed')
                    else:
                        log = f'{step}----Passed'
                        Logger.success(log)
                        _passed_count += 1
        ...
        if type(param_iter) not in (list, tuple):
            ...
            for param in param_iter:
                try:
                    ...
                    ...
        print('='*20, f'Passed: {_passed_count} Failed: {_failed_count} Error: {_error_count}', '='*20)
        return self
