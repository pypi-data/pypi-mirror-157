...
class Local:
    ...
    def send_active_time(self,tz: int = int(-timezone // 1000), timers: list = None):
        ...
        req = self.session.post(api(f"/x{self.comId}/s/community/stats/user-active-time?sid={self.sid}"), headers=headers.Headers(data=data).headers, proxies=self.proxies, data=data)
        if req.status_code != 200: return CheckExceptions(req.json())
        else: return Json(req.json())
