...
class SLocal:
    ...
    async def send_active_time(self,tz: int = int(-timezone // 1000), timers: list = None):
        ...
        async with self.session.post(api(f"/x{self.comId}/s/community/stats/user-active-time?sid={self.sid}"), headers=headers.Headers(data=data).headers, proxies=self.proxies, data=data) as req:
            if req.status != 200: return CheckExceptions(await req.json())
            else: return Json(await req.json())
