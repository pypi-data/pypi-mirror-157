...
class top_most:
    ...
    def get_atts(self):
        if not self.second:
            ...
            self.lemmas = pi.open_pickle(f'{fold}clemmas2', 1)
    ...
    def output(self, kind=1):
        ...
        if kind == 2:
            pi.save_pickle(self.lemmas, f'{fold}co_lemmas2', 1)
        if kind == 9:
...
class step_three(top_most):
    ...
    def fdiph_fu(self, med=0):
        if med:
            self.lemmas = pi.open_pickle(f'{fold}flemmas', 1)
            self.quick_fix()
            self.i2j = pi.open_pickle(f'{fold}i2j', 1)
        if not self.debug and not self.kind2 in ['e', 'f']:
            self.lemmas['nereides']['geninfm'] = ['nēried']
            self.lemmas['adrastis']['geninfm'] = ['ādrastid']
            self.lemmas['aeetias']['geninfm'] = ['aeētiad']
            self.lemmas['sanguinans']['geninfm'] = ['sanguinānt']
...
class step_one(step_two):
    ...
    def jv_rep(self):
        ...
        for k, v in self.col_dct.items():
            k1 = jv.replace(k)
            if k1 in self.lemmas:
                dct[k1] = v

        self.col_dct = dct
        self.lemmas = {k: v for k, v in self.lemmas.items() if k in self.col_dct}
        return
    ...
    def research_ostem(self, k):
        if self.lemmas[k].get('ogeninf') or self.lemmas[k].get('operf'):
            return 1
