    ...
    for i in range(n_main_iter):
        ...
        if input_data_in_center and return_data_in_center:
            ...
            for j in range(datasets[i].shape[0]):
                point_idx_target.append(
                        np.where(np.all(np.isclose(triangle_center_datasets[j,], triangle_center_surface), axis=1))[0])
...
def project_on_scalp(coords, mesh, scalp_tag=1005):
    ...
    from pynibs import TetrahedraLinear
...
def check_islands_for_single_elm(source_elm, connectivity=None, adjacency=None, one_node_neighbor=1):
    """
    ...
    one_node_neighbor : int, default: 1
        Number of neighbors to define one-node-neighbors. Use '2' to only return a list of 2-node-neighbors
    ...
    neighbors_visited = {i: False for i in np.where(adjacency[source_elm] == one_node_neighbor)[0]}
    ...
    neighs_to_check = set(np.where(adjacency[source_elm] > one_node_neighbor)[0].tolist())
    ...
    while neighs_to_check:
        ...
        for idx in np.arange((np.bincount(adjacency[elm])[(one_node_neighbor+2):]).sum()):
            i = sort[idx]
...
def find_islands(connectivity=None, adjacency=None, verbose=False, one_node_neighbor=1):
    """
    ...
    for elm_source in tqdm(range(adjacency.shape[0]), desc="Checking for islands."):
        _, n_not_visited, visited = check_islands_for_single_elm(elm_source, adjacency=adjacency, one_node_neighbor=one_node_neighbor)
        elms_not_visited = [i for i, v in visited.items() if v]
        ...
        if n_not_visited:
            ...
            islands.append(elm_source)
    a = np.argpartition(counter_not_visited, 20)
    counter_not_visited[1029]
    min(counter_not_visited)
    max(counter_not_visited)
    np.argwhere(counter_not_visited == 2)
    return islands


def find_island_elms(connectivity=None, adjacency=None, verbose=False, one_node_neighbor=1):
    """
    Searches for islands in a mesh and returns element indices of the smallest island.

    Island is defines as a set of elements, which share a single node with the rest of the mesh.
    ...
    all_islands = find_islands(connectivity, verbose=True, one_node_neighbor=one_node_neighbor)

    # find the size of the islands
    one_node_neighs = {k: np.where(adjacency[k] == one_node_neighbor)[0] for k in all_islands}
    ...
    for island, one_node_neigh in one_node_neighs.items():
        ...
        for island_i in one_node_neigh:
            n_visited, n_not_visited, _ = check_islands_for_single_elm(island_i, adjacency=adjacency,
                                                                       one_node_neighbor=one_node_neighbor + 1)
    ...
    _, _, elm_idx_from_smalles_island = check_islands_for_single_elm(smallest_island, one_node_neighbor=one_node_neighbor + 1,
                                                                     adjacency=adjacency)
