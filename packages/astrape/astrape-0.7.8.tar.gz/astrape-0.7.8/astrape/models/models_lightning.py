...
class _UNet(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(dims, int):
            ...
            if self.dataformats == "NCHW":
                dims = dims
            elif self.dataformats == "NHCW":
                height, channels, width = dims
                dims = (channels, height, width)
            elif self.dataformats == "NHWC":
                height, width, channels = dims
                dims = (channels, height, width)
            elif self.dataformats == "NCWH":
                channels, width, height = dims
                dims = (channels, height, width)
            elif self.dataformats == "NWHC":
                width, height, channels = dims
                dims = (channels, height, width)
            elif self.dataformats == "NWCH":
                width, channels, height = dims
                dims = (channels, height, width)
            else:
                raise ValueError(f"Invalid dataformats {self.dataformats}. It should start with 'N' followed by permutation of 'HWC'.")
        else:
            raise ValueError(f"Wrong dimension {dims} for an input.")
        self.dims = dims # (in_channels, height, width)
