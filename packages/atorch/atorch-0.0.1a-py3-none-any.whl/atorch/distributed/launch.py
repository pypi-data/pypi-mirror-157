...
Changes:
  ...
  2. use_env set default to True to use env for LOCAL_RANK.
...
def parse_args():
    ...
    parser.add_argument(
        ...
        "'local rank'. In atorch, the default value is True. "
...
def check_process_loop(processes):
    ...
    while running:
        ...
        if len(failed_processes) > 0:
            ...
            print(
                "subprocess failed with LOCAL_RANK, return code: "
                + str(failed_processes)
            )
...
def main(args):
    ...
    if args.nnodes > 0:
        nnodes = args.nnodes
    dist_world_size = args.nproc_per_node * nnodes
    ...
    for local_rank in range(0, args.nproc_per_node):
        ...
        if args.nsys_profiling_rank == dist_rank:
            ...
            envs = "LOCAL_RANK={},RANK={},WORLD_SIZE={},".format(
                local_rank, dist_rank, dist_world_size,
            )
            envs += "MASTER_ADDR={},MASTER_PORT={}".format(
                current_env["MASTER_ADDR"], current_env["MASTER_PORT"]
            )
            cmd.extend(["-e", envs])
        ...
        if with_python:
            ...
            if not args.use_env:
                raise ValueError(
                    "When using the '--no_python' flag, you must also set "
                    "the '--use_env' flag."
                )
            if args.module:
                raise ValueError(
                    "Don't use both the '--no_python' flag and the '--module' "
