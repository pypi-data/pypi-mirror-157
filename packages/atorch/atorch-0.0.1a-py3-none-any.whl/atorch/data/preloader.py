...
class GpuPreLoader(object):
    ...
    def to_gpu(self, data):
        if isinstance(data, collections.abc.Mapping):
            return {key: self.to_gpu(data[key]) for key in data}
        elif isinstance(data, collections.abc.Sequence):
            return [self.to_gpu(v) for v in data]
    ...
    def to_gpu_with_mask(self, data):
        if isinstance(data, collections.abc.Mapping):
            return {
                key: self.to_gpu(data[key] if self.mask[idx] else data[key])
                for idx, key in enumerate(data)
            }
        if isinstance(data, collections.abc.Sequence):
            return [
                self.to_gpu(v) if self.mask[idx] else v
