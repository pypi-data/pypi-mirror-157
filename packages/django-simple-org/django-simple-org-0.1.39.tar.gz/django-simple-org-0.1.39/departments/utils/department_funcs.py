...
def delete_department_user_map(organization_id: int, department_id: int, user_ids: list):
    ...
    :return:
    """
    department_instance = Department.objects.filter(organization_id=organization_id, pk=department_id).first()
    if not department_instance.exists():
        return Resp(code=BadRequestCode, msg="当前组织没有该部门")
    ...
    departments = Department.objects.filter(**dic).values_list("id", flat=True)
