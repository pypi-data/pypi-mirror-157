...
class TrainPipelineParameters(ABC):
    ...
    def __init__(self, automl_settings: Union[AutoMLBaseSettings, Dict[str, Any]]):
        if isinstance(automl_settings, AutoMLBaseSettings):
            self.automl_settings = automl_settings.as_serializable_dict()
        else:
            self.automl_settings = automl_settings
            message = "Please use 'AutoMLBaseSettings' class with 'ForecastingParameters' class to define "\
                      "'automl_base_settings' instead of directly supplying 'automl_settings' dictionary "\
