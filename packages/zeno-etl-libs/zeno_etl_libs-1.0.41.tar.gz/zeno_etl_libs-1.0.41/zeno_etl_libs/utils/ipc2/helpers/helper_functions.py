...
def add_correction_flag(df, corr_drug_list, corr_flag):
    if "corrections" not in df.columns:
        df["corrections"] = ""
    df["corrections"] = df["corrections"].fillna("")
        
    df["corrections"] = np.where((df["drug_id"].isin(corr_drug_list))&(df["corrections"]!=""), 
                                 df["corrections"]+'-'+corr_flag, df["corrections"])
    df["corrections"] = np.where((df["drug_id"].isin(corr_drug_list))&(df["corrections"]==""), 
                                 corr_flag, df["corrections"])
