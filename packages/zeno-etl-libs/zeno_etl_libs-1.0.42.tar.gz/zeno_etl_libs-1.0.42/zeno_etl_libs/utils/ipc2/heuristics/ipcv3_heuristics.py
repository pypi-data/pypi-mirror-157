...
def v3_corrections(final_ss_df, store_id, corrections_selling_probability_cutoff,
    ...
    drugs_lst = list(df_corrections_list["drug_id"].unique())
    drugs_lst_str = str(drugs_lst).replace('[', '(').replace(']', ')')

    q_prob = f"""
        select *
        from "{schema}"."ipc-corrections-rest-cases"
        where "store-id" = {store_id}
        and "drug-id" in {drugs_lst_str}
        """
    q_prob_111 = f"""
        select *
    ...
    #     final_ss_df[['store_id', 'drug_id', 'curr_inventory']],
