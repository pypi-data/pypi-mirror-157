...
def safety_stock_calc(agg_fcst, cal_sales, store_id, reset_date, v3_active_flag,
    ...
    # capping ss-doh to 15 days
    logger.info(f"DOH1 (SS-DOH) Correction starts")
    df_pre_corr = safety_stock_df.copy()
    safety_stock_df['safety_stock_max'] = np.round(
        (safety_stock_df['fcst'] / 28) * 15)
    safety_stock_df['safety_stock'] = np.where(
    ...
    corr_drug_lst = compare_df(df_pre_corr, df_post_corr,
    ...
    # keep minimum 14 days stock
    logger.info(f"DOH2 (OUP-DOH) Correction starts")
    df_pre_corr = safety_stock_df.copy()
    safety_stock_df["order_upto_point_min"] = np.round(
        14 * safety_stock_df['fcst'] / fcst_weeks / 7)
    safety_stock_df['order_upto_point'] = np.where(
        safety_stock_df['order_upto_point'] < safety_stock_df['order_upto_point_min'],
        safety_stock_df['order_upto_point_min'], safety_stock_df['order_upto_point'])
    safety_stock_df.drop('order_upto_point_min', axis=1, inplace=True)
    df_post_corr = safety_stock_df.copy()
    ...
    corr_drug_lst = compare_df(df_pre_corr, df_post_corr,
                               cols_to_compare=['order_upto_point'])
