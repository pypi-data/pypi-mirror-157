...
class TestCase(Node):
    ...
    def __run(
        ...
        start_time = now()
        try:
            set_execution_log(self, msg=f"{dependence.__name__} start.")
            dependence()
            ...
            status_code = STATUS.PASSED
        end_time = now()
        set_execution_log(self, msg=f"{dependence.__name__} end.")
    ...
    def teststep_thread(self, step: Step):
        ...
        result = "passed" if PISTAR_STATUS.PASSED == report.status else "failed"
        set_execution_log(self, msg=f"step {step.name} end, result: {result}")
