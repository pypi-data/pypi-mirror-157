...
class VisParser(Network):
    ...
    def _prepare_write_(
        ...
        for i in range(len(nodes)):
            ...
            nodes[i]['shape'] = NODE_TYPE_SHAPES.get(nodes[i].get('node_molecule_type'), 'hexagon')
            if nodes[i].get('node_molecule_type'):
                nodes[i]['label'] = nodes[i].get('representation_id', '')

            if nodes[i].get('data_name') is not None:
                if nodes[i].get('inferred'):
...
class DynamicVisParser(VisParser):
    ...
    def write_vis(
        ...
        network_params['nodes'], network_params['edges'] = \
            self._add_annotations_(network_params['nodes'], network_params['edges'])
