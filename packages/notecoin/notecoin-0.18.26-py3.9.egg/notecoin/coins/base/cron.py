...
def load_month(exchange: Exchange, tmp_path, timeframe='1m'):
    ...
    for month_index in range(1, 10):
        ...
        unix_start = int(datetime(today.year, today.month - month_index, 1).timestamp() * 1000)
        unix_end = int((datetime(today.year, today.month + 1 - month_index, 1) - timedelta(seconds=1)).timestamp() * 1000)
