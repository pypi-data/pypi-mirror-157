    ...
    def date_cols(self, data, cols=None):
        ...
        if cols:
            if self.conf.process.date_cols_num_cores == 1 or self.conf.process.date_cols_num_cores is None:
                data = self._handle_date_cols(data, cols)
            else:
                data = parrallelize_dataframe(data, partial(self._handle_date_cols, cols=cols),
                                              num_cores=self.conf.process.date_cols_num_cores)

            return data

    def _handle_date_cols(self, data, cols=None):
