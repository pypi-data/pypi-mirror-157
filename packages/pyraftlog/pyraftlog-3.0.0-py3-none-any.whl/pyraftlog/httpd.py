...
class RaftHTTPServer(socketserver.ThreadingMixIn, http.server.HTTPServer):
    raft = None
    transport = None
...
class RaftHTTPRequestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == '/' or self.path == '':
            ...
            if self.path == '/log':
                ...
                search = re.search(r'^/log/(\d)$', self.path)
                if not search:
