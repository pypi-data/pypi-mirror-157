...
class PickleStorage(FileStorage):
    """
    This storage type uses `pickle` to serialise the state which is then written to disk.
    """
    ...
    def retrieve(self, state):
        if self.exists():
            with open(self.file_path, 'rb') as f:
                state.populate(pickle.loads(f.read()))
...
class RedisStorage(Storage):
    ...
    def persist(self, state):
        self.redis_client.set(self.key_prefix + 'state', pickle.dumps(state, 2))
