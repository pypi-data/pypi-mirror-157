...
from .message import (
    Message,
    APPEND_ENTRIES as MESSAGE_APPEND_ENTRIES,
    APPEND_RESPONSE as MESSAGE_APPEND_RESPONSE,
    VOTE_REQUEST as MESSAGE_VOTE_REQUEST,
    VOTE_RESPONSE as MESSAGE_VOTE_RESPONSE
)
...
class State(object):
    ...
    def __init__(self, role, name, neighbourhood=None):
        ...
        # the latest term server has seen (increases monotonically)
        self.current_term = 0
        ...
        # index of the highest log entry known to be committed (increases monotonically)
        self.commit_index = 0
        # index of the highest log entry applied to state machine (increases monotonically)
        self.last_applied = 0
        ...
        self.next_index = {}
        # for each node, index of the highest log entry known to be replicated on node (increases monotonically)
        self.match_index = {}
    ...
    def _current_vote_for(self, name: str) -> bool:
        """ Check whether the current term voted was for `name`. """
        return self.voted_for[self.current_term] == name
    ...
    def increment_term(self, name, neighbour):
        ...
        :rtype: bool
        """
        if neighbour in self.votes or self.current_term not in self.voted_for or not self._current_vote_for(name):
            self.current_term += 1
    ...
    def _count_votes(self, for_or_against: bool) -> int:
        """
        Count the number of votes either for or against.

        :param for_or_against: Whether should count votes for (True) or against (False)
        """
        return len([x for x in self.votes.values() if x == for_or_against])
    ...
    def on_vote_request(self, node, msg):
        ...
        return self.vote_response_message(node,
                                          data['candidate_id'],
                                          self._current_vote_for(data['candidate_id']),
                                          eligible)
    ...
    def on_vote_response(self, node, msg):
        ...
        if node.has_majority(self._count_votes(True)):
        ...
        # If we are not eligible to be elected
            ...
            if self._count_votes(True) == 1 and self._count_votes(False) >= 1:
                return None
    ...
    def vote_request_message(self, candidate, recipient):
        ...
        :rtype: pyraftlog.message.Message
        """
        return Message.build(MESSAGE_VOTE_REQUEST, candidate, self, recipient, {
    ...
    def vote_response_message(self, voter, candidate, response, eligible=True):
        ...
        :rtype: pyraftlog.message.Message
        """
        return Message.build(MESSAGE_VOTE_RESPONSE, voter, self, candidate, {
            "response": response,
            "eligible": eligible,
        })
    ...
    def append_entry_message(self, leader, recipient, heartbeat=False):
        ...
        return Message.build(MESSAGE_APPEND_ENTRIES, leader, self, recipient, {
    ...
    def append_response_message(self, follower, recipient, response, reason):
        ...
        :rtype: pyraftlog.message.Message
        """
        return Message.build(MESSAGE_APPEND_RESPONSE, follower, self, recipient, {
            "response": response,
            "reason": reason,
            "last_applied": self.last_applied,
