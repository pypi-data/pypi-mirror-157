...
class Interleaving:
	...
	def get_freq_agility(self) -> bool:
		...
		Snippet: value: bool = driver.scenario.localized.interleaving.get_freq_agility() \n
		...
		"""
		response = self._core.io.query_str('SCENario:LOCalized:INTerleaving:FREQagility?')
		return Conversions.str_to_bool(response)

	def set_freq_agility(self, freq_agility: bool) -> None:
		"""SCPI: SCENario:LOCalized:INTerleaving:FREQagility \n
		Snippet: driver.scenario.localized.interleaving.set_freq_agility(freq_agility = False) \n
		Enables frequency agility in interleaving. Requires R&S SMW with firmware version 5.xx.xxx and higher. To query the
		installed firmware version of the selected instrument, use the command method RsPulseSeq.Instrument.firmware. \n
			:param freq_agility: ON| OFF| 1| 0
		"""
		param = Conversions.bool_to_str(freq_agility)
		self._core.io.write(f'SCENario:LOCalized:INTerleaving:FREQagility {param}')

	def get_value(self) -> bool:
		"""SCPI: SCENario:LOCalized:INTerleaving \n
		Snippet: value: bool = driver.scenario.localized.interleaving.get_value() \n
		...
		"""
		response = self._core.io.query_str('SCENario:LOCalized:INTerleaving?')
		return Conversions.str_to_bool(response)

	def set_value(self, interleaving: bool) -> None:
		"""SCPI: SCENario:LOCalized:INTerleaving \n
		Snippet: driver.scenario.localized.interleaving.set_value(interleaving = False) \n
		If enabled, multiple PDW lists are interleaved into a single output file using a priority-based dropping algorithm. Set
		the priority with the command method RsPulseSeq.Scenario.Cpdw.priority. \n
			:param interleaving: ON| OFF| 1| 0
		"""
		param = Conversions.bool_to_str(interleaving)
		self._core.io.write(f'SCENario:LOCalized:INTerleaving {param}')
