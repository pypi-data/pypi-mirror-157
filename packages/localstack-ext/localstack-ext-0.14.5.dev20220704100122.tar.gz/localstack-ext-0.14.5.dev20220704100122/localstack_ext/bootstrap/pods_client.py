...
class CloudPodsVersionManager(CloudPodsManager):
	...
	def _add_state_to_cloud_pods_store(A,extract_assets=_A,services=_C):
		...
		for (B,G) in C.state.items():
			for (H,I) in G.backends.items():A.pods_api.create_state_file_from_fs(file_name=H,service=B.service,region=B.region,root=_F,account_id=B.account_id,serialization=Serialization.MAIN,object=E(I))
		if extract_assets:
