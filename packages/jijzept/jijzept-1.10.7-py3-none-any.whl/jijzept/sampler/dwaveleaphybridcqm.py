...
class JijLeapHybridCQMParameters:
    ...
    Attributes:
        time_limit (Union[int, float]): the maximum run time, in seconds, the solver is allowed to work on the given problem.
        Must be at least the minimum required for the problem, which is calculated and set by default. It is deprecated to set this up due to high credit consumption.
...
class JijLeapHybridCQMSampler(JijModelingInterface):
    ...
    def sample_model(
        ...
        **kwargs,
        ...
        sampler = JijLeapHybridCQMParameters(config="XX", token_leap="XX")
