...
class Encoder(Module):
    def __init__(self,
        ...
        self.snr = snr
        self.t = 1
        self.enc_net = Sequential(Linear(in_dim, m_dim))
        self.A = nn.Parameter(torch.randn(in_dim, m_dim))
    ...
    def forward(self, x):
        ...
        x = x  # + noise
        A = gumbel_sigmoid(self.A, self.t, self.hard)
        # y = self.enc_net(x)
        # A = self.A
        y = x.mm(A)
...
class Decoder(Module):
    def __init__(self,
                 ...
                 **kwargs):
        super().__init__()
        self.dec_net = Sequential(Linear(m_dim, hidden_dim),
                                  Mish(),
...
class CSNet(Module):
    def __init__(self,
        ...
        self.decoder = DecoderSCVI(
            n_input=self.m_dim, n_output=self.in_dim)
    ...
    def forward(self, batch):
        ...
        loss_dict = {}
        loss_dict['A_loss'] = A_loss
        xhat, loss_dict['recon_loss'] = self.recon(
            recon_type='ZINB', target=x, px_rate=px_rate, px_r=px_r, px_dropout=px_dropout)
        loss_dict['cos_loss'] = (1-F.cosine_similarity(x, px_rate, dim=1)).sum()
        loss_dict['cos_loss2'] = (1-F.cosine_similarity(x, px_rate, dim=0)).sum()
        
        loss_dict['loss'] = 0        
        for loss_str, lambda_ in zip(['recon_loss', 'A_loss', 'cos_loss', 'cos_loss2'], [1, self.lambda_sparsity, self.lambda_cos, self.lambda_cos]):
            loss_ = loss_dict[loss_str]
            loss_ = loss_ * lambda_ #/ (loss_/loss_dict['recon_loss']).detach() 
