...
class ReportingExecutor(DefaultExecutor):
    ...
    def execute(self, request: ExecutionRequest) -> ExecutionResult:
        ...
        completion_mcp = self._build_completion_mcp(exec_request=request, status=status, start_time_ms=start_time_ms, duration_ms=duration_ms, report=report)
    ...
    def _build_kickoff_mcp(self, exec_request: ExecutionRequest, start_time_ms: int) -> MetadataChangeProposal: 
    ...
    def _build_completion_mcp(self, exec_request: ExecutionRequest, status: str, start_time_ms: int, duration_ms: int, report: str) -> MetadataChangeProposal: 
        key_aspect = self._build_execution_request_key_aspect(exec_request.exec_id)
        result_aspect = self._build_execution_request_result_aspect(
            status=status,
            start_time_ms=start_time_ms,
    ...
    # Builds an MCP to report the completion of execution request handling 
    def _build_empty_cancel_mcp(self, exec_id: str) -> MetadataChangeProposal: 
    ...
    def _build_execution_request_key_aspect(self, execution_request_id: str) -> GenericAspectClass:
        # Build the JSON map
        execution_request_key: Dict[str, Any] = {
            "id": execution_request_id
        }

        serialized = json.dumps(execution_request_key)
    ...
    def _build_execution_request_result_aspect(
        ...
        # Build the JSON map for execution result . 
        ...
        serialized = json.dumps(pre_json_transform(execution_request_result))
