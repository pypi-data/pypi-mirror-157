...
class TwitchIRC:
        ...
        while True:
            ...
            for message in resp.split("\n"):
                ...
                if message.startswith('PING'):
                    ...
                    m = parsers.parse_message(message)

                    self._session_channel = m.channel
                    self._execute_message(m)
                    self._session_channel = None
    ...
    def start_async(self, thread_name: str = "TwitchChatBot", daemon: bool = True) -> threading.Thread:
        ...
        t = threading.Thread(target=self.start, name=thread_name, daemon=daemon)
        t.start()
        return t
